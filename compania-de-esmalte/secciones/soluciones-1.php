...
<body>
  ...
  <div class="info-general pag-soluciones">
      ...
      <div class="con-soluciones">
      	<img src="<?php echo Link::Build() ?>/imagenes/titulos/home-1.png" width="500" height="100" alt="">
      	<!--<h2>Nuestras</h2>
      	<h1>Soluciones</h1>-->
        <div class="con-nav-sol">
            ...
            ?>
          	<a class="item-solucion <?= ($sol==1)?"sub-actual ":" "; ?>itemsol1" data-id="solucion-1"><li class="bt-nav-sol bt-nav-sol-1"><img src="<?php echo Link::Build() ?>/imagenes/solucion-t1.png"></li></a>
            <a class="item-solucion <?= ($sol==2)?"sub-actual ":" "; ?>itemsol2 " data-id="solucion-2"><li class="bt-nav-sol bt-nav-sol-2"><img src="<?php echo Link::Build() ?>/imagenes/solucion-t2.png"></li></a>
            <a class="item-solucion <?= ($sol==3)?"sub-actual ":" "; ?>itemsol3 " data-id="solucion-3"><li class="bt-nav-sol bt-nav-sol-3"><img src="<?php echo Link::Build() ?>/imagenes/solucion-t3.png"></li></a>
          </ul>
        </div>     
...
for($general=0; $general<3; $general++): ?>
        <div id="solucion-<?=($general+1)?>" class="con-solucion" style="display:<?= ($sol==($general+1))?"block":"none"; ?>">
            ...
            <div>
              ...
              <ul class="submenu-sol <?= $activosub; ?>" data-id="<?= $coccion[$i]['id']; ?>" style="display:<?php 
                if($cat==$coccion[$i]['id'] || empty($cat)) :
                    echo"block"; $cat=$coccion[$i]['id'];
                else : echo "none";
                endif; ?>">
                  ...
                  for($h=0, $count=  count($subs); $h<$count; $h++):
                      ...
                      ?><li><a class="item-gal-<?=($general+1)?> <?= ($scat==$consecutivo)?$activosubnav:""; ?>" data-id="info-sol-<?=($general+1)?>_<?= $consecutivo ?>" data-title="<?= $activosubnav ?>"><?php echo utf8_encode($subs[$h]['titulo']); ?></a></li><?php
                  endfor;
                  ?>
              </ul>
            </div>
...
?>
            <div id="info-sol-<?=($general+1)?>_<?= $consecutivo ?>" class="con-item-soluciones-<?=($general+1)?>" style="display:<?= ($sol == ($general+1) && 
                         $cat == $coccion[$i]['id'] &&
                         $scat==$consecutivo || empty($scat))?'block':"none"; ?>">
              <div class="paging_site-<?=($general+1)?>_<?= $consecutivo ?>" data-id="<?= 
								($sol == ($general+1) && 
								$cat == $coccion[$i]['id'] && 
								$scat==$consecutivo)?$pag:"1";  ?>">
                <ul class="content_paging">
                  <?php $pag_actual=1;?>
                    <?php for($p=0; $p<$contadorproductos; $p++):
                    $pag_actual=ceil(($p+1)/12);
                        ?>
                  <li>
                    <a href="<?php echo Link::Build() ?>/site/solucion-info/&idp=<?=$productos1[$p]['id']."&sol=".$coccion[$i]['lineas_id']."&cat=".$coccion[$i]['id']."&scat=".$consecutivo."&pag=".$pag_actual?>">
                      <div class="item-sol">
                        <img src="<?php echo Link::Build() ?>/img/productos/<?php echo str_replace('original','redimension',$productos1[$p]['imagen1']) ?>" width="220" height="208" alt="">
                        <div class="over-item-sol">
                          <div class="pick-over-sol"><img src="<?php echo Link::Build() ?>/imagenes/pick-over-sol.png" width="42" height="42" alt=""></div>
                          <p><?php echo utf8_encode($productos1[$p]['nombre']); ?></p>
                          <h4>REFERENCIA</h4>
                          <h3><?php echo utf8_encode("#".$productos1[$p]['referencia']); ?></h3>  
                        </div>
                      </div>
                    </a>
