		...
		if ($page->entry_id and $page->layout->stream_id)
		{
			...
			if ($stream)
			{
				...
				if ($entry = $this->streams->entries->get_entries($params))
				{
					...
					{
								if(isset($entry['entries'][0]["description_{$stream->stream_slug}"]))
								     $entry['entries'][0]['description_encode'] = urlencode(strip_tags($this->quitar_tildes($entry['entries'][0]["description_{$stream->stream_slug}"])));
			                    
			                 //   if(isset($entry['entries'][0]["image_{$stream->stream_slug}"]))
								//     $entry['entries'][0]['image_encode'] = urlencode($this->quitar_tildes($entry['entries'][0]["image_{$stream->stream_slug}"]->filename));

								$entry['entries'][0]['title_encode'] = urlencode($this->quitar_tildes($page->title));
								$entry['entries'][0]['url_current_encode'] = urlencode($this->quitar_tildes(current_url()));
								$entry['entries'][0]['url_base_encode'] = urlencode($this->quitar_tildes(base_url()));
		...
		if ($page and $page->type_id and $get_data)
		{
			...
			if ($stream)
			{
				...
				if (isset($ret['entries'][0]))
				{
					...
					$ret['entries'][0]['description_encode'] = "holamundo";
                    $ret['entries'][0]['image_encode'] = "holamundo";
					$ret['entries'][0]['title_encode'] = "holamundo";
					$ret['entries'][0]['url_encode'] = "holamundo";
