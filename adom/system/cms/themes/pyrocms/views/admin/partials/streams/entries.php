...
<?php if ($entries) { ?>
    ...
    <table class="table-list" cellpadding="0" cellspacing="0">
		...
		<?php
			$i = 0;
			foreach ($entries as $field => $data_item) {$i++;}
			foreach ($entries as $field => $data_item) { ?>
			<tr>
				<?php if ($stream->sorting == 'custom'): ?>
        <td width="30" class="handle"><?php echo Asset::img('icons/drag_handle.gif', 'Drag Handle'); ?></td><?php endif; ?>
          ...
          <?php
					...
					elseif($view_option == 'archivo') {
						?><a target="_blank" href="<?php echo site_url().'uploads/default/files/'. $data_item->archivo; ?>"><?php echo $data_item->archivo; ?></a><?php
					}
					else {
					...
					<?php
						...
						{						
							...
							foreach($buttons as $button)
							{
                                                            ...
                                                            if(!$delete_frist && lang('global:delete')==$button['label']){
                                                               if($i != 1){
