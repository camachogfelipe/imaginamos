...
class Events_Rss {
    ...
    public function __construct()
    {
		...
		// To execute the "run" method below you would use: Events::trigger('rss_event');
		// in any php file within PyroCMS, even another module.
		Events::register('rss_event', array($this, 'run'));
    }
    
    public function run()
    {
        $this->ci->load->model('rss/rss_m');
        
        // we're fetching this data on each front-end load. You'd probably want to do something with it IRL
        $this->ci->rss_m->limit(5)->get_all();
    }
