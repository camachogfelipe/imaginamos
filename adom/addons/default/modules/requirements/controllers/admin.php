...
class Admin extends Admin_Controller {
    ...
    protected $section = 'requirements';
    private $_tabs = array();
    ...
    public function index($page = 1) {
        ...
        $extra['columns'] = array("country_id_{$this->slug_stream}", "city_id_{$this->slug_stream}", "building_id_{$this->slug_stream}", "office_id_{$this->slug_stream}", "description_{$this->slug_stream}", "state_{$this->slug_stream}", "created_by", "created");

        $model = $this->load->model('entities/countries_m');

        $extra['filters'] = array(
            array(
                'field' => "country_id_{$this->namespace}",
                'label' => 'País',
        ...
        $this->streams->cp->entries_table($this->slug_stream, $this->namespace, 30, "admin/{$this->namespace}/index", true, $extra);
    }
