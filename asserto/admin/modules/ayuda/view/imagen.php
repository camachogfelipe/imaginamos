...
if(isset($_GET["idimagen"])){
  $db->doQuery("SELECT * FROM imagen_ayuda WHERE idimagen=".(int)$_GET["idimagen"], SELECT_QUERY); 
...
//  }
    ...
    $retorno = ClassFIle::UploadImagenFile("img", "../imagenes", "original_" . $idinsertado, "redimension_" . $idinsertado, 470, 300);
    if ($retorno["Status"]=="Uploader"){
    $db->doQuery("UPDATE imagen_ayuda SET imagen='".$retorno["NameFile"]."' WHERE idimagen=".$idinsertado, UPDATE_QUERY); 
...
//                        if ($id>0){ 
                        ?>
                            ...
                            <br/><span style="color: black;">Tama&ntilde;o de la Imagen: <i>470 x 300</i></span>
                            <?php 
//                        }else{
                         ?>
<!--                         <label class="cabinet"> -->
