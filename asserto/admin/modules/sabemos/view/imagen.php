...
// Traemos datos edit
if(isset($_GET["idsabemos"])){
  $db->doQuery("SELECT * FROM sabemos_imagen WHERE idsabemos=".(int)$_GET["idsabemos"], SELECT_QUERY); 
  $id = $db->results[0]["idsabemos"];
...
//    $update=$db->doQuery("UPDATE home SET  texto='".$nombre_c."' WHERE idhome=".(int)$_POST["idhome"], UPDATE_QUERY);
    $idinsertado=$_REQUEST["idsabemos"];
//    if($update){
...
//  }
    ...
    $db->doQuery("UPDATE sabemos_imagen SET imagen='".$retorno["NameFile"]."' WHERE idsabemos=".$idinsertado, UPDATE_QUERY); 
  ...
  <div class="content">
    <div class="formEl_b">
      <fieldset>
        ...
        <form class="forminterno" enctype="multipart/form-data" id="forminterno" name="forminterno" action="index.php?seccion=imagen&idsabemos=1" method="post" style="width: 900px;">
