...
$db->doQuery("SELECT * FROM archivos WHERE id_usuario='".$_REQUEST['id']."' ORDER BY id DESC", SELECT_QUERY);
$dataAll = $db->results;
?>
<script>
  function confirmar(){
...
<div class="widget">
  <div class="content">
    <div class="formEl_b">
      ...
      <a class="uibutton" href="index.php?seccion=addarchivos&id=<?= $id; ?>">Adjuntar archivo</a>
      ...
      <div class="tableName">
        <table class="display data_table2">
          ...
          <tbody>
            <?php $i = 1; foreach($dataAll as $archivo) : ?>
            <tr class="odd gradeX">
              <td class="center" width="70px"><?= $archivo['id']; ?></td>
              <td class="center" width="70px"><a target="_blank" href="<?= $result[0][config_web]."files/".$archivo["nombre"]?>"><?= $archivo["nombre"]?></a></td>
              <td class="center" width="70px"><?= $archivo["tipo"]?></td>
              <td class="center" width="70px"><a class="uibutton special delete" href="index.php?seccion=archivos&idfile=<?= $archivo["id"]?>&id=<?= $id ?>">Eliminar</a></td>
            </tr>
