...
$(document).ready(function(){
	...
	var over4 = 0;
	$(".con-comments-ov").hover(function(){
		if(over4==0){
			$(this).children(".form-bg").animate({opacity:1}, 250);
			over4=1
		}else{
	...
	if($("#form-contact").size()>0){$("#form-contact").validationEngine({scroll:false});};
	...
	var logBt = 0; $(".login-bt").click(function(){if (logBt == 0){$(this).hide(); $(this).siblings(".login-form").fadeIn(); logBt = 1} else {logBt = 0}});
	if($(".modals-act").size()>0){$(".modals-act").fancybox();};
	if($("#modal-rec-ok").size()>0){$("#modal-rec-ok").fancybox().trigger('click');};
	if($(".login-form").size()>0){$(".login-form").validationEngine({scroll:false});};
	if($(".grl-form").size()>0){$(".grl-form").validationEngine({scroll:false});};
	if($(".footer-ahorranito").size()>0){$(".footer-ahorranito").ahorranito({theme:'claro',type:1});};
});