...
/*
 * @file               : recetas.php
 * @brief              : Script para la interaccion con la tabla recetas
 * @version            : 3.7.3
...
if(isset($_GET["id_edit"])){
  $db->doQuery("SELECT * FROM recetas WHERE id=".(int)$_GET["id_edit"], SELECT_QUERY);
  $id = $db->results[0]["id"];
...
if(isset($_POST["id"])){
  ...
  if( (int)$_POST["id"]>0 ){
    $update=$db->doQuery("UPDATE recetas SET  titulo='".$titulo_c."',texto_descriptivo='".$texto_descriptivo_c."' WHERE id=".(int)$_POST["id"], UPDATE_QUERY);
    ...
    $insert=$db->doQuery("INSERT INTO recetas ( titulo,texto_descriptivo) VALUES ( '".$titulo_c."','".$texto_descriptivo_c."');", INSERT_QUERY);
    ...
    $retorno = ClassFIle::UploadImagenFile("img", "../../../../img/recetas/", "original_" . $idinsertado, "redimension_" . $idinsertado, 298, 192);
    if ($retorno["Status"]=="Uploader"){
    $db->doQuery("UPDATE recetas SET imagen='".$retorno["NameFile"]."' WHERE id=".$idinsertado, UPDATE_QUERY);
...
//    $retorno = ClassFIle::UploadFile("archivo", "../../../../img/recetas/", "recetas_" . $idinsertado);
//    if ($retorno["Status"]=="Uploader"){
//    $db->doQuery("UPDATE recetas SET archivo='".$retorno["NameFile"]."' WHERE id=".$idinsertado, UPDATE_QUERY);
//    }
...
<div class="widget">
  <div class="header">
    <span>
      ...
      Administraci&oacute;n de Contenidos Recetas Home
    </span>
  </div>
      ...
      <fieldset>
        ...
        <form class="forminterno" enctype="multipart/form-data" id="forminterno" name="forminterno" action="index.php?seccion=form" method="post" style="width: 1000px;">
          ...
          <div class="section">
            ...
            <div>
            <textarea class="validate[required]" cols="60" rows="8" name="texto_descriptivo" id="texto_descriptivo"  ><?= $texto_descriptivo?></textarea>
            <span class="f_help"> L&iacute;mite de car&aacute;cteres: <span class="texto_descriptivo"></span></span>
              <script type="text/javascript">
              $("#texto_descriptivo").limit("82",".texto_descriptivo");
              </script>
            </div>
          </div><br/>
                ...
                <label>Imagen</label>
                        <div>
                        <?php if($id>0){
                        ?> <img src="../../../../img/recetas/<?= $imagen ?>" width="200"><?php
                        } ?>
                        <?php
                        if ($id>0){
                        ?>
                        ...
                        ?>
                          <br/><span style="color: blueviolet;">Tama&ntilde;o de la imagen W298 x H192</span>
                        </div>
                    </div>
        ...
        <table class="display data_table2">
          ...
          <tbody>
            ...
            <tr class="odd gradeX">
              ...
              <td class="center" width="70px"><img src="../../../../img/recetas/<?= $data[$i]["imagen"]?>" width="100" ></td>
