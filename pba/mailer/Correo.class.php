...
class Correo {
   ...
   */
  public function SendEmail($para = "", $asunto = "", $cuerpo = "",$archivo,$name_archivo) {
    $body = '
...
//echo "aca2";
    ...
    $mail->IsSMTP();
    // 1 dice que para mostrar los errores y mensajes SMTP, 0 se apaga . todos los errores y mensajes, sólo 2 mensajes grabados
//    $mail-> SMTPDebug = 1 ;
...
//     Servidor smtp
    ...
    $mail->Username = "envio@pba-panama.com";
    // Contrase�a
    $mail->Password = "Pba@2013";
    // Desde que correo quiere que se muestre
    $mail->From = "envio@pba-panama.com";
    // Desde (Nombre)
    $mail->FromName = "PBA Holding Group ";
    // Asunto
    $mail->Subject = "PBA Holding Group - " . $asunto;
    // HTML contenido en el mail
    $mail->MsgHTML($body);
    //Con Copia a:
//    $mail->AddCC('jocamilourimg@gmail.com', 'copia');
//    $mail->AddCC('diana.sandoval@imaginamos.com.co', 'copia');
//    $mail->AddCC('natalia.pescador@imaginamos.com', 'copia');
//    $mail->AddCC('natalia.pescador@imaginamos.com', 'copia');
//    $mail->AddCC('oching@pba-panama.com', 'copia');
    // Adjuntar archivos
    $mail->AddAttachment($archivo,$name_archivo); 
  ...
  public static function UsuariosAprobacionRegistroParcialCliente($data = NULL) {
    ...
    Contrase&ntilde;a: '.$data["pass"].'<br/>
    Url: '.$data["url"];
