...
function printFormBanners($funcionality)
		{	
			...
			{
			case 1:
				$query = "SELECT * FROM cms_testimonials ";
		...
		{	
			...
			{
			case 1:
				$query = "SELECT * FROM cms_aboutus_titulo ORDER BY id_titulo DESC";
				$this->db->doQuery($query,SELECT_QUERY);
				$results = $this->db->results;
				...
				$html .= '
					...
					<a  href="editTitulo.php?id='.$result[id_titulo].'&funcionality=2" class="uibutton">Editar</a>
					</td>
					</tr>';		
				}
		...
		{	
			...
			{
			case 1:
				$query = "SELECT t.titulo,i.id_item ,i.item FROM cms_aboutus_titulo as t inner join cms_aboutus_item as i on (t.id_titulo=i.id_titulo) ORDER BY t.id_titulo DESC";
				$this->db->doQuery($query,SELECT_QUERY);
				$results = $this->db->results;
                                
                                $query2 = "SELECT * FROM cms_aboutus_item ";
				$this->db->doQuery($query2,SELECT_QUERY);
				$results2 = $this->db->results;
                                
                                $query3 = "SELECT * FROM cms_aboutus_titulo order by id_titulo ";
				...
				';			
						...
						{
							$html .= "<option value='".$result3['id_titulo']."'>".trim($result3['titulo'])."</option>";
						}
				$html .= '		
						</select>
					</div>
				...
				<div>	
					<input type="text" name="item" id="item" maxlength="61" class="large" />
				</div>
				</div>
				...
				$html .= '
					...
					<td class="center" width="100px">
					<a id="'.$result[id_item].'" class="Delete uibutton special" tableToDelete="cms_aboutus_item" nameField="id_item">Eliminar</a>
					<a  href="edit.php?id='.$result[id_item].'&funcionality=3" class="uibutton">Editar</a>
					</td>
					</tr>';		
				}
...
function printFormEdit($id,$funcionality)
		...
		$html = '
				...
				<div class="imu_info" id="info"></div>
                    ...
                    <legend><h1>Testimonials Edicion de Imagen</h1></legend>
		...
		function tit_edit($id,$funcionality){
			...
			$query = "SELECT * FROM cms_aboutus_titulo WHERE id_titulo = '$id'";
		        $this->db->doQuery($query,SELECT_QUERY);
		        $results = $this->db->results;
		...
		return $html;
                ...
                function edit_contenido_testimonials(){
                
                      $query2 = "SELECT * FROM cms_testimonials";
				$this->db->doQuery($query2,SELECT_QUERY);
				$results = $this->db->results;
