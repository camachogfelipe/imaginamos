...
{  
	$objResponse = new xajaxResponse();
        ...
        htmlentities($formData[titulo]);
        str_replace("'","&#39",$formData[titulo]);
	...
	if($formData[titulo] == "")
		...
		$db->doQuery("UPDATE cms_aboutus_titulo SET titulo = '$formData[titulo]' WHERE id_titulo = '$formData[id]'",UPDATE_QUERY);
...
{  
	$objResponse = new xajaxResponse();
        ...
        htmlentities($formData[item]);
        str_replace("'","&#39",$formData[item]);
	...
	if($formData[item] == "")
		...
		$db->doQuery("UPDATE cms_aboutus_item SET item = '$formData[item]' WHERE id_item = '$formData[id]'",UPDATE_QUERY);
...
{  
	...
	if($formData[titulo] == "" )
		...
		$db->doQuery("INSERT INTO cms_aboutus_titulo (id_titulo,titulo) VALUES ('','$formData[titulo]')",INSERT_QUERY);
...
{  
	...
	if($formData[item] == "" )
		...
		$db->doQuery("INSERT INTO cms_aboutus_item (id_item,id_titulo,item) VALUES ('','$formData[titulo]','$formData[item]')",INSERT_QUERY);
...
//		include("../../../core/class/db.class.php");
		...
		$db->doQuery("UPDATE cms_foundation SET texto = '$texto'",UPDATE_QUERY);
