...
{  
	...
	if($formData[titulo] == "" or $formData[texto]=="" )
	
		$objResponse->script("showError('Ingrese un titulo y texto',3000);");
		...
		$db->doQuery("INSERT INTO cms_aboutpan_itembloque (id_item,id_bloque,titulo,texto) VALUES ('','$formData[nombreBloque]','$titulo','$texto')",INSERT_QUERY);
                
                
		$objResponse->script("setInterval(\"window.location.reload();\", 3000 );");
		$objResponse->script("loading('Agregando..',1);");
		$objResponse->script("setTimeout(\"unloading()\", 2000 );");
                $objResponse->script("showSuccess('FAQ agregado correctamente',3000);");
...
function edit_FAQ( $formData )
{  
	$objResponse = new xajaxResponse();
	
//        $nombreBloque = mysql_real_escape_string($formData[nombreBloque]);
//        htmlentities($nombreBloque);
        $titulo = str_replace("'","&#39",$formData[titulo]);
        htmlentities($titulo);
        $texto = str_replace("'","&#39",$formData[texto]);
        htmlentities($texto);
	...
	if($formData[titulo] == "")
		...
		$db->doQuery("UPDATE cms_aboutpan_itembloque SET titulo = '$titulo',texto = '$texto' WHERE id_item = '$formData[id]'",UPDATE_QUERY);
		
		$objResponse->script("loading('Procesando',1);");
		$objResponse->script("setTimeout(\"unloading()\", 2000 );");
		$objResponse->script("showSuccess('Actualización FAQ correcta',3000);");
	}
...
//		include("../../../core/class/db.class.php");
		...
		$db->doQuery("UPDATE cms_aboutpan_cont SET texto = '$texto'",UPDATE_QUERY);
...
$xajax->registerFunction("edit_FAQ");
$xajax->registerFunction("upload");
$xajax->registerFunction("agregar_FAQ");
