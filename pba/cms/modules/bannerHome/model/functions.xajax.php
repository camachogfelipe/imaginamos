...
/********************************************************************************************/
function edit_contenido_banner( $formData )
{  
	$objResponse = new xajaxResponse();
        
        $titulo = str_replace("'","&#39",$formData[titulo]);
        htmlentities($titulo);
        $subtitulo = str_replace("'","&#39",$formData[subtitulo]);
        htmlentities($subtitulo);
        $url = $formData[url];
        htmlentities($url); 
		...
		$db->doQuery("UPDATE cms_banners SET banners_id = '$formData[id]', banners_txt1 = '$titulo',banners_txt2 = '$subtitulo',banners_url = '$url'",UPDATE_QUERY);
...
$xajax->registerFunction("edit_contenido_banner");
