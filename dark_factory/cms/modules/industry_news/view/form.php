<?php
/*
 * @file               : industry_news.php
 * @brief              : Script para la interaccion con la tabla industry_news
...
if(isset($_GET["id_edit"])){
  $db->doQuery("SELECT * FROM industry_news WHERE id=".(int)$_GET["id_edit"], SELECT_QUERY);
  $id = $db->results[0]["id"];
...
if(isset($_POST["id"])){
  ...
  if( (int)$_POST["id"]>0 ){
    $update=$db->doQuery("UPDATE industry_news SET  titulo='".$titulo_c."',texto='".$texto_c."',texto_largo='".$texto_largo_c."' WHERE id=".(int)$_POST["id"], UPDATE_QUERY);
    ...
    $insert=$db->doQuery("INSERT INTO industry_news ( titulo,texto,texto_largo) VALUES ( '".$titulo_c."','".$texto_c."','".$texto_largo_c."');", INSERT_QUERY);
    ...
    $retorno = ClassFIle::UploadImagenFile("img", "../../../../img/industry_news/", "original_" . $idinsertado, "redimension_" . $idinsertado, 355, 195);
    if ($retorno["Status"]=="Uploader"){
    $db->doQuery("UPDATE industry_news SET imagen='".$retorno["NameFile"]."' WHERE id=".$idinsertado, UPDATE_QUERY);
    ...
    $retorno = ClassFIle::UploadFile("archivo", "../../../../img/industry_news/", "industry_news_" . $idinsertado);
    if ($retorno["Status"]=="Uploader"){
    $db->doQuery("UPDATE industry_news SET archivo='".$retorno["NameFile"]."' WHERE id=".$idinsertado, UPDATE_QUERY);
    }
...
if( isset($_GET["id_del"]) && isset($_GET["confirm"]) ){
  if($_GET["confirm"] == base64_encode(md5($_GET["id_del"]))){
    $db->doQuery("DELETE FROM industry_news WHERE id=".(int)$_GET["id_del"], DELETE_QUERY);
    echo "<script type='text/javascript'>setTimeout(function(){showSuccess('Eliminado',3000) }, 1500);</script>";
  }
}

// Obtenemos las datos
$db->doQuery("SELECT * FROM industry_news ORDER BY id ASC", SELECT_QUERY);
$dataAll = $db->results;
?>
...
<div class="widget">
  <div class="header">
    <span>
      ...
      Administraci&oacute;n de Contenidos Industry news
    </span>
  </div>
  ...
  <div class="content">
    <div class="formEl_b">
      <fieldset>
        <?php if($id>0){
          echo "<h3>Editando Industry news '".$id."'</h3>";
        }else{
          echo "<h3>Industry news</h3>";
        } ?>
...
//                $("#texto").limit("140",".texto");
                ...
                <label>Imagen</label>
                        <div>
                        <?php if($id>0){
                        ?> <img src="../../../../img/industry_news/<?= str_replace("original","redimension",$imagen); ?>?<?php echo substr(md5(uniqid(rand())), 0, 6); ?>" width="200"><?php
                        } ?>
                        <?php
                        if ($id>0){
                        ?>
                        ...
                        ?>
                          <br/><span style="color: blueviolet;">Tama&ntilde;o de la Imagen: <i>355 x 195</i></span>
                        </div>
                    </div>
        ...
        <table class="display data_table2">
          ...
          <tbody>
            ...
            <tr class="odd gradeX">
              ...
              <td class="center" width="70px"><img src="../../../../img/industry_news/<?= str_replace("original","redimension",$data[$i]["imagen"])?>?<?php echo substr(md5(uniqid(rand())), 0, 6); ?>" width="100" ></td>
