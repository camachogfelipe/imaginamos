 ...
 * @brief              : Script para la interaccion con la tabla latest_news
...
if(isset($_GET["id_edit"])){
  $db->doQuery("SELECT * FROM latest_news WHERE id=".(int)$_GET["id_edit"], SELECT_QUERY);
  $id = $db->results[0]["id"];
...
if(isset($_POST["id"])){
  ...
  if( (int)$_POST["id"]>0 ){
    $update=$db->doQuery("UPDATE latest_news SET  titulo='".$titulo_c."',texto='".$texto_c."',link='".$link_c."' WHERE id=".(int)$_POST["id"], UPDATE_QUERY);
    ...
    $insert=$db->doQuery("INSERT INTO latest_news ( titulo,texto,link) VALUES ( '".$titulo_c."','".$texto_c."','".$link_c."');", INSERT_QUERY);
    ...
    $retorno = ClassFIle::UploadImagenFile("img", "../../../../img/latest_news/", "original_" . $idinsertado, "redimension_" . $idinsertado, 236, 123);
    if ($retorno["Status"]=="Uploader"){
    $db->doQuery("UPDATE latest_news SET imagen='".$retorno["NameFile"]."' WHERE id=".$idinsertado, UPDATE_QUERY);
    ...
    $retorno = ClassFIle::UploadFile("archivo", "../../../../img/latest_news/", "latest_news_" . $idinsertado);
    if ($retorno["Status"]=="Uploader"){
    $db->doQuery("UPDATE latest_news SET archivo='".$retorno["NameFile"]."' WHERE id=".$idinsertado, UPDATE_QUERY);
    }
...
if( isset($_GET["id_del"]) && isset($_GET["confirm"]) ){
  if($_GET["confirm"] == base64_encode(md5($_GET["id_del"]))){
    $db->doQuery("DELETE FROM latest_news WHERE id=".(int)$_GET["id_del"], DELETE_QUERY);
    echo "<script type='text/javascript'>setTimeout(function(){showSuccess('Eliminado',3000) }, 1500);</script>";
  }
}

// Obtenemos las datos
$db->doQuery("SELECT * FROM latest_news ORDER BY id ASC", SELECT_QUERY);
$dataAll = $db->results;
?>
...
<div class="widget">
  <div class="header">
    <span>
      ...
      Administraci&oacute;n de Contenidos Latest news
    </span>
  </div>
  ...
  <div class="content">
    <div class="formEl_b">
      <fieldset>
        <?php if($id>0){
          echo "<h3>Editando Latest news '".$id."'</h3>";
        }else{
            ...
            <div>
              <input style="border-radius: 5px;" class="validate[required,custom[url]] large" type="text" name="link" id="link" value="<?= $link?>" />
                ...
                <label>Imagen</label>
                        <div>
                        <?php if($id>0){
                        ?> <img src="../../../../img/latest_news/<?= str_replace("original","redimension",$imagen); ?>?<?php echo substr(md5(uniqid(rand())), 0, 6); ?>" width="200"><?php
                        } ?>
                        <?php
                        if ($id>0){
                        ?>
        ...
        <table class="display data_table2">
          ...
          <tbody>
            ...
            <tr class="odd gradeX">
              ...
              <td class="center" width="70px"><img src="../../../../img/latest_news/<?= str_replace("original","redimension",$data[$i]["imagen"])?>?<?php echo substr(md5(uniqid(rand())), 0, 6); ?>" width="100" ></td>
