...
/*
 * @file               : affiliates.php
 * @brief              : Script para la interaccion con la tabla affiliates
...
if(isset($_GET["id_edit"])){
  $db->doQuery("SELECT * FROM affiliates WHERE id=".(int)$_GET["id_edit"], SELECT_QUERY);
  $id = $db->results[0]["id"];
...
if(isset($_POST["id"])){
  ...
  if( (int)$_POST["id"]>0 ){
    $update=$db->doQuery("UPDATE affiliates SET  titulo='".$titulo_c."',link='".$link_c."' WHERE id=".(int)$_POST["id"], UPDATE_QUERY);
    ...
    $insert=$db->doQuery("INSERT INTO affiliates ( titulo,link) VALUES ( '".$titulo_c."','".$link_c."');", INSERT_QUERY);
    ...
    $retorno = ClassFIle::UploadImagenFile("img", "../../../../img/affiliates/", "original_" . $idinsertado, "redimension_" . $idinsertado, 165, 141);
    if ($retorno["Status"]=="Uploader"){
    $db->doQuery("UPDATE affiliates SET imagen='".$retorno["NameFile"]."' WHERE id=".$idinsertado, UPDATE_QUERY);
    ...
    $retorno = ClassFIle::UploadFile("archivo", "../../../../img/affiliates/", "affiliates_" . $idinsertado);
    if ($retorno["Status"]=="Uploader"){
    $db->doQuery("UPDATE affiliates SET archivo='".$retorno["NameFile"]."' WHERE id=".$idinsertado, UPDATE_QUERY);
    }
...
if( isset($_GET["id_del"]) && isset($_GET["confirm"]) ){
  if($_GET["confirm"] == base64_encode(md5($_GET["id_del"]))){
    $db->doQuery("DELETE FROM affiliates WHERE id=".(int)$_GET["id_del"], DELETE_QUERY);
    echo "<script type='text/javascript'>setTimeout(function(){showSuccess('Eliminado',3000) }, 1500);</script>";
  }
}

// Obtenemos las datos
$db->doQuery("SELECT * FROM affiliates ORDER BY id ASC", SELECT_QUERY);
$dataAll = $db->results;
?>
...
<div class="widget">
  <div class="header">
    <span>
      ...
      Administraci&oacute;n de Contenidos Affiliates
    </span>
  </div>
        ...
        <form class="forminterno" enctype="multipart/form-data" id="forminterno" name="forminterno" action="index.php?seccion=form" method="post" style="width: 1000px;">
          <input type="hidden" name="id" id="id" value="<?= $id?>" />
                ...
                <label>Imagen</label>
                        <div>
                        <?php if($id>0){
                        ?> <img src="../../../../img/affiliates/<?= str_replace("original","redimension",$imagen); ?>?<?php echo substr(md5(uniqid(rand())), 0, 6); ?>" width="200"><?php
                        } ?>
                        <?php
                        if ($id>0){
                        ?>
                        ...
                        ?>
                          <br/><span style="color: blueviolet;">Tama&ntilde;o de la Imagen: <i>165 x 141</i></span>
                        </div>
                    </div>
        ...
        <table class="display data_table2">
          ...
          <tbody>
            ...
            <tr class="odd gradeX">
              ...
              <td class="center" width="70px"><img src="../../../../img/affiliates/<?= str_replace("original","redimension",$data[$i]["imagen"])?>?<?php echo substr(md5(uniqid(rand())), 0, 6); ?>" width="100" ></td>
