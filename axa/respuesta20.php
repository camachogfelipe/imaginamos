...
<meta name="description" content=" Encuentra los puntos Wifi ETB m�s cercanos y navega desde cualquier sitio. ">
...
//					
//				
//				$elCSV = utf8_decode($nombres).",".$identificacion.",".$email.",".$nacimiento.",".$edad.",".$nombreplan.",".$nombrepais.",".$fecha_salida.",".$fecha_regreso.",".$diasviaje.",".$canal.",".$fechaventa.",".$valor.",".$observaciones.",".$terminos.",".$ref_final." \n ";
...
//				$pdf->Image('http://tarjetasdeasistencia-axa.com/images/pdf/'.$imgcobertura,75,60,50,40);
				...
				$pdf->MultiCell(0,5,'Que '.utf8_decode($nombres).'  con identificaci�n No. '.$identificacion.' cuenta con el servicio de asistencia m�dica en el exterior por adquirir la tarjeta de asistencia respaldada por Inter Partner Assistance, con el plan '.($nombreplan).' radicado bajo el n�mero '.$ref_final.' y con las siguientes condiciones:',0,'J');
				$pdf->Cell(0,5,'',0,1);
				$pdf->Cell(0,5,'',0,1);
				$pdf->Cell(0,5,'',0,1);
				$pdf->Cell(0,5,'',0,1);
				...
				$pdf->Cell(0,5,'La asistencia se prestar� durante los primeros d�as de viaje a partir de la fecha de salida del pa�s y con destino a: ',0,1);
				...
				$txt .= 'disposici�n en la p�gina web www.tarjetasdeasistencia-axa.com, las cuales usted declara haber le�do, atendido y aceptado. Las ';
				...
				$pdf->Cell(0,5,'4. El valor de la cobertura corresponde al monto equivalente de la moneda legal vigente de cada pa�s donde',0,1);
				$pdf->Cell(0,5,'se preste el servicio de asistencia.',0,1);
				$pdf->Cell(0,5,'Este certificado se expide en Colombia el d�a 16 de Junio de 2012 a solicitud del interesado y de acuerdo con',0,1);
				$pdf->Cell(0,5,'el decreto 2150 de 5 de diciembre de 1995 no requiere autenticaci�n para su validez.',0,1);
...
//		$mail->FromName = "AXA";
//		$mail->Subject = '45086';
//		$mail->AddAddress('dayron.garzon@imaginamos.com.co');
////                $mail->AddBCC('marketing@axa-assistance.com.co');
////                $mail->AddBCC('dayron.garzon@imaginamos.com.co');
//		$mail->Body = $body;
//		$mail->IsHTML(true);
//		$mail->Send();
		# FIN envio .csv a AXA
		...
		$mail->AddAddress('eduardo.ruiz@imaginamos.com.co');
