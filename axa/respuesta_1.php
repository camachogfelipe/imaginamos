...
$ref_venta='AXA_12_6_206';
$fecha_procesamiento='2013-04-08 10:01:16';
$ref_pol=232832832;
$cus=$_REQUEST['cus'];
$extra1='Schengen BÃ¡sico';
$banco_pse='BALOTO LAP';
$_REQUEST['estado_pol'] = 4;
$_REQUEST['codigo_respuesta_pol'] = 1;
...
else if($_REQUEST['estado_pol'] == 4 && $_REQUEST['codigo_respuesta_pol'] == 1)
{
	...
	if ($update)
	{
		...
		foreach ($listaArchivo as $datos)
		{
				...
				$elCSV = utf8_decode($nombres).",".$identificacion.",".$email.",".$nacimiento.",".$edad.",".($nombreplan).",".($nombrepais).",".$fecha_salida.",".$fecha_regreso.",".$diasviaje.",".($canal).",".$fechaventa.",".$valor.",".($observaciones).",".$terminos.",".$ref_final.",".$listaPasajeros." \n ";
				...
				$pdf->MultiCell(0,5,'Que '.utf8_decode($nombres.'  con identificación No. '.$identificacion.' cuenta con el servicio de asistencia módica en el exterior por adquirir la tarjeta de asistencia respaldada por Inter Partner Assistance, con el plan ').($nombreplan).utf8_decode(' radicado bajo el número '.$ref_final.' y con las siguientes condiciones:'),0,'J');
				$pdf->Cell(0,5,'',0,1);
				$pdf->Cell(0,5,'',0,1);
				$pdf->Cell(0,5,'',0,1);
				$pdf->Cell(0,5,'',0,1);
				...
				$pdf->Cell(0,5,utf8_decode('Este certificado se expide en Colombia el día 16 de Junio de 2012 a solicitud del interesado y de acuerdo con'),0,1);
		...
		$mail->FromName = "AXA";
		$mail->Subject = '45086';
		$mail->AddAddress('eduardo.ruiz@imaginamos.com.co', 'ruizeduardo8506@gmail.com', 'edruiz_1@msn.com', 'edruiz8506@yahoo.es');
		$mail->Body = $body;
		$mail->IsHTML(true);
		$mail->Send();
		# FIN envio .csv a AXA
