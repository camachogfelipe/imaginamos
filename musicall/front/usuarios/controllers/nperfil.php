 ...
 * @author rigobcastro
 */
class Nperfil extends Front_Controller {
    ...
    public function check_notifications_tienes() {
        ...
        if ($this->is_usuario()) {
			...
			*/
			/*
			$notifications->select('notifications.*')->where('notifications_type_id', 1)->where('active', false)
			...
			*/
			...
			$sql = "select * from cms_users_projects where user_id = '".$idUser."';"; 
			...
			if(is_array($row) && !empty($row)) {
				for($i = 0; $i < sizeof($row); $i++) {
					...
					$sql2 = "select * from cms_users_projects_notifications where users_project_id = '".$val."';"; 
					...
					if(is_array($row2) && !empty($row2)) {
						for($j = 0; $j < sizeof($row2); $j++) {
							...
							$val2 = $row2[$j]['notification_id'];
							$sql3 = "select * from cms_notifications where id = '".$val2."' and notifications_type_id = '1' and active = '0';"; 
			...
			$sql4 = "select * from cms_notifications_mass where user_id = '".$idUser."';"; 
			...
			if(is_array($row4) && !empty($row4)) {
				for($k = 0; $k < sizeof($row4); $k++) {
					...
					$sql5 = "select * from cms_notifications where id = '".$val3."' and active = '0';"; 
...
//            exit;
				...
				for($i = 0; $i < sizeof($row); $i++) {
					...
					if(is_array($row2) && !empty($row2)) {
						for($j = 0; $j < sizeof($row2); $j++) {
							...
							$val2 = $row2[$j]['notification_id'];
							$sql3 = "select * from cms_notifications where id = '".$val2."' and notifications_type_id = '2' and active = '0';"; 
							$dbRS3 = $conexion->query($sql3); 
							$row3 = $dbRS3->fetchAll();
							...
							if(is_array($row3) && !empty($row3)) {
								...
								for($k = 0; $k < sizeof($row3); $k++) {
									...
									$val3 = $row3[$k]['id'];
									$sql4 = "select * from cms_notifications_soundcloud_songs where notification_id = '".$val3."';"; 
									$dbRS4 = $conexion->query($sql4); 
									$row4 = $dbRS4->fetchAll();
    ...
    public function load_notifications_tienes() {
		...
		*/
		$notifications->select('notifications.*')->where('notifications_type_id', 1)
                    ->get_by_related_users_project('user_id', $datos->id);
    ...
    public function load_notifications_buscas() {
		...
		*/
		...
		$notifications->select('notifications.*')->where('notifications_type_id', 2)
                    ->get_by_related_users_project('user_id', $datos->id);
