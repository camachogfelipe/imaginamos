...
class ClassFile {
  ...
  public static function UploadFile($name_input_file = "", $carpeta = "") {
    $path = $carpeta;
...
//        $nombre = $raiz_nuevo_nombre . $ext;
        move_uploaded_file($_FILES[$name_input_file]['tmp_name'], "$path/$nombre_archivo");
        @chmod("$path/$nombre_archivo", 0777);

        //Retorno array con los parametros basicos necesaros
        return array("Status" => "Uploader", "Mensaje" => "Se subio el Archivo " . $nombre_archivo, "Ext" => $ext, "NameOriginal" => $nombre_archivo, "NameFile" => $nombre_archivo, "SizeFile" => $_FILES[$name_input_file]['size'], "URL" => "$path/$nombre_archivo");
      } else {
