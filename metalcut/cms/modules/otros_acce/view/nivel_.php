...
if (empty($id)){
    $sql="SELECT * FROM nivel_portafolio where  nivel='".$nivel."' and  idportafolio=" . $idportafolio ." ORDER BY idnivel_portafolio ASC";
   // echo $sql.'ca..';
    $db->doQuery($sql, 1);
...
if (!empty($id)){
   // echo "SELECT * FROM nivel_portafolio WHERE idnivel_portafolio=" . $id;
    $db->doQuery("SELECT * FROM nivel_portafolio WHERE nivel='".$nivel."' and  idnivel_portafolio=" .$idportafolio, 1);
                        ...
                        for ($i = 0; $i < $fil; $i++) {
                            ...
                            <tr class="odd gradeX">
                                ...
                                <td class="center titulo" width="100px">
                                    ...
                                    <?php } ?>
                                    <a class="uibutton icon edit" href="index.php?seccion=nivel&id=<?= $img["idnivel_portafolio"] ?>">Editar</a>
                                    <a class="uibutton icon edit" href="index.php?seccion=menu&idportafolio=<?= $img["idportafolio"] ?>&nivel=<?= $nivel ?>&idpadre=<?= $idportafoliopadre ?>">Opc. SubNivel</a>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
