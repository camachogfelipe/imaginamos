...
if (!empty($id)){
 //   echo "SELECT * FROM portafolio WHERE idportafolio=" . $id;
    $db->doQuery("SELECT * FROM portafolio WHERE modulo='".$modulo."' and nivel='".$nivel."'   and idportafolio=" . $id, 1);
   // echo "SELECT * FROM portafolio WHERE modulo='".$modulo."' and nivel='".$nivel."'   and idportafolio=" . $id;
...
<div class="content">
    <a class="uibutton icon normal answer" href="index.php?seccion=menu&idportafolio=<?=$idportafolio ?>&nivel=<?=$nivel ?>&idpadre=<?= $idportafoliopadre ?>">Atras</a>
    <div class="formEl_b">
        <?php if ($fil < 10) { ?>
            <fieldset>
                        ...
                        for ($i = 0; $i < $fil; $i++) {
                            ...
                            <tr class="odd gradeX">
                                ...
                                <td class="center idtitulo" width="100px">
                                    ...
                                    <?php //} ?>
                                    <a class="uibutton icon edit" href="index.php?seccion=menu&id=<?= $img["idportafolio"] ?>&nivel=<?php echo $nivel ?>&idpadre=<?= $_REQUEST['idpadre'] ?>">Editar</a>
                                    <?php if ($nivel<7){ ?>
                                        ...
                                        <?php }else{ 
                                             $nivel1=$img["nivel"]+1;
                                               ?>
                                                <a class="uibutton icon edit" href="index.php?seccion=nivel&idportafolio=<?= $img["idportafolio"] ?>&nivel=<?= $nivel1 ?>&idpadre=<?= $img["idportafolio"]?>">Crear SubNivel</a>
                                             <?php 
                                             } ?>
                                   <?php } ?>
                                </td>
