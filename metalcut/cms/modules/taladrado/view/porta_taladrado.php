...
$db->doQuery("SELECT idporta_taladrado FROM porta_taladrado where idtaladrado='".$idtaladrado."'  ORDER BY idporta_taladrado DESC Limit 1", 1);
$limit = $db->results[0];
$img = $_POST['idporta_taladrado'];
//echo $img.'-->ok';
$id=!empty($_REQUEST['id']) ? $_REQUEST['id'] : null;  
$idtaladrado=!empty($_REQUEST['idtaladrado']) ? $_REQUEST['idtaladrado'] : null; 
...
if (empty($_REQUEST['id']) && isset($_POST['tipo'])){
    if ( $_POST['ref'] == "" && $_POST['diam_corte'] == "" && $_POST['long_corte'] == "" && $_POST['long_total'] == "" && $_POST['diam_espigo'] == "" && $_POST['inserto'] == "" && $_POST['valor'] == "" ){
        $Erno = 2;
    }else{
    ...
    if($_POST['idporta_taladrado'] == "" && $_POST['ref'] == "" && $_POST['diam_corte'] == "" && $_POST['long_corte'] == "" && $_POST['long_total'] == "" && $_POST['diam_espigo'] == "" && $_POST['inserto'] == "" && $_POST['valor'] == "" ) {
            $Erno = 2;
     }else{
         $sql="UPDATE porta_taladrado SET 
             ...
             inserto='".$inserto."',
             rosca='".$rosca."',
             valor='".$valor."' WHERE idporta_taladrado='".$id."'";
         // echo $sql;
         $db->doQuery($sql, 4); 
    }
}
...
if (empty($id)){
    $db->doQuery("SELECT * FROM porta_taladrado where idtaladrado='".$idtaladrado."' ORDER BY idporta_taladrado ASC", 1);
    $fil = $db->rows;
}

if (!empty($id)){
 //   echo "SELECT * FROM idporta_taladrado WHERE idporta_taladrado=" . $id;
    $db->doQuery("SELECT * FROM porta_taladrado WHERE idtaladrado='".$idtaladrado."' ", 1);
...
<div class="content">
     ...
     <a class="uibutton icon normal answer" href="index.php?seccion=porta_taladrado&idtaladrado=<?php echo $_REQUEST["idtaladrado"] ?>">Atras</a>
    <?php }else{ ?>
            ...
            <fieldset>
                ...
                <form method="post" action="" name="forminterno2" id="forminterno2" enctype="multipart/form-data">
                   ...
                   <div class="section">                                                                                                  
                    <label>Valor</label>
                    <div>
                    <input type="text" name="valor" id="valor" class="medium" value="<?php echo $porta_taladrado["valor"] ?>">
        ...
        <div class="tableName toolbar">
                <table class="display data_table2" >
                    ...
                    <tbody>
                        <?php
                        $db->doQuery("SELECT * FROM porta_taladrado WHERE idtaladrado='".$idtaladrado."' order by idporta_taladrado asc", 1);
                      //  echo "SELECT * FROM porta_taladrado WHERE idtaladrado='".$idtaladrado."' order by idporta_taladrado asc";
                        for ($i = 0; $i < $fil; $i++) {
