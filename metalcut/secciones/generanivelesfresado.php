...
$db->connect();
    $datos=""; $carimg=''; $tipo='';
    $sql="select * from nivel_fresado where nivel='".$nivel."' and idportafolio_fresado='".$idpadre."' ";
    // echo $sql.'<br>';
    $db->doQuery($sql, 1);
    $datos1 = $db->results;
    
    $sql1="select * from portafolio_fresado where nivel='".$nivel."' and modulo='".$modulo."' and idportafoliopadre='".$idpadre."' ";
    //echo $sql1;
    $db->doQuery($sql1, 1);
    $datos = $db->results;
    $cant=count($datos);
            ...
            <div class="paso">
              <ul class="slider-tree">
                ...
                for($i = 0 ; $i < $cant ; $i++){
                    ...
                    $idp=$datos[$i]['idportafolio_fresado'];
                    $texto=$datos[$i]['texto'];
                    $paso=$datos[$i]['paso'];
                ...
                ?>    
                  <li>
                	<div class="paso-item">
                        ...
                        <?php if ($paso=='P'){ ?>
                        <a href="index.php?base&seccion=fresado-per&idportafolio_fresafo=<?php echo $idp ?>" class="vn-nav paso-item-p1" ><div class="paso-bt-vn"></div></a>
                        <?php }else{ ?>
                        <a href="<?php echo $hre ?>" class="<?php echo $cla ?>" data-id="<?php echo $dis ?>" onclick="displayNivel1('<?php echo $nivel1 ?>','<?php echo $modulo ?>','<?php echo $paso1 ?>','<?php echo $pas ?>','<?php echo $idp ?>')"><div class="paso-bt"></div></a>
                        <?php  } ?>
