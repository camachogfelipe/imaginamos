...
class Messaging
{
    public function GetMsgs($user)
    {
        ...
        try {
            $sth = DB::prep("SELECT TIME_FORMAT(m1.time, '%k:%i') as time,IF(m1.type = 'admin', m3.username, m2.nick) as user_nick, m1.user as user_id, m1.to_user as admin, m1.type, m1.msg
             FROM cms_messaging AS m1
             RIGHT JOIN cms_messaging_users AS m2 ON m1.user = m2.user_id
             RIGHT JOIN cms_messaging_admin m3 ON m1.to_user = m3.id
    ...
    {
        try {
            $sth = DB::prep("SELECT COUNT(id) as c FROM cms_messaging WHERE (user = :curr_user AND to_user = :this_admin AND type = 'user')  AND time > FROM_UNIXTIME(:time) ");
            $sth->bindParam(":curr_user", $user, PDO::PARAM_INT);
    ...
    {
        try {
            $sth = DB::prep("DELETE FROM cms_messaging WHERE time < (NOW() - INTERVAL :interval MINUTE)");
