...
/********************************************************************************************/
function enlace( $formData )
{  
	$objResponse = new xajaxResponse();
        
        $pieces = explode("&", $formData[opciones]);
                                $url=$pieces[0];
                                $sess=$pieces[1];
	
	if($formData[id] == "")
	
		$objResponse->script("showError('Ingrese',3000);"); 
...
//		include("../../../core/class/db.class.php");
		$db = new Database();
		$db->connect();
		//UPDATE cms_news SET url = '$enlace' WHERE news_id = '$id'"
		$db->doQuery("UPDATE cms_news SET url = '$url',url_name = '$sess' WHERE news_id = '$formData[id]'",UPDATE_QUERY);
