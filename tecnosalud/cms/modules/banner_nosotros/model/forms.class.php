...
';
       // echo count($results);
        if(count($results) < 3){
             echo '<p>&nbsp;</p>
				<a href="nuevo.php" class="uibutton submit_form">Nuevo Banner</a>';
        }else{
        ...
        if ($results == 0) {
            ...
            foreach ($results as $result) {
                $html .= '
					...
					<td class="center" width="100px"><img src="../files/middle/'.$result[bannernosotros_image].'"></td>
					<td class="center" width="20px">
					<a id="' . $result[bannernosotros_id] . '" class="Delete uibutton special" tableToDelete="cms_bannernosotros" nameField="bannernosotros_id">Eliminar</a>
					<a  href="edit.php?id=' . $result[bannernosotros_id] . '&funcionality=2" class="uibutton">Editar</a>
<input type="hidden"  class="amt" >					
</td>
					</tr>';
    ...
    function printFormEdit($id, $funcionality) {
        ...
        $query = "SELECT * FROM cms_bannernosotros WHERE bannernosotros_id = '$id'";
        $this->db->doQuery($query, SELECT_QUERY);
        $results = $this->db->results;
...
<fieldset>
				<div>
				<div>
				<label>Imagen Banner - (el tamaño debe ser de 976px x 350px) en formato JPG</label>
                                ';
                                 
                foreach ($results as $result) {
                    $html .= '
					
				<p>&nbsp;</p><img src="../files/middle/'.$result[bannernosotros_image].'"><p>&nbsp;</p><p>&nbsp;</p>	
';
                }
                                $html.='
                                
				<input class="IMG" type="file" path="files/" multi="false" startOn="onSubmit:form" ajax="true" ajaxInfoId="info" ajaxLoaderId="loader" button="../images/buttonSingle.jpg" thumbnails="90x90,100x,976x350" thumbnailsFolders="files/small/,files/middle/,files/big/" fileExt="jpg,jpeg" fileDesc="Images (*.jpg, *.jpeg)" allowRemove="true" />
