...
if($title == "" or $text == "")
	...
	else
	{
		...
		$db->doQuery("UPDATE cms_categoria SET categoria_title = '".mysql_real_escape_string($title)."', categoria_texto = '".mysql_real_escape_string($text)."' WHERE categoria_id = ".$id,UPDATE_QUERY);
                //var_dump($_POST);exit;
                    
                }else{
		$nameImageUpload = $_POST["IMGFiles"][0];
                 $db->doQuery("UPDATE cms_categoria SET categoria_title = '".mysql_real_escape_string($title)."', categoria_texto = '".mysql_real_escape_string($text)."', categoria_image = '".$nameImageUpload."' WHERE categoria_id = ".$id,UPDATE_QUERY);
