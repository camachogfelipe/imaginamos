jQuery(document).ready(function($){
	function roundNumberP(num, dec)
	{
		var result = Math.round( Math.round( num * Math.pow( 10, dec + 1 ) ) / Math.pow( 10, 1 ) ) / Math.pow(10,dec);
		return result;
	}
	function updateUploadifySettingsP(uploader)
	{
	...
	function preventSubmitP(event)
	{
		event.preventDefault();
	}
	function submitFormP(event)
	{
		event.preventDefault();
		var self = event.data.uploader;
		updateUploadifySettingsP(self);
		var queue;
		...
		if(queue.children().length>0)
		{
			...
			$(this).bind("submit", preventSubmitP);
			...
			if(typeof(self.attr("ajax"))!="undefined" && self.attr("ajax")=="true")
			{
				...
				$.ajax({
					...
					success: function(data){
						...
						form.one("submit", {'uploader':self}, submitFormP);
					}
				});
			}
			else
	...
	$(".IMG").each(function(index){
		var self = $(this);
		self.attr("id", "IMG"+index);
		if(typeof(self.attr("startOn"))!="undefined")
		{
			if(self.attr("startOn")=="manually")
			{
				self.after("<input type='button' value='" + (typeof(self.attr("buttonCaption"))=="undefined" ? '':self.attr("buttonCaption")) + "' id='IMG"+index+"start' class='uploadButton' />");
				$("#IMG"+index+"start").click(function(){
					updateUploadifySettingsP(self);
					self.uploadifyUpload();					
				});
			}
			else if(self.attr("startOn").substr(0, 8)=="onSubmit")
				$("#"+self.attr("startOn").substr(9)).one("submit", {'uploader':self}, submitFormP);
		}
		var fileExt = "";
		...
		self.uploadify({
			...
			'onComplete' : function(event, queueId, fileObj, response, data){
				...
				else
				{
					if(self.attr("startOn").substr(0, 8)=="onSubmit")
						$("#"+self.attr("startOn").substr(9)).append("<input type='hidden' name='IMGFiles[]' value='"+responseJSON.filename.replace("'", "%27")+"' />");
			...
			'onAllComplete' : function(event, data){
				if(typeof(self.attr("afterUpload"))!="undefined" && self.attr("afterUpload")!="link" && self.attr("afterUpload")!="filename" && self.attr("afterUpload")!="image" && self.attr("afterUpload")!="none")
				{
					...
					message = message.replace("IMGfilesUploaded", data.filesUploaded);
					message = message.replace("IMGerrors", data.errors);
					message = message.replace("IMGtotalSizeB", data.allBytesLoaded);
					message = message.replace("IMGtotalSizeKB", roundNumberP(data.allBytesLoaded/1024, 2));
					message = message.replace("IMGtotalSizeMB", roundNumberP(data.allBytesLoaded/1024/1024, 2));
					message = message.replace("IMGtotalSizeGB", roundNumberP(data.allBytesLoaded/1024/1024/1024, 2));
					message = message.replace("IMGspeedB", roundNumberP(Math.round(data.speed*1024), 2));
					message = message.replace("IMGspeedKB", roundNumberP(Math.round(data.speed), 2));
					message = message.replace("IMGspeedMB", roundNumberP(Math.round(data.speed/1024), 2));
					message = message.replace("IMGspeedGB", roundNumberP(Math.round(data.speed/1024/1024), 2));
					$("#" + self.attr("id") + "Queue").html(message);
				}
				if(self.attr("startOn").substr(0, 8)=="onSubmit")
				{
					var form = $("#"+self.attr("startOn").substr(9));
					form.unbind("submit", preventSubmitP);
					if(ajax=="true")
					{
