...
{  
	...
	if($formData[titulo] == "" or $formData[articulo] == "" )
	
		$objResponse->script("showError('Porfavor ingrese todos los datos del formulario',3000);");
	
	else {
		
		$title = htmlspecialchars($formData[titulo]);
                $texto = htmlspecialchars($formData[articulo]);
           include("../../../core/class/db.class.php");
		$db = new Database();
		$db->connect();
		 
		$db->doQuery("UPDATE cms_nosotros SET nosotros_title = '".mysql_real_escape_string($title)."', nosotros_article = '".mysql_real_escape_string($texto)."' WHERE nosotros_id = '$formData[id]'",UPDATE_QUERY);
		
		$objResponse->script("loading('Procesando',1);");
		$objResponse->script("setTimeout(\"unloading()\", 2000 );");
		$objResponse->script("showSuccess('Modificado Correctamente',3000);");
