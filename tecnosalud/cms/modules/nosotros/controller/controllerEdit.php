...
if($title == "" or $article == "")
	...
	{
		if($imageUpload == 0)
		$db->doQuery("UPDATE cms_news SET news_title = '$title', news_article = '$article' WHERE news_id = '$id'",UPDATE_QUERY);
		else{
		$nameImageUpload = $_POST["CMSFiles"][0];
		$db->doQuery("UPDATE cms_news SET news_title = '$title', news_article = '$article', news_image = '$nameImageUpload' WHERE news_id = '$id'",UPDATE_QUERY);
		}
