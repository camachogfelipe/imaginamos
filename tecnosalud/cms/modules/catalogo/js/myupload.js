jQuery(document).ready(function($){
	...
	$(".CMS").each(function(index){
		var self = $(this);
		self.attr("id", "CMS"+index);
		if(typeof(self.attr("startOn"))!="undefined")
		{
			if(self.attr("startOn")=="manually")
			{
				self.after("<input type='button' value='" + (typeof(self.attr("buttonCaption"))=="undefined" ? '':self.attr("buttonCaption")) + "' id='CMS"+index+"start' class='uploadButton' />");
				$("#CMS"+index+"start").click(function(){
					updateUploadifySettings(self);
					self.uploadifyUpload();					
				});
			}
		...
		self.uploadify({
			...
			'onComplete' : function(event, queueId, fileObj, response, data){
				...
				else
				{
					if(self.attr("startOn").substr(0, 8)=="onSubmit")
						$("#"+self.attr("startOn").substr(9)).append("<input type='hidden' name='CMSFiles[]' value='"+responseJSON.filename.replace("'", "%27")+"' />");
			...
			'onAllComplete' : function(event, data){
				if(typeof(self.attr("afterUpload"))!="undefined" && self.attr("afterUpload")!="link" && self.attr("afterUpload")!="filename" && self.attr("afterUpload")!="image" && self.attr("afterUpload")!="none")
				{
					...
					message = message.replace("CMSfilesUploaded", data.filesUploaded);
					message = message.replace("CMSerrors", data.errors);
					message = message.replace("CMStotalSizeB", data.allBytesLoaded);
					message = message.replace("CMStotalSizeKB", roundNumber(data.allBytesLoaded/1024, 2));
					message = message.replace("CMStotalSizeMB", roundNumber(data.allBytesLoaded/1024/1024, 2));
					message = message.replace("CMStotalSizeGB", roundNumber(data.allBytesLoaded/1024/1024/1024, 2));
					message = message.replace("CMSspeedB", roundNumber(Math.round(data.speed*1024), 2));
					message = message.replace("CMSspeedKB", roundNumber(Math.round(data.speed), 2));
					message = message.replace("CMSspeedMB", roundNumber(Math.round(data.speed/1024), 2));
					message = message.replace("CMSspeedGB", roundNumber(Math.round(data.speed/1024/1024), 2));
