...
if ($c>0){
    ...
    $output_string .= "<table border='1' width='500px' ><tr style='background-color:#E0E0E0 ;'>";
        ...
        $output_string .= "<td><center><b>Estado/Codigo</b></center></td>";
    ...
    for ($i = 0, $fin = count($result); $i < $fin; $i++) {
       // $output_string .= "<tr>";
        ...
        if ($result2[0]['id_EstadoVenta'] == 2) {
            $output_string .= '<td>' . $result[$i]['nombre_Cliente'] . '</td>';
            $output_string .= '<td>' . $result[$i]['celular_Cliente'] . '</td>';
            $output_string .= '<td><div style="font-size: 11px;color: green;"><center>' . $result2[0]['nombre_Estado'] . '</center></div></td>';
            $output_string .= '<td>' . $result[$i]['fecha_Venta'] . '</td>';
        } elseif ($result2[0]['id_EstadoVenta'] == 3) {
            $output_string .= '<td>' . $result[$i]['nombre_Cliente'] . '</td>';
            $output_string .= '<td>' . $result[$i]['celular_Cliente'] . '</td>';
            $output_string .= '<td><div style="font-size: 11px;color: blue;"><center>' . $result[$i]['codigo_Venta'] . '</center></div></td>';
            $output_string .= '<td>' . $result[$i]['fecha_Venta'] . '</td>';
        }
        $output_string .= "</tr>";
        $j++;
