...
;(function($){
	...
	var iButton = function (input, options){
		var self = this
			...
			, bDefaultLabelsUsed = (options.labelOn == Si && options.labelOff == No)
    ...
    $input
    	...
    	.after(
			'<div class="' + options.classHandle + '"><span class="handle_bg"/></div>'
    ...
    // automatically resize the control
		...
		var handleRight = width.container - width.handle - 1;
    
		var positionHandle = function (animate){
			var checked = $input[0].checked
				, x = (checked) ? handleRight : -1
	...
	var defaults = {
		...
		, labelOn: "Si"                           // the text to show when toggled on
	...
	}, Si = defaults.labelOn, No = defaults.labelOff;
