...
<html lang="es">
    <head>
        ...
        <meta charset="UTF-8">
        <title>CMS imaginamos V.1.7.1</title>
        <meta name="viewport" content="initial-scale=1.0,maximum-scale=1.0,user-scalable=no">
        <link rel="icon" type="image/gif" href="<?php echo back_asset('beoro/img/ico-imaginamos.gif') ?>">
        ...
        <link rel="stylesheet" href="<?php echo back_asset('beoro/css/radio.css') ?>">
        <?php if (!empty($public_assets['css'])) : ?>
            <?php foreach ($public_assets['css'] as $css) : ?>
...
<!--[if IE 9]><link rel="stylesheet" href="<?php echo back_asset('beoro/css/ie9.css') ?>"><![endif]-->
    ...
    <footer>
        <div class="container ">
            <div class="row">
                <div class="span5">
                    <div>&copy; Copyright 2013 <span class="tip"><a  href="http://imaginamos.com/" title="Todos los derechos reservados" target="_blank" >Imaginamos.com</a> </span> </div>
                </div>
                <div class="span7">
    ...
    <script src="<?php echo back_asset('beoro/js/lib/coolcarrousel/jquery.carouFredSel-6.0.4-packed.js') ?>"></script>
    <script src="<?php echo back_asset('beoro/js/lib/coolcarrousel/jquery.carouFredSel-6.0.4-packed.js') ?>"></script>
    <script src="<?php echo back_asset('beoro/js/lib/x-editable/bootstrap-editable/js/bootstrap-editable.min.js') ?>"></script>
    <script src="<?php echo back_asset('beoro/js/lib/x-editable/inputs-ext/address/address.js') ?>"></script>
    <script src="<?php echo back_asset('beoro/js/handlebars.js') ?>"></script>
    <!-- FUNCIONES -->
    <script src="<?php echo back_asset('beoro/js/pages/beoro_notifications.js') ?>"></script>
    <script src="<?php echo back_asset('beoro/js/pages/beoro_form_elements.js') ?>"></script>
    <script src="<?php echo back_asset('beoro/js/pages/beoro_form_validation.js') ?>"></script>
    <script src="<?php echo back_asset('beoro/js/functions.js') ?>"></script>
    ...
    if($respuesta){
      if(substr($respuesta, 0, 5) == "Error") {?>
        <script>
          $.sticky("<?= $respuesta ?>", {autoclose : 5000, position: "top-right" , type: "st-error"});
        </script>
      <?php 
      }else if (substr($respuesta, 0, 5) == "Alert"){
        ?>
        <script>
          $.sticky("<?= $respuesta ?>", {autoclose : 5000, position: "top-right" , type: "st-info"});
        </script>
        <?php
      }else{
        ?>
        <script>
          $.sticky("<?= $respuesta ?>", {autoclose : 5000, position: "top-right" , type: "st-success"});
        </script>
        <?php
      }
    }
