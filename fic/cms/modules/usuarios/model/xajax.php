...
{  
	...
	$maximoscaracteres=38; 

	if($formData[nombre] == "" or $formData[apellido] == "" or $formData[email] == "" or $formData[telefono] == "" or $formData[pais] == "" or $formData[ciudad] == "" or $formData[contrasena] == "" or $formData[subtitulo] == "" or $formData[descripcion2] == "")
	
		$objResponse->script("showError('Lo sentimos todos los campos son obligatorios.',3000);");
		...
		$query = "INSERT INTO t_usuarios (id_usuario,nombre,apellido,email,telefono,pais,ciudad,contrasena,subtitulo,descripcion) VALUES ('','$formData[nombre]','$formData[apellido]','$formData[email]','$formData[telefono]','$formData[pais]','$formData[ciudad]','$formData[contrasena]','$formData[subtitulo]','$formData[descripcion2]')";
...
function menus_edit( $formData )
{  
	$objResponse = new xajaxResponse();
	$maximoscaracteres=38; 
	if($formData[img] == "" or $formData[texto] == "" or strlen($formData[texto])>$maximoscaracteres)
	
		$objResponse->script("showError('Ingrese imagen, texto con menos de 38 caractereshh',3000);");
		...
		$query = "UPDATE t_usuarios SET img = '$formData[img]', texto = '$formData[texto]' WHERE id_img ='$formData[id_img]'";
		$db->doQuery($query,INSERT_QUERY);
...
$xajax->registerFunction("menus_edit");
