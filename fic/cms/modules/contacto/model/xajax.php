...
{  
	...
	if($formData[img] == "" or $formData[texto] == "" or strlen($formData[texto])>$maximoscaracteres)
		...
		$objResponse->script("showError('Ingrese imagen, texto con menos de 38 caracteres',3000);");
		...
		$query = "INSERT INTO t_carrusel (id_img,img,texto) VALUES ('','$formData[img]','$formData[texto]')";
...
{  
	...
	if($formData[img] == "" or $formData[texto] == "" or strlen($formData[texto])>$maximoscaracteres)
		...
		$query = "UPDATE t_carrusel SET img = '$formData[img]', texto = '$formData[texto]' WHERE id_img ='$formData[id_img]'";
		$db->doQuery($query,INSERT_QUERY);
