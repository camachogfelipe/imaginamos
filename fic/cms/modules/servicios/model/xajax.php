...
{  
	...
	if($formData[titulo] == "" or $formData[subtitulo] == "" or $formData[descripcion_detalle] == "" or $formData[descripcion_completa] == "" ){
		$objResponse->script("showError('Verifique que todos los campos se encuentren diligenciados',3000);");
	}else{
		if(strlen($formData[titulo])>23 or strlen($formData[subtitulo])>27 or strlen($formData[descripcion_detalle])>80 or strlen($formData[descripcion_completa])>1300){
			$objResponse->script("showError('Verifique que la cantidad de caracteres corresponda a la especificada',3000);");
		}else {
		...
		$query = "INSERT INTO t_servicios(id_servicios, titulo, subtitulo, descripcion_detalle, descripcion_completa, datos_cuenta)VALUES('','$formData[titulo]','$formData[subtitulo]','$formData[descripcion_detalle]','$formData[descripcion_completa]','$formData[datos_cuenta]')";
