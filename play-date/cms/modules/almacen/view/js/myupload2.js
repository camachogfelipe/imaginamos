jQuery(document).ready(function($){
	...
	$(".IMU2").each(function(index){
		var self = $(this);
		self.attr("id", "IMU2"+index);
		if(typeof(self.attr("startOn"))!="undefined")
		{
			if(self.attr("startOn")=="manually")
			{
				self.after("<input type='button' value='" + (typeof(self.attr("buttonCaption"))=="undefined" ? '':self.attr("buttonCaption")) + "' id='IMU2"+index+"start' class='uploadButton' />");
				$("#IMU2"+index+"start").click(function(){
					updateUploadifySettings2(self);
					self.uploadifyUpload();					
				});
			}
		...
		self.uploadify({
			...
			'onComplete' : function(event, queueId, fileObj, response, data){
				...
				else
				{
					if(self.attr("startOn").substr(0, 8)=="onSubmit")
						$("#"+self.attr("startOn").substr(9)).append("<input type='hidden' name='IMU2Files[]' value='"+responseJSON.filename.replace("'", "%27")+"' />");
			...
			'onAllComplete' : function(event, data){
				if(typeof(self.attr("afterUpload"))!="undefined" && self.attr("afterUpload")!="link" && self.attr("afterUpload")!="filename" && self.attr("afterUpload")!="image" && self.attr("afterUpload")!="none")
				{
					...
					message = message.replace("IMU2filesUploaded", data.filesUploaded);
					message = message.replace("IMU2errors", data.errors);
					message = message.replace("IMU2totalSizeB", data.allBytesLoaded);
					message = message.replace("IMU2totalSizeKB", roundNumber2(data.allBytesLoaded/1024, 2));
					message = message.replace("IMU2totalSizeMB", roundNumber2(data.allBytesLoaded/1024/1024, 2));
					message = message.replace("IMU2totalSizeGB", roundNumber2(data.allBytesLoaded/1024/1024/1024, 2));
					message = message.replace("IMU2speedB", roundNumber2(Math.round(data.speed*1024), 2));
					message = message.replace("IMU2speedKB", roundNumber2(Math.round(data.speed), 2));
					message = message.replace("IMU2speedMB", roundNumber2(Math.round(data.speed/1024), 2));
					message = message.replace("IMU2speedGB", roundNumber2(Math.round(data.speed/1024/1024), 2));
