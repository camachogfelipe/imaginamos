...
class Galerias_hacemos extends MX_Controller {
  ...
  function __construct() {
    parent::__construct();
    $this->gallery_path = realpath(APPPATH . "../application/modules/galerias_hacemos/assets/images");
  ...
  function index($return = '') {
    ...
    $this->load->model('galerias_hacemos_model');        

    if ($query = $this->galerias_hacemos_model->getRecords($idp)) {
    ...
    $this->load->view('galerias_hacemos_view', $template_date);
  }
  ...
  function edit($return = '') {
    ...
    if ($query = $this->galerias_hacemos_model->getRecord()) {
    ...
    $this->load->view('galerias_hacemos_edit', $template_date);
  }
  ...
  function add($return = '') {
    ...
    $this->load->view('galerias_hacemos_new', $template_date);
  }
    ...
    if ($is_logged === TRUE) {
      ...
      $this->load->model('galerias_hacemos_model');
        ...
        if ($this->upload->do_upload()) {
          ...
          $galerias_hacemos['imagen'] = $name;
        }

        $return = $this->galerias_hacemos_model->update_record($this->input->post('id'), $galerias_hacemos);
        if ($return) {
          $this->index('ok');
        } else {
          $this->add('erSizeImg');
  ...
  function new_record() {
    ...
    $idp = $this->uri->segment(3);
      ...
      $galerias_hacemos['quehacemos_id'] = $idp;

      $result = $this->galerias_hacemos_model->add_record($galerias_hacemos);
  ...
  function delete() {
    ...
    $this->load->model('galerias_hacemos_model');
    $this->galerias_hacemos_model->delete_record();
