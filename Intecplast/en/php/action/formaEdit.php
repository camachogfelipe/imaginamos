...
$forma_id = str_replace(',', '.',$forma_id);
if (!validarNumero($forma_id)){
    header($location.'&error1');
    exit;
}

if (!validarTexto($forma_nombre_e)){
    header($location.'&error1');
    exit;
}

if (!validarTexto($forma_nombre_i)){
    header($location.'&error1');
    exit;
}
...
$location = "location: ./../../admin/formasAdd.php?edit=";
