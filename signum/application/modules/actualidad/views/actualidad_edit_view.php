    ...
    <div class="widget"> 
        ...
        <div class="formEl_b">
            ...
            <fieldset>
                      ...
                      <form enctype="multipart/form-data" action="<?php echo base_url(); ?>actualidad/update" method="POST" onsubmit="return validate();" >
                <img src="<?php echo base_url().'assets/actualidad_img/'.$data->image; ?>" width="200" /> <br>
                <br>
                <br>
                <br>
                <label>Imagen<strong> (996 x 308px)</strong></label>
                ...
                <br>
                <br>
                <label>pdf</strong></label>
                <input class="file fileupload2" placeholder="Seleccionar archivo" style="display: inline; color: rgb(102, 102, 102); font-size: 11px; width: 142px;">
                <div class="filebtn" style="width: 190px; height: 30px; background-image: url(<?php echo base_url(); ?>/alcance/edit/images/addFiles.png); display: inline; position: absolute; margin-left: -168px; background-position: 100% 50%; background-repeat: no-repeat no-repeat;">
                	<input type="file" name="pdf" value="" title="Solo archivos pdf" class="fileupload2" style="position: absolute; height: 30px; width: 170px; margin-left: 5px; display: inline; cursor: pointer; opacity: 0;">
                </div>
                <br>
                <br>
                <label>Titulo<strong> *</strong></label>
                <input type="text" name="title" id="title" class="large" value="<?php echo $data->title; ?>" />
                <br>
                <br>
                <label>Fecha<strong> (mm/dd/yyyy)*</strong> </label>
                <?php
                                        	$fecha = substr($data->date,0,10);
																					$fecha = explode("-", $fecha);
																					$ffecha = array($fecha[1], $fecha[2], $fecha[0]);
																					$fecha = implode("/", $ffecha);
																				?>
                <input type="text" tabindex="1" name="date" id="dateinput" size="10" value="<?= $fecha ?>">
                <br>
                <br>
                ...
                <?php if($data->daily == 1): ?>
                <input type="checkbox" name="daily" id="daily" checked />
                <?php else: ?>
                ...
                <input type="hidden" name="pdfOld" value="<?php echo $data->pdf; ?>" />
              </form>
                    </fieldset>
          </div>
                </div>
...
<!--// End content -->
    ...
    jQuery(document).ready(function(){
        jQuery("textarea").cleditor();
				jQuery("#dateinput").mask("99/99/9999");
    });
    function validate(){
