 ...
 * @author Jose Fonseca
 */
class Tecnico extends Front_Controller {
    ...
    public function index() {
        ...
        $this->_data['categorias'] = $categorias->join_related('imagen')->where('categoria','CERTIFICACIONES')
                                                ->or_where('categoria','PROPUESTA DE VALOR')
                                                ->or_where('categoria','SERVICIOS')
                                                ->get();
        
        //  $banners = new\ banners();
        $this->_data['dat_ban'] = array(); //$banners->get_banner();
        
        return $this->build('tecnico.php');
    }
