 ...
 * @author Elbert Tous
 */
class Comunicaciones extends Front_Controller {
    ...
    public function index() {
        ...
        return $this->build('comunicaciones.php');
    }
    ...
    public function categorias() {
        ...
        $cat = new categoria();
        $linea = new linea();
        $linea->get_by_titulo('LÍNEA DE COMUNICACIÓN');
    ...
    public function get_list_categoria($parent, $level, $linea) {
        ...
        if ($cat->exists()) {
            foreach ($cat as $row) {
                ...
                if ($cat->result_count() > 1) {
                    $html .= "<a id=\"action_ajax\" class=\"menu-vn-t" . ($level + 1) . "\" href='#' data-url=\"".base_url()."comunicaciones/grupos\" data-datos=\"id=".$row->id."\" >" . $row->categoria . "</a>";
      ...
      public function get_list() {
           ...
           $html .= "<ul class=\"menu-lt\">";
               ...
               foreach ($data as $v) {
                  $html .= "<li class=\"menu-lt-act\"><a id=\"action_ajax\" data-url=\"".base_url()."comunicaciones/subcategorias\" href='#' data-datos=\"id=".$v->id."\" class=\"menu-vn-t2\" >".$v->categoria."</a>";
    ...
    public function items_group($rowId, $level,$linea){
       $html = "";  
        if (!is_null($rowId)) {
                        ...
                        if ($group->exists()) {
                            foreach ($group as $gr) {
                                $html .= "<li><a id=\"action_ajax\" class=\"menu-vn-t" . ($level + 2) . "\" data-url=\"".base_url()."comunicaciones/productos\" data-datos=\"id=".$gr->id."&cat=".$rowId."\" href=\"#\"  >" . $gr->grupo . "</a></li>";
                            }
                        }
                    }
       return $html;              
