...
function printTableMenus()
		{
		$query = "SELECT * FROM cms_menu WHERE id_parent = 0 ORDER BY menu_id DESC";
		$this->db->doQuery($query,SELECT_QUERY);
		$results = $this->db->results;
		$numOfResults = $this->db->getNumResults();
  ...
  <tr>
	...
	if($numOfResults>0)
	{
		...
		$html .= '
			...
			<td class="center" width="100px">
                            <a  href="menu.php?id_parent='.$result[menu_id].'" class="uibutton icon confirm edit ">Hijos</a>
                            <a id="'.$result[menu_id].'" class="Delete uibutton special" tableToDelete="cms_menu" nameField="menu_id">Eliminar</a>		
			</td>
			</tr>';
		}
	}
...
function printTableMenusChildrens($id_parent)
		{
    
                $query = "SELECT * FROM cms_menu WHERE menu_id = " . $id_parent;
		$this->db->doQuery($query,SELECT_QUERY);
		$parent = $this->db->results;
    

		$query = "SELECT * FROM cms_menu WHERE id_parent = " . $id_parent. " ORDER BY menu_id DESC";
		$this->db->doQuery($query,SELECT_QUERY);
		$results = $this->db->results;
		$numOfResults = $this->db->getNumResults();
		...
		$html = '
                ...
                <div>
				  ...
				  <form id="menus">         
                                  <a  href="menu.php" class="uibutton icon confirm edit ">Regresar</a>
                                  <input type="hidden" name="id_parent" value="' . $id_parent . '" />
                                      
                  <legend><h1>MENUS HIJOS DE ' . $parent[0]['menu_title'] . '</h1></legend>                    
                  <div class="section">
                      <label>Nombre del menú</label>
                      <div><input type="text" name="namemenu" id="namemenu" class="large"/></div>
				  </div>
				  ...
				  <a class="uibutton" onclick="xajax_menusChildren(xajax.getFormValues(\'menus\')); return false;">Agregar menú</a> 
            	  </form>
