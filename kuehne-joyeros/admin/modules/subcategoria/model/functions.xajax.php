...
{  
	...
	if($formData[categoria] == 0)
	...
	else if($formData[desc_en] == "" or $formData[desc_en] == "")
	
		$objResponse->script("showError('Ingrese un titulo Sub Categoria en Español e Ingles',3000);");
		...
		$db->doQuery("INSERT INTO cms_subcategorias (desc_subcategoria_es,desc_subcategoria_en,id_categoria) VALUES ('$formData[desc_es]','$formData[desc_en]','$formData[categoria]')",INSERT_QUERY);
...
{  
	...
	if($formData[categoria] == 0)
		...
		$db->doQuery("UPDATE cms_subcategorias SET desc_subcategoria_es = '$formData[desc_es]', desc_subcategoria_en = '$formData[desc_en]', id_categoria = '$formData[categoria]' WHERE id_subcategoria = '$formData[id]'",UPDATE_QUERY);
