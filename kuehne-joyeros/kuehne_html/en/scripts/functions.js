...
(function($) {
	...
	$(window).bind("load", function() {			
		...
		$("#slider").sudoSlider({prevNext:false,numeric:true});
		...
		$(".menu_desplegado").bind("mouseleave", function(){		
			$('.menu_desplegado').slideUp(500);
			$('.catalogo').removeClass('active');
			$('.content_filtrar').slideUp(500);
			$('.filtrar').removeClass('active');
			$('.content_servicio').slideUp(500);
		...
		if( $(this).attr("value")  == 'Name' || $(this).attr("value")  == 'E-mail' || $(this).attr("value")  == 'Comentario' || $(this).html("Comentario") || $(this).attr("value")  == 'Tel\xe9fono' || $(this).attr("value")  == 'Email' ){
            $(this).attr("value", "");
        }
