...
if (isset($_GET["id_del"])) {
    if ($_GET["confirm"] == base64_encode(md5($_GET["id_del"]))) {
        ...
        $todos = $db->results;
        if (count($todos) > 1) {
            $db->doQuery("DELETE FROM promociones WHERE id=" . (int) $_GET["id_del"], DELETE_QUERY);
        } else {
            $valor = 0;
        }
    }
...
?>
<?php
$db->doQuery("SELECT * FROM  promociones ORDER BY id ASC", SELECT_QUERY);
$dataAll = $db->results;
?>
<script type="text/javascript" language="javascript">
    function confirmar() {
        var answer = confirm("¿Desea borrar la promoción?");
        if (answer) {
            return true;
        } else
            return false;
...
<!-- full width -->
<div class="widget">
    <div class="header"><span><span class="ico gray window"></span>PROMOCIONES</span>
    </div><!-- End header -->
    <div class="content">

        <div class="formEl_b">

            <a class="uibutton normal" href="index.php?seccion=productos&id=0">Agregar nueva promoci&oacute;n</a>
            ...
            <table class="display" >
                ...
                <tbody>
                    <?php
                    $db->doQuery("SELECT * FROM promociones ORDER BY id DESC", SELECT_QUERY);
                    $dataAll = $db->results;
                    for ($i = 0, $tot = count($dataAll); $i < $tot; $i++) {
                        ...
                        <tr class="odd gradeX">
                            <td class="center" width="100px">
                                <img src="../../../../img/promociones/166_151_<?php echo $data["img"] . "?" . rand(0, 9999999); ?>" width="166" height="151" />
                            </td>                                                      
