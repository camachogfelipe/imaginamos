...
if (isset($_POST["id"])) {
    ...
    if ($id == 0) {
        ...
        $db->doQuery("UPDATE subproductos_detalles SET                                           
                    texto='" . $texto . "' ,                            
                    receta='" . GetData("receta") . "'                             
                    WHERE id_subproducto=" . $id, 4);
        $retorno = ClassFile::UploadImagenFile("img", "../../../../img/subproductos/california", "productos_" . $id, "520_300_productos_" . $id, 520, 300);
        if ($retorno["Status"] == "Uploader") {
            //  echo $retorno["NameFile"];
            $db->doQuery("UPDATE subproductos_detalles SET img='" . $retorno["NameFile"] . "' WHERE id_subproducto=" . $id, 4);
            $val = 2;
        } else {
            //  echo $retorno["Error"];
            $val = 2;
...
$db->doQuery("SELECT * FROM subproductos_detalles WHERE id_subproducto=" . $id, 1);
...
<div class="widget">
    <div class="header">
        <span>
            ...
            PRODUCTOS CALIFORNIA >> PRODUCTOS >> SUBPRODUCTOS >> DETALLES
        </span>
    </div>
    <div class="content">
        <div class="formEl_b">
            ...
            <fieldset>
                <a class="uibutton confirm" href="index.php?seccion=subproductos&id=<?php echo $_GET["id"]?>">Atr&aacute;s</a><br><br>
                <h3><?php echo ($id == 0) ? "Agregando" : "Editando" ?></h3>
                <form method="post" action="" name="forminterno" id="forminterno" enctype="multipart/form-data">
                    <input type="hidden" value="<?php echo $oficina["id_subproducto"] ?>" name="id" id="id"> 
                    ...
                    <?php if ($id > 0) { ?>
                        <?php if ($oficina["img"] != "") { ?>
                            <div>
                                ...
                                <div  style="margin-left: 200px">
                                    <br><br>
                                    <img src="../../../../img/subproductos/california/520_300_<?php echo $oficina["img"] . "?" . rand(0, 9999999); ?>" width="520" height="300" />
                                </div>
                            </div>  
                            <?php
                        }
