...
class Correo {
   ...
   */
  public function SendEmail($para = "", $asunto = "", $cuerpo = "",$correo) {
...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
    ...
    $mail->SMTPAuth = $correo[0]["SMTPAuth"];
    // Obligamos certificado de seguridad (Debemos habilitar el SSL en la configuracion PHP)
    $mail->SMTPSecure = $correo[0]["SMTPSecure"];
    // Servidor smtp
    $mail->Host = $correo[0]["Host"];
    // Puerto de salida de envio de email
    $mail->Port =$correo[0]["Port"];
    // Usuario (Cuenta de correo)
    $mail->Username = $correo[0]["Username"];
    // Contrase�a
    $mail->Password = $correo[0]["Password"];
    // Desde que correo quiere que se muestre
    $mail->From = $correo[0]["Froms"];
    // Desde (Nombre)
    $mail->FromName = $correo[0]["FromName"];
    // Asunto
    $mail->Subject = $correo[0]["Subject"] . $asunto;
