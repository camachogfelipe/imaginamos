...
{  
	...
	if($formData[desc_es] == "")
		...
		$objResponse->script("showError('Ingrese un Contenido.',3000);");
		...
		$db->doQuery("UPDATE cms_contenidos SET descripcion_es = '$formData[desc_es]', descripcion_en = '$formData[desc_en]', link_cont = '$formData[link_c]' WHERE id_contenido = '$formData[id]'",UPDATE_QUERY);
...
{  
	...
	$db = new Database();
	$db->connect();
	$query = "SELECT * FROM cms_banner WHERE id_banner = ".$id;
	$db->doQuery($query,SELECT_QUERY);
	$results = $db->results;
	
	@unlink("../files/".$results[0][imagen]);
	...
	$objResponse = new xajaxResponse();
		...
		$db->doQuery("DELETE FROM cms_banner WHERE id_banner = ".$id,INSERT_QUERY);
		
		$objResponse->script("setInterval(\"window.location.reload();\", 3000 );");
		$objResponse->script("loading('Eliminado...',1);");
		$objResponse->script("setTimeout(\"unloading()\", 2000 );");
