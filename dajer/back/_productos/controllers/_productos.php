...
class _productos extends Back_Controller {
    ...
    public function form_edit() {
        ...
        $data['form_content'] .= $this->combox($obj->categoria_id,$new_obj->get_subcategorias_list(), "categoria_id", "Sub-Categoria", "Seleccione una sub-categoria", $new_obj->is_rule("categoria_id","required"), "span5");
        //$data['form_content'] .= $this->text($obj->intro, "intro", "Introducción", "Maximo ".$obj->get_rule("intro","max_length")." caracteres", $obj->is_rule("intro","required"), "span7", false, true, $obj->get_rule("intro","max_length"),3,3);
        $data['form_content'] .= $this->text($obj->texto, "texto", "Texto descriptivo", "", $obj->is_rule("texto","required"), "span7", true, false, $obj->get_rule("texto","max_length"),3,4);
    ...
    public function form_add() {
        ...
        $new_obj = new categoria();// $this->model();
        $data['form_content'] .= $this->combox(0,$new_obj->get_subcategorias_list(), "categoria_id", "Sub-Categoria", "Seleccione una sub-categoria", $new_obj->is_rule("categoria_id","required"), "span5");
        //$data['form_content'] .= $this->text($obj->intro, "intro", "Introducción", "Maximo ".$obj->get_rule("intro","max_length")." caracteres", $obj->is_rule("intro","required"), "span7", false, true, $obj->get_rule("intro","max_length"),3,3);
        $data['form_content'] .= $this->text("", "texto", "Texto descriptivo", "", $obj->is_rule("texto","required"), "span7", true, false, $obj->get_rule("texto","max_length"),3,4);
        $data['direct_form'] = $this->name.'/add'; 
        return $this->buildajax($this->name.'/form', $data);
    }
