...
function __autoload($fClass) {
  ...
  try {
    ...
    } else if (!class_exists($fClass, false)) {
      throw new Exception('La Clase: ' . $fClass . ' no se puede encuentrar');
    }
  } catch (Exception $e) {
    echo $e->getMessage();
    die();
...
// Funcion para saber si es fan de la APP
function FacebookUserIsFan( $facebook=NULL ) {
  $fql = "SELECT uid from page_fan WHERE uid=me() AND page_id='" . FACEBOOK_PAGE_ID . "'";
