...
$('.searchAutocomplete').click(function() {
 $('.searchText').toggle('slow', function() {
	...
	$('.deleteMenu').live('click',function() { 
		  ...
		  loading(str,overlay);
		  setTimeout("unloading()",1500);
		   setTimeout( "window.location.href='../../../js/logout.php'", 2000 );
	  });
 ...
 function Processgraph(){
	...
	// mutiupload
	  ...
	  'sizeLimit'	: parseInt($('#maxUploadFileSize').text()),
		...
		'onSelectOnce'   : function(event,data) {
		  ...
		  if(data.errors){
						$('#status-message').html('Completo '+ data.filesUploaded + ' archivos, <font color=red>y ' + data.errors + ' no fueron cargados </font>.');
						showError('Carga completa '+ data.filesUploaded + 'archivos, <font color=red>y ' + data.errors + ' no fueron cargados </font>.',7000);
						
				  }else{
						  $('#status-message').html('Completo '+ data.filesUploaded + ' file');
						  showSuccess('Carga completa '+ data.filesUploaded  +' File',7000);
	...
	function Delete(data,name,row,type,dataSet,tableToDelete,nameField){
				...
				var data = data;
		$.confirm({
		'title': 'Confirme','message': "<strong>¿Realmente desea borrar este ítem?</strong>",'buttons': {'SI': {'class': 'special',
		'action': function(){
					...
					loading('Checking');
								  ...
								  $.get("../../../js/deleteItemTable.php", {id: data, ttd: tableToDelete, nf: nameField} );
									 
								  row.slideUp(function(){   showSuccess('Item eliminado correctamente!',5000); unloading(); }); return false;
								  //if(type==1){ row.slideUp(function(){   showSuccess('Item eliminado correctamente!',5000); unloading(); }); return false;}
								  //showSuccess('Menú eliminado correctamente!',5000);
