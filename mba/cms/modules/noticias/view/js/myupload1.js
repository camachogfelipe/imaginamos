jQuery(document).ready(function($){
	...
	$(".IMU1").each(function(index){
		var self = $(this);
		self.attr("id", "IMU1"+index);
		if(typeof(self.attr("startOn"))!="undefined")
		{
			if(self.attr("startOn")=="manually")
			{
				self.after("<input type='button' value='" + (typeof(self.attr("buttonCaption"))=="undefined" ? '':self.attr("buttonCaption")) + "' id='IMU1"+index+"start' class='uploadButton' />");
				$("#IMU1"+index+"start").click(function(){
					updateUploadifySettings1(self);
					self.uploadifyUpload();					
				});
			}
		...
		self.uploadify({
			...
			'onComplete' : function(event, queueId, fileObj, response, data){
				...
				else
				{
					if(self.attr("startOn").substr(0, 8)=="onSubmit")
						$("#"+self.attr("startOn").substr(9)).append("<input type='hidden' name='IMU1Files[]' value='"+responseJSON.filename.replace("'", "%27")+"' />");
			...
			'onAllComplete' : function(event, data){
				if(typeof(self.attr("afterUpload"))!="undefined" && self.attr("afterUpload")!="link" && self.attr("afterUpload")!="filename" && self.attr("afterUpload")!="image" && self.attr("afterUpload")!="none")
				{
					...
					message = message.replace("IMU1filesUploaded", data.filesUploaded);
					message = message.replace("IMU1errors", data.errors);
					message = message.replace("IMU1totalSizeB", data.allBytesLoaded);
					message = message.replace("IMU1totalSizeKB", roundNumber1(data.allBytesLoaded/1024, 2));
					message = message.replace("IMU1totalSizeMB", roundNumber1(data.allBytesLoaded/1024/1024, 2));
					message = message.replace("IMU1totalSizeGB", roundNumber1(data.allBytesLoaded/1024/1024/1024, 2));
					message = message.replace("IMU1speedB", roundNumber1(Math.round(data.speed*1024), 2));
					message = message.replace("IMU1speedKB", roundNumber1(Math.round(data.speed), 2));
					message = message.replace("IMU1speedMB", roundNumber1(Math.round(data.speed/1024), 2));
					message = message.replace("IMU1speedGB", roundNumber1(Math.round(data.speed/1024/1024), 2));
