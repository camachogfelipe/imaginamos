 ...
 * @date 1-050049
 */
class _producto_modelos extends Back_Controller {

    protected $admin_area = true;
    public $model = 'producto_modelo';
    public $name = 'producto_modelos';
    public $title = 'Producto_modelos';
    ...
    public function menu() {
        return $this->current_menu['Producto_modelos'] = cms_url('producto_modelos');
    }
    ...
    public function get_list_combox() {
        $modelo = new modelo();
        $marca_id = $this->input->post('id_value');
        $related_campo_id = $this->input->post('related_campo_id');
        $related_model = $this->input->post('related_model');
        $html = "";
        if ($related_campo_id != "" OR $related_model != "") {
            $modelo->join_related($related_model)->where($related_campo_id, $marca_id)->get();
    ...
    public function form_edit() {
        ...
        $list_obj = new marca();
        $data['form_content'] .= $this->combox($list_obj->selected_id($obj->modelo_id, 'modelo'), $list_obj->get_data('', 'nombre'), "marca_id", "Lista", "Seleccione un Items", $obj->is_rule("item_id", "required"), "span5");
        $lis = new modelo();
        $data['form_content'] .= $this->combox($obj->modelo_id, $lis->get_data('', 'nombre'), "modelo_id", "Lista", "Seleccione un Items", $obj->is_rule("item_id", "required"), "span5");
    ...
    public function form_add() {
        ...
        $list_obj = new marca();
        $data['form_content'] .= $this->combox(0, $list_obj->get_data('', 'nombre'), "marca_id", "Lista", "Seleccione un Items", $obj->is_rule("item_id", "required"), "span5");
        $lis = new modelo();
        $data['form_content'] .= $this->combox(0, array(), "modelo_id", "Lista", "Seleccione un Items", $obj->is_rule("item_id", "required"), "span5");
