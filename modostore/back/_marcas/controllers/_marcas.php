...
class _marcas extends Back_Controller {
	...
	public function menu() {
		return $this->current_menu['Home']['Nuestras Marcas'] = cms_url('marcas');
	}
	...
	public function form_edit() {
		$obj = new $this->model();
		$id = $this->_post('id');
		$obj->join_related('imagen')->get_by_id($id); /* o $obj->join_relate('mixed_model')->get_by_id($id); */
		$data['form_content'] = "";
		$data['form_content'] .= $this->inputHidden($obj->id, "id", 11);
                $data['form_content'] .= $this->imagen($obj->imagen_id, $obj->imagen_path, "Imagen", "235px × 125px",false,$obj->is_rule("imagen_id","required"),"span3");
		$data['form_content'] .= $this->input($obj->nombre, "nombre", $obj->get_rule("nombre","max_length"), "Nombre de Marca", "Maximo ".$obj->get_rule("nombre","max_length")." caracteres",$obj->is_rule("nombre","required"),"span3");
	...
	public function form_add() {
		...
		$data['form_content'] .= $this->inputHidden("","id", 11);
                $data['form_content'] .= $this->imagen("", NULL, "Imagen", "235px × 125px",false,$obj->is_rule("imagen_id","required"),"span3");
		$data['form_content'] .= $this->input("", "nombre", $obj->get_rule("nombre","max_length"), "Nombre de Marca", "Maximo ".$obj->get_rule("nombre","max_length")." caracteres",$obj->is_rule("nombre","required"),"span3");
		$data['direct_form'] = $this->name.'/add';
		return $this->buildajax('control/form', $data);
	}
