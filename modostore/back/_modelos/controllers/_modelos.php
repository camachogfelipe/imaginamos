 ...
 * @date 1-050049
 */
class _modelos extends Back_Controller {

    protected $admin_area = true;
    public $model = 'modelo';
    public $name = 'modelos';
    public $title = 'Modelos';
    ...
    public function menu() {
        return $this->current_menu['Modelos'] = cms_url('modelos');
    }
    ...
    public function form_edit() {
        $obj = new modelo(); //$this->model();
        $id = $this->_post('id');
        $obj->get_by_id($id); /* o $obj->join_relate('mixed_model')->get_by_id($id); */
        $data['form_content'] = "";
        $data['form_content'] .= $this->inputHidden($obj->id, "id", 11);
        $list_obj = new marca();
        $data['form_content'] .= $this->combox($obj->marca_id, $list_obj->get_data('', 'nombre'), "marca_id", "Lista de Marcas", "Seleccione un Marca", $obj->is_rule("marca_id", "required"), "span5");
    ...
    public function form_add() {
        ...
        $list_obj = new marca();
        $data['form_content'] .= $this->combox(0, $list_obj->get_data('', 'nombre'), "marca_id", "Lista de Marcas", "Seleccione un Marca", $obj->is_rule("marca_id", "required"), "span5");
        $data['form_content'] .= $this->input("", "nombre", $obj->get_rule("nombre", "max_length"), "Nombre", "Maximo " . $obj->get_rule("nombre", "max_length") . " caracteres", $obj->is_rule("nombre", "required"), "span3");
        $data['direct_form'] = $this->name . '/add';
        return $this->buildajax('control/form', $data);
    }
    ...
    public function lista() {
        $obj = new $this->model();
        $data['datos'] = $obj->join_related('marca')->get(); /*  $obj->join_related('imagen')->get(); */
