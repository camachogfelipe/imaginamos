/*
 * @file               : funciones.js
 * @brief              : Clase interaccion consultas Ajax y demas funcionalidad javascript
 * @version            : 3.2
 * @ultima_modificacion: 09-sep-2013
 * @author             : Ruben Dario Cifuentes T, rubensho@misena.edu.co
 */

$(document).ready( function()
{
  // Funcion para validar el ingreso de caracteres a un input text
  $(".solo_numeros").validCampoFranz('0123456789');
  $(".solo_texto").validCampoFranz(' abcdefghijklmnÒopqrstuvwxyz·ÈÌÛ˙');
  $(".solo_texto_numeros").validCampoFranz('0123456789abcdefghijklmnÒopqrstuvwxyz·ÈÌÛ˙');
  ...
  $(".submit_frm").click(function (){
    ...
    $("."+idfrm+" .field_frm").each(function (index){
      ...
      var valueTemp = $(this).val()==undefined ? $(this).html() : $(this).val();

      if( $(this).attr("id")=="myFunct" ){
        if( banderaFunction ){
    ...
    if(formSubmit && formComplete){
      ...
      $.ajax({
        ...
        error: function (jqXHR, textStatus, errorThrown){
          ...
          alert( "Error\nAlgo ha salido mal. Por favor intÈntalo de nuevo en unos minutos -> "+textStatus);
        }
      });
    }
  });
    ...
    success: function( data ) {
      ...
      for( i=0; i<data.count; i++){
        var $option = $("<option></option>")
          .attr("value", data.datos[i].id)
          .text(data.datos[i].txt_nombre);
        self.append($option);
      }
      //$("#"+target).html( html );
    },
...
function RecargarProfesion2( target, value )
{
  var myObject = {myFunct:"RecargarProfesion", valor:value };
  $.ajax({
    url: ajaxRutaAbs,
    type: "POST",
    data: myObject,
...
// Funcion para retornar los servicios asociados a un proveedor para la reserva actual
function postuladosListado( oferta )
{
  var myObject = {myFunct:"PostuladoListado", id_oferta:oferta};
  $.ajax({
    url: ajaxRutaAbs,
    type: "POST",
    data: myObject,
