...
class registroPersona
{
      ...
      if( isset( $_SESSION["user"] ) )
      {
        ...
        $mSql = "SELECT a.id AS id_registro, a.id_tipo, a.txt_nickname, a.txt_email, ".
                       ...
                       "c.txt_perfil, c.txt_habilidades, ".
											 "d.id_area AS id_area ".
                  "FROM zona_registrados AS a ".
									"LEFT JOIN zona_personas AS b ON a.id = b.id_registrado ".
									"LEFT JOIN zona_personas_perfil AS c ON b.id = c.id_persona ".
									"LEFT JOIN zona_profesion AS d ON d.id = c.id_profesion ".
                 "WHERE a.id = '".$_SESSION["user"]["id"]."' ";

        $this -> cData = DbHandler::GetRow( $mSql );
				
				if(!isset($this->cProfe)) {
					$mProfesion = new Dbzona_profesion();
					$this->cProfe = $mProfesion->getList(array( "ind_estado" => "1", "id_area"=>(int)$this->cData["id_area"]));
				}
  ...
  private function capturar()
  {
    ...
    $this -> cData["lab_fec_ingreso"] = GetData( "lab_fec_ingreso", NULL );
