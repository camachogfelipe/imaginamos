...
class Ajax 
{
  ...
  public function FunctLogin()
  {
    ...
    $mTxtURLPerfil = GetData("txt_urlempresa", "" );
    ...
    {
      echo json_encode(array("title" => "Error", "message" => "Digite la contraseña"));
    }
    else
    {
      // Hacemos consulta a DB
      $cUsuarios = new Dbzona_registrados();
      $usuarios = $cUsuarios -> getList( array( "txt_nickname" => $mTxtNickname , "txt_passwd"=> sha1( $mTxtPasswd ), "ind_estado" => "1" ) );
      ...
      if ( count($usuarios) == 1 )
      {
        ...
        if( $_SESSION["user"]["id_tipo"] == '1' )
        {
          ...
          if ($mTxtURLPerfil!="") {
           ...
           echo json_encode( array( "event" => "window.location.href='".str_replace( "&amp;", "&", Link::ToSection( array( "s" => "detalle_persona" ) ) )."&id=".str_replace("=", "", $mTxtURLPerfil)."';" ) );
          }
          else{
      ...
      {
        unset( $_SESSION["user"] );
        echo json_encode( array( "title" => "Error", "message" => utf8_encode( "Revise correo electrónico y/o contraseña.\n\nSi usted es un usuario registrado y ha solicitado dar de baja su cuenta escriba un correo a info@zonafranca.com solicitando su activación en el sistema." ) ) );
      }
    }
  }
    ...
    if( isset( $_SESSION["user"] ) )
    {
      if( 0 != $mId )
      {
        ...
        if( 1 == count( $mOfertas ) )
        {
          ...
          echo json_encode( array( "title" => utf8_encode( "Éxito" ), "message" => utf8_encode( "Cambio de estado con éxito" ) ) );
        }
        else
          echo json_encode( array( "title" => "Error", "message" => utf8_encode( "Cambio de estado sin éxito" ) ) );
      }
      else
        echo json_encode( array( "title" => "Error", "message" => utf8_encode( "Cambio de estado sin éxito" ) ) );
    }
    ...
    $mHtml = '<script type="text/javascript">
                ...
                $(document).ready( function()
                {
                  ...
                  $(".trav_emar_a" ).click(function (){
                    ...
                    $("#if_si3").css( "display", "block" );
                    $("#if_no2").css( "display", "none" );
                  });
  
                  $(".trav_emar_b" ).click(function (){
                    $("#if_si2").css( "display", "none" );
                    $("#if_si3").css( "display", "none" );
                      ...
                      <div class="row-fluid form_fluid">
                        <div class="span12">
                          <div class="class_frm frmborrar_oferta">
                            ...
                            <input type="hidden" class="field_frm" id="radio" value="" title="Debe ingresar si consiguió empleado para su oferta" />
                              ...
                              <fieldset class="pasos_formulario" id="info1">
                                ...
                                <div id="if_si" style="display: none;" >
                                  ...
                                  <div id="if_si3" style="display: none;" >
                                    ...
                                    <div class="span4">
                                      <select class="selectdd" name="num_emple" id="num_emple" style="width:100%;">
                                        <option value=""></option>
                                    ...
                                    <div id="cedulas" style="" >
                                      <div class="clear espacio_en_blanco"></div>
                                      <div class="span8">
                                        <label>Cédula de los empleados contratados (separadas por comas )</label>
                                      </div>
                                      <div class="span4">
                                        <input type="text" name="num_cedulas" id="num_cedulas" placeholder="" value="">
                                      </div>
                                    </div>
                                  </div>
    ...
    if( isset( $_SESSION["user"] ) )
    {
      if( 0 != $mId )
      {
        ...
        if( 1 == count( $mOfertas ) )
        {
          ...
          echo json_encode( array( "title" => utf8_encode( "Éxito" ), "message" => utf8_encode( "Oferta eliminada con éxito" ), "event"=> urldecode( Link::ToSection( array( "s" => "zona_empresa" ) ) ) ) );
        }
        else
          echo json_encode( array( "title" => "Error", "message" => utf8_encode( "Oferta eliminada sin éxito" ) ) );
      }
      else
        echo json_encode( array( "title" => "Error", "message" => utf8_encode( "Oferta eliminada sin éxito" ) ) );
    }
  ...
  public function FunctDetalleOfertaH()
  {
    ...
    $mHtml = '<h1 class="centrar_contenido">'.$mResoult["txt_cargo"].'</h1>
              ...
              <div class="row-fluid">
                <div class="span4">
                  <div class="logo_empresa1 clearfix"><img alt="" src="'.$mSiteUrl.'/cms/files/empresas/emp_'.$mResoult["fil_logo"].'"></div>
                </div>
                <div class="span8">
                  ...
                  <p><span class="text_naranja">&Aacute;rea:</span> '.$mResoult["nom_area"].'</p>
                   <p><span class="text_naranja">Jerarquía:</span> '.$mResoult["nom_jerarquia"].' </p>
...
<h2>Información general del aspirante</h2>
<div class="clear espacio_en_blancopeque"></div>-->
...
<p><span class="text_naranja">Departamento de residencia del aspirante:</span> '.$mResoult["dep_aspi"].'</p>
 <p><span class="text_naranja">Ciudad / Municipio del aspirante:</span> '.$mResoult["ciu_aspi"].' </p>
  <p><span class="text_naranja">Edad:</span> '.$mResoult["num_edad_aspi"].'</p>
    ...
    $mHtml .= '<div class="clear espacio_en_blanco"></div>
                <div class="centrar_inline iconos_izq">
                  ...
                  <a id="mensaje" class="inline" href="#" onclick="window.open(\'mailto:?subject='.$mResoult["txt_cargo"].'&body='.$mResoult["txt_descripcion"].'\');"></a>
      ...
      if( $_SESSION["user"]["id_tipo"] == '2' )
      {
        $mIdPersona = $_SESSION["user"]["info"]["id"];
				...
				$mSql = "SELECT a.id ".
								...
								"WHERE a.id_oferta = '".$mIdOferta."' AND a.id_persona = '".$mIdPersona."' ";

    		$postulado = print_r(DbHandler::GetRow( $mSql ), true);
				
				if(!empty($postulado)) {
					$mHtml = '<h1 class="centrar_contenido">Disculpanos</h1>
										<div class="clear espacio_en_blanco"></div>
										<h2 class="centrar_contenido">Tu hoja de vida ya esta registrada para esta oferta</h2>
										<div class="sombra_division"></div>
					...
					//$mMessage = '<p>Ingresa <a href="'.Link::ToSection( array( "s" => "detalle_persona" ) ).'&email=1&id='.base64_encode( $mIdPersona ).'" target="_blank">aqu&iacute;</a> para ver el perfil del aspirante y acceder a los datos de contacto.</p>';
					$mMessage = '<p>Ingresa <a href="http://www.empleoenmarcha.com/index.php?base&seccion=detalle_persona&email=1&id='.base64_encode( $mIdPersona ).'" target="_blank">aqu&iacute;</a> para ver el perfil del aspirante y acceder a los datos de contacto.</p>';
					
					mail( $mResoult["txt_email"], "Alguien ha tocado tu puerta", $mMessage, $cabeceras );
  ...
  // Funcion para enviar correo electronico con informaicon de nueva contraseña
  ...
  public function FunctRecordarContrasenaActivar(){
    ...
    if(count($recordar)>0){
      ...
      if($_GET["estado"]==1){
        ...
        $cRegistrados->save();
        echo '<script>
          alert("Su nueva contraseña es: '.$datos[1].'");
          window.location.href = "'.Link::Build("").'";
        </script>';
      }else{
        echo '<script>
          alert("Se a eliminado la peticion de cambio de contraseña");
          window.location.href = "'.Link::Build("").'";
        </script>';
      }
    }else{
  ...
  public function FunctPostuladoListado()
  {
    $mIdOferta = GetData( "id_oferta" );

    $mSql = "SELECT b.id, b.txt_prim_nom, b.txt_seg_nom, b.txt_prim_apell, b.txt_seg_apell ".
              "FROM zona_oferta_postulado AS a LEFT JOIN zona_personas AS b ON a.id_persona = b.id ".
             "WHERE a.id_oferta = '".$mIdOferta."' ";
    
    $mResoults = DbHandler::GetAll( $mSql );


    $mHtml = '<h2 class="tit_emergente">Lista de postulados</h2>';
