...
$db = new Database();
$db->connect();
$db->doQuery("SELECT * FROM cotizaciones WHERE id=" . $_POST["user"], SELECT_QUERY);
$resultado = $db->results[0];
$id = explode("-", $_POST["idf"]);
$cant = explode("-", $_POST["cantidadf"]);
$iva = explode("-", $_POST["ivaf"]);
$precio = explode("-", $_POST["valoruf"]);
...
$condiciones = explode("%", $_POST["cond"]);
...
$content .= ' <link href="css/norquimicos.css" rel="stylesheet" type="text/css" />
                ...
                <!--Cot. No-->
                <div class="campo-3" style="font-size: 9px;margin-top:15px;color:#666;margin-left:100px;">' . $_POST["user"] . '</div>
                <!--Señores-->
...
for ($bandera = 0, $pro = count($id); $bandera < $pro; $bandera++) {
    ...
    if (isset($id[$bandera])) {
        ...
        $content .='<tr>
                ...
                <td><div class="ct30" style="font-size: 9px;">' . $presentacion . '</div></td>
...
$content .='              </table>
                ...
                <!--Subtotal-->
                <div class="campo-15" style="font-size: 9px;margin-top:8px;margin-left:10px;color:#666;" id="subtotje">' . $_POST["subenviototal1"] . '</div>
                <!--IVA-->
                <div class="campo-16" style="font-size: 9px;margin-top:8px;margin-left:10px;color:#666;" id="ivaje">' . $_POST["ivatotal1"] . '</div>
                <!--Total-->
                <div class="campo-17" style="font-size: 9px;margin-top:8px;margin-left:10px;color:#666;" id="campsfinals">' . $_POST["totaltotal1"] . '</div>
              </div> 
                    </div>
                </div>
';

$content = str_replace("<0,3 ", "\"<0,3\"", $content);
$content = str_replace("<20 ", "\"<20\"", $content);
//echo $content;
//
//exit();
...
//echo 1;
$db->doQuery("UPDATE cotizaciones SET  estado ='completo' WHERE id=" . $_POST["user"], 4);
echo "SELECT * FROM cotizaciones WHERE id=" . $_POST["user"];
exit;
$db->doQuery("SELECT * FROM cotizaciones WHERE id=" . $_POST["user"], SELECT_QUERY);
$resultado = $db->results[0];
//para el cotizante
$body = '
           ...
           Tu solicitud de cotizaci&oacute;n ha sido realizada correctamente<br>
            Para ver y descargar la cotizaci&oacute;n haga click aqu&iacute;: <a href="http://www.norquimicos.com.co/imagenes/pdfs/PDFUSER' . $_POST["user"] . '.pdf">PDF</a><br><br>            
           ...
           ';
$asunto = utf8_decode("Solicitud de cotizacion");
$cCorreo = new Correo();
...
//para el cotizador
$db->doQuery("SELECT * FROM user WHERE id=" . $_POST["asesor"], SELECT_QUERY);
$je = $db->results[0];
$body1 = '
