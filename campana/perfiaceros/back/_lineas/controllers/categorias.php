...
class categorias extends Back_Controller {
    ...
    public function datos_ajax() {
        ...
        $this->db->join('cms_linea', 'cms_linea.id  = cms_categoria.cms_linea_id');
                ...
                $salida .="  <tr class='odd gradeX parent-delete'>
                                          <td>
                                            ...
                                            <td class='center' width='100px'>
                                                <span class='tip'>
                                                    <a href='".cms_url('lineas/categorias/delete')."' class='uibutton special' data-action='special-delete' data-value='".$obj->id."'>Eliminar</a>
                                                 <a id='edit_".$obj->id."' href='".cms_url('lineas/categorias/edit/')."' data-value='".$obj->id."' class='uibutton' >Editar</a>
                                    ...
                                    $.ajax({
                                        ...
                                        success: function(json) {
                                            if (json.ok) {
                                                ...
                                                $('#cms_linea_id').val(json.cms_linea_id);
                                                $('#img').val(json.img);
...
";
           ...
           if($object->exists()){
              ...
              $fmedia->get_by_id($object->cms_media_id);
               ...
               $datos = array(
                  ...
                  "cms_linea_id" => $object->cms_linea_id,
    ...
    public function add() {
        ...
        $this->form_validation->set_rules('cms_linea_id', 'Linea', 'required');
        if (!$this->form_validation->run()) {
          ...
          if ($this->_post('img') != "-1") {
            ...
            if ($img->save()) {
                ...
                $obj->cms_linea_id = $this->_post('cms_linea_id');
                $obj->cms_media_id = $img->id;
                if (!$obj->save()) {
                    $img->delete();
                    @unlink($this->dirImg . trim($this->_post('img')));
                    $error = true;
                    $this->set_message("Error Ingresando categoria...!, favor intente mas tarde...!", 'error');
                }
            } else {
                        $this->set_message("Error Ingresando Imagen de categoria..!, favor intente mas tarde...!", 'error');
                        $error = true;
                        @unlink($this->dirImg . trim($this->_post('img')));
            }
        }else{
        ...
        if (!$error)
            $this->set_message("Categoria registrada o actualizada con exito...!", 'info');
    ...
    public function index() {
        ...
        return $this->build('categorias_/body', array('error' => ' '));
    }
