...
$(document).ready(function(){	
    ...
    $('.regexonly input').autotab_magic().autotab_filter({
        format: 'custom', 
        pattern: '[^0-9\.]'
    });
    $('.alluppercase input').autotab_magic().autotab_filter({
        format: 'alphanumeric', 
        uppercase: true
    });
...
$(function() {		
    ...
    $(document).on('click', '.logout', function(){
        loading('Cerrando sesión de forma segura...', 1);
...
var camera = $('#camera'),screen =  $('#screen');	
    ...
    $("#picThumb").sortable({
        opacity: 0.6,
        handle : '.move', 
        connectWith: '.picThumbUpload', 
        items: '.picThumbUpload'
    });
    $("#main_menu").sortable({
        opacity: 0.6,
        connectWith: '.limenu',
    ...
    $(".preOrder").iphoneStyle({  //  Custom Label 
        checkedLabel: "Administrador",
        uncheckedLabel: "Cliente",
        labelWidth:'76px'
    }); 
     $(".YesOrNo").iphoneStyle({  //  Custom Label 
        checkedLabel: "SI",
        uncheckedLabel: "NO",
        labelWidth:'20px',
        onChange: function() {
            var chek=$(".YesOrNo").attr('checked');
            if(chek){
                 $("#content_yes_or_no").fadeIn();
            }else{
                 $("#content_yes_or_no").fadeOut();
            }
        }
    }); 
    ...
    $('#albumsList,.albumpics,.overflow,.todate').jScrollPane({
...
// Textarea limit
$('#Textarealimit_732').limit('732','.limitchars_732');
$('#Textarealimit_86').limit('86','.limitchars_86');
$('#Textarealimit_1070').limit('1070','.limitchars_1070');
$('#Textarealimit_500').limit('500','.limitchars_500');

$('#Textarealimit_1050_1').limit('1050','.limitchars_1050_1');
$('#Textarealimit_1050_2').limit('1050','.limitchars_1050_2');
$('#Textarealimit_1050_3').limit('1050','.limitchars_1050_3');
$('#Textarealimit_1050_4').limit('1050','.limitchars_1050_4');

$('#Textarealimit_190').limit('190','.limitchars_190');
$('#Textarealimit_430').limit('430','.limitchars_430');
$('#Textarealimit_50').limit('50','.limitchars_50');

$('#Textarealimit_2000').limit('2000','.limitchars_2000');
...
$(".Delete").live('click',function() {
    ...
    var id = $(this).attr("id");
    var name = $(this).attr("name");
    var tableToDelete = $(this).attr("tableToDelete");
    var nameField = $(this).attr("nameField");
    var docs = $(this).attr("docs");		  		  
    var data = id;
    Delete(data,name,row,0,dataSet,tableToDelete,nameField);
});
	
function Delete(data,name,row,type,dataSet,tableToDelete,nameField){
    var loadpage = dataSet.hdata(0);
    var url = dataSet.hdata(1);
    var table = dataSet.hdata(2);
    //var tableToDelete = dataSet.hdata(5);
    var data = data;
    $.confirm({
        'title': 'Confirme',
        'message': "<strong>Â¿Realmente desea borrar este Ã­tem?</strong>",
        'buttons': {
            'SI': {
                'class': 'special',
                'action': function(){
                    ...
                    row.slideUp(function(){
                        showSuccess('Item eliminado correctamente!',5000);
                        unloading();
                    });
                    return false;
                    //if(type==1){ row.slideUp(function(){   showSuccess('Item eliminado correctamente!',5000); unloading(); }); return false;}
                    //showSuccess('MenÃº eliminado correctamente!',5000);
