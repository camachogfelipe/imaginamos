  ...
  $(".cont_abrirsecciones_trabaje").each(function() {
    ...
    $(this).children(".abrirsecciones_trabaje").click(function(event){
       ...
       console.log(ancho_ventana);
			 var form = jQuery(this).attr("data-rel");
			 jQuery.ajax({
				 url  :   'trabaja/vacante',
				 type :   'POST',
				 data :   {
				 ...
				 success:function(data){
					 jQuery(".form"+form).children(".cont_tittrabaje").children("#vacante").html(data.nombre);
					 jQuery(".form"+form).children(".cont_tittrabaje").children("#subtitulo").html(data.subtitulo);
           var datos = "<div style='width:96%'>"+data.detalle; 
           datos = datos+"</div>";           
					 jQuery(".form"+form).children("#detalle").children('div').children('div').html("<p>"+datos+"</p>");
					 var d = event.timeStamp;
					 jQuery(".form"+form).children(".img_detalletrabaje").children("#imagen").attr("src", data.imagen+"?"+d);
					 jQuery(".form"+form).children("#postularme_trabaje").children(".cont_tittrabaje").children("#postularme_titulo").html(data.nombre);
					 jQuery("#vancante_id"+form).val(data.id);
					 /* modificación link redes sociales */
					 // TWITTER: //twitter.com/home?status=titulo - link
					 jQuery("#vc_twitter").attr("data-link", "//twitter.com/home?status="+data.nombre+" - "+window.location.href+"#"+data.id);
					 // FACEBOOK: https://www.facebook.com/sharer/sharer.php?s=100&p[title]=TITULO&p[summary]=SUBTITULO&p[url]=URL
					 jQuery("#vc_facebook").attr("data-link", 'http://www.facebook.com/sharer.php?s=100&p[title]='+encodeURI(data.nombre)+'&p[summary]='+encodeURI(data.subtitulo)+'&p[url]='+encodeURI(window.location.href)+'&p[images][0]='+encodeURI(BASE_URL+data.imagen));
           //www.facebook.com/sharer.php?s=100"+"&p[title]="+data.nombre+"&p[summary]="+data.subtitulo+"&p[url]="+window.location.href+"#"+data.id);
					 // GOOGLE PLUS: //plus.google.com/share?url=link
					 jQuery("#vc_google").attr("data-link", "//plus.google.com/share?url="+window.location.href+"#"+data.id+"-"+data.nombre);
					 // LINKEDIN: //www.linkedin.com/cws/share?url=link
					 jQuery("#vc_linkedin").attr("data-link", "//www.linkedin.com/cws/share?url="+window.location.href+"#"+data.id+"-"+data.nombre);
					 /* modificación link redes sociales */
				 }
			 });
...
$(".bt_postularme").click(function(event) {
     ...
     $(".cerrar_formtrabaje").click(function(event) {
       event.preventDefault();
       $(this).parent(".cont_formpostularse").css({'height':'0','overflow':'hidden', 'opacity':'0'}); 
