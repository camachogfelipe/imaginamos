...
public function AdminOnline()
{
	...
	try {
		...
		$sth = DB::prep("SELECT COUNT(*) as c FROM cms_messaging_admin WHERE time > (NOW() - INTERVAL 30 SECOND)");
...
public function UserProperties()
{
    try {
        ...
        $sth = DB::prep("SELECT upload FROM cms_messaging_users WHERE user_id = :user");
