...
class _modelos extends Back_Controller {
    ...
    public function menu() {
        return $this->current_menu['Home']['Tipos de Vehiculos']['Modelos'] = cms_url('modelos');
    }
    ...
    public function form_edit() {
        $obj = new $this->model();
        $id = $this->_post('id');
        $obj->join_related('marca')->get_by_id($id);
        $data['form_content'] = "";
        $data['form_content'] .= $this->inputHidden($obj->id, "id", $obj->get_rule("id", "max_length"));
        $data['form_content'] .= $this->input($obj->nombre, "nombre", $obj->get_rule("nombre", "max_length"), "Modelo", "Maximo " . $obj->get_rule("nombre", "max_length") . " caracteres", $obj->is_rule("nombre", "required"), "span3");
        $var_marca_id = new modelo();
        $data['form_content'] .= $this->combox($obj->marca_id, $var_marca_id->get_marca_list('nombre'), "marca_id", "Marca", "", $obj->is_rule("marca_id", "required"), "span3");
