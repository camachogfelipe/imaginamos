...
class search extends Front_Controller {
    ...
    public function search_data() {
        ...
        foreach ($categorias as $cat) {
            ...
            if (($needlePos !== false))
                $datos[] = array('titulo' => $cat->titulo, 'contents' => strip_tags(substr($cat->titulo, $needlePos, 1000)), 'url' => base_url() . "nuestros_productos/productos_catgoria/" . $cat->id);
        }
        ...
        foreach ($pro as $p) {
           $needlePos = array(); 
            ...
            foreach ($needlePos as $bool) {
                if($bool !== false){
                      $datos[] = array('titulo' => $p->nombre, 'contents' => strip_tags(substr($p->descripcion, $bool, 1000)), 'url' => base_url() . "detalle_producto/info/" . $p->id);
                    break;   
                }
            }
        }
