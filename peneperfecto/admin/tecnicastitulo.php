...
$tecnicastitulos = $tecnicastituloDAO->gets();
$total = $tecnicastituloDAO->total();
...
<input id="filterBoxOne" value="" maxlength="30" size="30" type="text" />
    ...
    <?php if( isset($_GET['ok']) ){ ?>
        <tr>
            <td class="EstiloGreen">tecnicastitulo eliminado</td>
        </tr>
    <?php }?>
    <?php if( isset($_GET['error']) ){ ?>
        <tr>
            <td class="EstiloRed">Error al eliminar tecnicastitulo</td>
        </tr>
    <?php }?>
...
<tbody>
  <?php if($total>0): ?>
    <?php foreach ($tecnicastitulos as $tecnicastitulo): ?>
      <tr>

        <td width="" align="center"><?=$tecnicastitulo->gettitulo() ?></td>
        <td width="20%" align="center">
          <a href="menuAdmin.php?s=tecnicastituloVer&a=<?=$tecnicastitulo->getid() ?>" title="Ver" >Ver</a>
           | 
          <a href="menuAdmin.php?s=tecnicastituloEdit&a=<?=$tecnicastitulo->getid() ?>" title="Modificar" >Modificar</a>
   </td>
      </tr>
    <?php endforeach; ?>
  <?php endif; ?>
