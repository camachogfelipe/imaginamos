...
$encuestas = $encuestaDAO->gets();
$total = $encuestaDAO->total();
...
<input id="filterBoxOne" value="" maxlength="30" size="30" type="text" />
    ...
    <?php if( isset($_GET['ok']) ){ ?>
        <tr>
            <td class="EstiloGreen">encuesta eliminado</td>
        </tr>
    <?php }?>
    <?php if( isset($_GET['error']) ){ ?>
        <tr>
            <td class="EstiloRed">Error al eliminar encuesta</td>
        </tr>
    <?php }?>
...
<tbody>
  <?php if($total>0): ?>
    <?php foreach ($encuestas as $encuesta): ?>
      <tr>
        <td width="" align="center"><?=$encuesta->getpregunta() ?></td>
        <td width="20%" align="center">
          <a href="menuAdmin.php?s=encuestaVer&a=<?=$encuesta->getid() ?>" title="Ver" >Ver</a>
           | 
          <a href="menuAdmin.php?s=encuestaEdit&a=<?=$encuesta->getid() ?>" title="Modificar" >Modificar</a>
        </td>
      </tr>
    <?php endforeach; ?>
  <?php endif; ?>
