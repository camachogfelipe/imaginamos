...
<div class="subcontenido2">
    <table border="0" align="center" width="400" cellspacing="4">
        ...
        <?php if( isset($_GET['ok']) ){ ?>
            <tr>
                <td class="EstiloGreen" colspan="2">PREGUNTAS, agregado correctamente</td>
            </tr>
        <?php }?>
        <?php if( isset($_GET['ok2']) ){ ?>
            <tr>
                <td class="EstiloGreen" colspan="2">PREGUNTAS, modificado correctamente</td>
            </tr>
        <?php }?>
    </table>
...
<form action="./../php/action/preguntasEdit.php" method="post" enctype="multipart/form-data">
<table width="500">

<tr>
    <td>
        <span class="Estilo3">Pregunta:</span><br />
        <input type="text" name="pregunta" value="<?=$preguntas->getpregunta() ?>" size="40">
    </td>
</tr>
<tr>
    <td>
        <span class="Estilo3">Respuesta:</span><br />
        <?php
            $oFCKeditor_l = new FCKeditor('respuesta') ;
            $oFCKeditor_l->BasePath = 'fckeditor/';
            $oFCKeditor_l->Width  = '600' ;
            $oFCKeditor_l->Height = '400' ;
            $oFCKeditor_l->Value = ''.$preguntas->getrespuesta();
            $oFCKeditor_l->Create() ;
        ?>
