...
$testimonioss = $testimoniosDAO->gets();
...
<td width="16%" class="tableHeader">
    ...
    <a href="menuAdmin.php?s=testimoniosAdd">A&ntilde;adir testimonios [+]</a>
...
<input id="filterBoxOne" value="" maxlength="30" size="30" type="text" />
    ...
    <?php if( isset($_GET['ok']) ){ ?>
        <tr>
            <td class="EstiloGreen">testimonios eliminado</td>
        </tr>
    <?php }?>
    <?php if( isset($_GET['error']) ){ ?>
        <tr>
            <td class="EstiloRed">Error al eliminar testimonios</td>
        </tr>
    <?php }?>
...
<tbody>
  <?php if($total>0): ?>
    <?php foreach ($testimonioss as $testimonios): ?>
      <tr>
        <td width="" align="center"><?=$testimonios->gettitulo() ?></td>
        <td width="" align="center"><img src="./..<?=$testimonios->getimagen() ?>" width="250" height="150" /></td>
        <td width="" align="center"><?=$testimonios->getfecha() ?></td>
        <td width="20%" align="center">
          <a href="menuAdmin.php?s=testimoniosVer&a=<?=$testimonios->getid() ?>" title="Ver" >Ver</a>
           | 
          <a href="menuAdmin.php?s=testimoniosEdit&a=<?=$testimonios->getid() ?>" title="Modificar" >Modificar</a>
           | 
          <a href="./../php/action/testimoniosDelete.php?id=<?=$testimonios->getid() ?>" onClick="return confirm('Desea eliminar este item?')" title="Eliminar">Eliminar</a>
        </td>
      </tr>
    <?php endforeach; ?>
  <?php endif; ?>
