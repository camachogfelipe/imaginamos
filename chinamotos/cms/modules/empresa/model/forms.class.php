...
function printFormEmpresa($funcionality)
		{	
			switch($funcionality)
			{
			case 1:
				$query = "SELECT * FROM cms_empresa ORDER BY empresa_id DESC";
				...
				';	
					...
					foreach ($results as $result)
					{
						$html .= '
							...
							<fieldset>
								<div>'.$result[empresa_titulo].'</div>
							</fieldset>
							<p>&nbsp;</p>
							
							<label>Contenido:</label>
							<fieldset>
								<div>'.nl2br($result[empresa_contenido]).'</div>
							</fieldset>
							<p>&nbsp;</p>

							<label>Imagen:</label>
							<fieldset>
								<div><img src="../files/big/'.$result[empresa_imagen].'"></div>
							</fieldset>
							<p>&nbsp;</p>	
							
							<fieldset>
								<div><a  href="edit.php?id='.$result[empresa_id].'&funcionality=1" class="uibutton">Editar</a></div>
							</fieldset>
							<p>&nbsp;</p>
						';
					}
...
function printFormEdit($id,$funcionality)
		...
		$query = "SELECT * FROM cms_empresa WHERE empresa_id = '$id'";
		$this->db->doQuery($query,SELECT_QUERY);
		$results = $this->db->results;
		...
		$html = '
				...
				<div class="imu_info" id="info"></div>
					...
					<br /><a class="uibutton icon special answer" href="javascript:history.back();">Volver</a>
						...
						<p>&nbsp;</p>
                        <div>
						<label>T&iacute;tulo:  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;*Obligatorio (m&aacute;ximo 25 caracteres)</label>
						<div>
							<input type="text" name="titulo" id="titulo"  maxlength="25"  value="'.$results[0][empresa_titulo].'" class="large" />
						</div>
						</div>
						...
						<div><textarea name="contenido" id="contenido" style="resize: none;" cols="5" rows="10" class="large">'.$results[0][empresa_contenido].'</textarea></div>
						</div>
						...
						<div>
						<label>Imagen:  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;*Obligatorio (tama&ntilde;o: 410px de ancho x 410px de alto, formatos: jpg,jpeg &oacute; png)</label>
						<div>
						
						<fieldset>
						<div>
						<div>
							<img src="../files/big/'.$results[0][empresa_imagen].'">
							<p>&nbsp;</p>
							<input class="CMS" type="file" path="files/" multi="false" startOn="onSubmit:form" ajax="true" ajaxInfoId="info" ajaxLoaderId="loader" button="../images/buttonSingle.jpg" thumbnails="82x82,205x205,410x410" thumbnailsFolders="files/small/,files/middle/,files/big/" fileExt="jpg,jpeg,png" fileDesc="Images (*.jpg, *.jpeg, *.png)" allowRemove="true" />
