...
$htmlFormconformato2 = new Forms($db);
?>
			...
			<div id="shadowhead"></div>
              ...
              <div id="content">
                <div class="inner">
					...
					<!-- full width -->
                    <div class="widget" >
                        <div class="header"><span ><span class="ico gray pictures_folder"></span>SECCI&Oacute;N canales para presentar peticiones, quejas o reclamos</span>
					...
					$db->doQuery("SHOW TABLES LIKE 'cms_conformato2'",SHOW_TABLE_QUERY);
					//Si recibimos TRUE como respuesta quiere decir que si existe la tabla
					if($show = $db->show)
						{
						$db->doQuery("SELECT conformato2_config_funcionality FROM cms_conformato2_config WHERE conformato2_config_id = 1",SELECT_QUERY);					
						$result = $db->results;
						//Imprimimos el formulario que necesitamos
						echo $htmlFormconformato2->printFormE403($result[0][conformato2_config_funcionality]);
						}
					else
						//Si la tabla no existe presentamos el formulario de configuración del módulo "NEWS"
						echo $htmlFormconformato2->printFormInstall();
					?>
                    </fieldset>
