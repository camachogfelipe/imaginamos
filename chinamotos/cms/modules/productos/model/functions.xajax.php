...
function editNov( $formData )
{  
	$objResponse = new xajaxResponse();
	
	$texto1 = $formData[cat];
	$texto2 = $formData[producto];
	...
	if($texto2 == 0)
		...
		$objResponse->script("showError('Debe seleccionar un producto',3000);");
		...
		$objResponse->script("setInterval('window.location.href = \"../view/indexNov.php\"', 2000);");
	}
...
{  
	...
	$texto1 = $formData;
		...
		$db->doQuery("UPDATE cms_vendemos_prod SET vendemos_prod_nov = '0' WHERE vendemos_prod_id = '$texto1'",UPDATE_QUERY);
		
		$objResponse->script("loading('Procesando',1);");
		$objResponse->script("setTimeout(\"unloading()\", 2000 );");
		$objResponse->script("showSuccess('Se elimino correctamente',3000);");
		$objResponse->script("setInterval('window.location.href = \"../view/indexNov.php\"', 2000);");
...
{  
	...
	if($texto2 == 0)
		...
		$db->doQuery("UPDATE cms_vendemos_prod SET vendemos_prod_prom = '1' WHERE vendemos_prod_id = '$texto2'",UPDATE_QUERY);
		
		$objResponse->script("loading('Procesando',1);");
		$objResponse->script("setTimeout(\"unloading()\", 2000 );");
		$objResponse->script("showSuccess('Carga correcta',3000);");
		$objResponse->script("setInterval('window.location.href = \"../view/indexProm.php\"', 2000);");
	}
...
{  
	...
	$texto1 = $formData;
		...
		$db->doQuery("UPDATE cms_vendemos_prod SET vendemos_prod_prom = '0' WHERE vendemos_prod_id = '$texto1'",UPDATE_QUERY);
...
$xajax->registerFunction("editNov");
$xajax->registerFunction("elimNov");
$xajax->registerFunction("editProm");
$xajax->registerFunction("elimProm");
