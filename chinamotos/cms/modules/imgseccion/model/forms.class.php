...
/**/
//IMPRIME FORMULARIO DE imgseccion
/**/

function printFormimgseccion($funcionality)
		{	
			switch($funcionality)
			{
			case 1:
				$query = "SELECT * FROM cms_imgseccion ORDER BY imgseccion_id DESC";
				...
				foreach ($results as $result)
				{
					$html .= '
						<tr class="odd gradeX">
						<td class="center" width="350px"><img src="../files/small/'.$result[imgseccion_image].'"></td>
						<td class="center">'.$result[imgseccion_titulo].'</td>
						<td class="center" width="100px">
						<a  href="edit.php?id='.$result[imgseccion_id].'&funcionality='.$conta.'" class="uibutton">Editar</a>
						</td>
						</tr>';		
					$conta++;	
				}
...
function printFormEdit($id,$funcionality)
		...
		$query = "SELECT * FROM cms_imgseccion WHERE imgseccion_id = '$id'";
		$this->db->doQuery($query,SELECT_QUERY);
		$results = $this->db->results;
		...
		$html = '
				...
				<div class="imu_info" id="info"></div>
                    ...
                    <legend><h1>Edición de Imagen en Secci&oacute;n</h1></legend>
					...
					<br /><a class="uibutton icon special answer" href="javascript:history.back();">Volver</a>
						...
						<label>Titulo</label>
						<div>	
							<input type="text" name="titulo" id="titulo"  value="'.$results[0][imgseccion_titulo].'" class="large" />
						</div>
						</div>
						
						<p>&nbsp;</p>
						
						<label>Imagen dimensiones 1400px ancho x 263px alto </label>
						<fieldset>
						<div>
						<div>
						
							<img src="../files/middle/'.$results[0][imgseccion_image].'"><br><br>
							<input class="CMS" type="file" path="files/" multi="false" startOn="onSubmit:form" ajax="true" ajaxInfoId="info" ajaxLoaderId="loader" button="../images/buttonSingle.jpg" thumbnails="350x66,980x184,1400x263" thumbnailsFolders="files/small/,files/middle/,files/big/" fileExt="jpg,jpeg,png,gif" fileDesc="Images (*.jpg, *.jpeg, *.png, *.gif)" allowRemove="true" />
