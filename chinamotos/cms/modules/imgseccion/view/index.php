...
$htmlFormimgseccion = new Forms($db);
?>
			...
			<div id="shadowhead"></div>
              ...
              <div id="content">
                <div class="inner">
					...
					<!-- full width -->
                    <div class="widget" >
                        <div class="header"><span ><span class="ico gray pictures_folder"></span>Imagen en Secci&oacute;n </span>
					...
					$db->doQuery("SHOW TABLES LIKE 'cms_imgseccion'",SHOW_TABLE_QUERY);
					//Si recibimos TRUE como respuesta quiere decir que si existe la tabla
					if($show = $db->show)
						{
						$db->doQuery("SELECT imgseccion_config_funcionality FROM cms_imgseccion_config WHERE imgseccion_config_id = 1",SELECT_QUERY);					
						$result = $db->results;
						//Imprimimos el formulario que necesitamos
						echo $htmlFormimgseccion->printFormimgseccion($result[0][imgseccion_config_funcionality]);
						}
					else
						//Si la tabla no existe presentamos el formulario de configuración del módulo "NEWS"
						echo $htmlFormimgseccion->printFormInstall();
					?>
                    </fieldset>
