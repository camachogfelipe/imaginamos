...
class Clientes extends MX_Controller {
  ...
  function __construct() {
    parent::__construct();
    $this->gallery_path = realpath(APPPATH . "../application/modules/clientes/assets/images");
  ...
  function index($return = '') {
    ...
    if ($query = $this->clientes_model->getRecords()) {
      $template_date['records'] = $query;
      $template_date['num'] = $this->db->query('select * from clientes')->num_rows();
    } else {
    ...
    $this->load->view('clientes_view', $template_date);
  }
  ...
  function edit($return = '') {
    ...
    if ($query = $this->clientes_model->getRecord()) {
    ...
    $this->load->view('clientes_edit', $template_date);
  }
  ...
  function add($return = '') {
    ...
    $this->load->view('clientes_new', $template_date);
  }
    ...
    if ($is_logged === TRUE) {
      ...
      $this->load->model('clientes_model');
        ...
        if ($this->upload->do_upload()) {
          ...
          $clientes['imagen'] = $name;
        }

        $return = $this->clientes_model->update_record($this->input->post('id'), $clientes);
        if ($return) {
          $this->index('ok');
        } else {
          $this->add('erSizeImg');
  ...
  function new_record() {   
      ...
      $result = $this->clientes_model->add_record($clientes);
  ...
  function delete() {
    ...
    $this->load->model('clientes_model');
    $this->clientes_model->delete_record();
    $this->index('ok');
  }
