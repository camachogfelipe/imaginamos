...
class Noticias extends MX_Controller {
  ...
  function __construct() {
    parent::__construct();
    $this->gallery_path = realpath(APPPATH . "../application/modules/noticias/assets/images");
  ...
  function index($return = '') {
    ...
    if ($query = $this->noticias_model->getRecords()) {
    ...
    $this->load->view('noticias_view', $template_date);
  }
  ...
  function edit($return = '') {
    ...
    if ($query = $this->noticias_model->getRecord()) {
    ...
    $this->load->view('noticias_edit', $template_date);
  }
  ...
  function add($return = '') {
    ...
    $this->load->view('noticias_new', $template_date);
  }
  ...
  function update() {
    ...
    if ($is_logged === TRUE) {
      ...
      if ($this->input->post('titulo') == '' || $this->input->post('texto') == '') {
        ...
        if($tipo == 1){
            $noticias['link'] = $this->input->post('link'); 
        }
        ...
        if ($this->upload->do_upload()) {
          $data = array('upload_data' => $this->upload->data());
          $name = $data['upload_data']['file_name'];
          $noticias['imagen'] = $name;
        }
        

        $return = $this->noticias_model->update_record($this->input->post('id'), $noticias);
        if ($return) {
          $this->index('ok');
        } else {
          $this->add('erSizeImg');
  ...
  function new_record() {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $result = $this->noticias_model->add_record($noticias);
  ...
  function delete() {
    ...
    $this->load->model('noticias_model');
    $this->noticias_model->delete_record();
    $this->index('ok');
  }
}
