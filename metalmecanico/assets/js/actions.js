...
function onclosefade(r)
{
	...
	for (var i = 0; i < q.length; i++) {
		$('#'+q[i]).animate({width:333},800);
		$('#'+q[i]).find('img').animate({width:320,height:100},800);
	}
}
function reduccion(a)
{
	...
	for (var i = 1; i < q.length+1; i++) {
		if( i > a.id)
		{
			...
			$('#'+i).animate({width: 333-dif-20+"px",}, 800 );

			//animate image
			var my_img = $('#'+i).find('img');
			$(my_img).animate({width: 320-dif+"px", height: 100-dif+"px"},800);
...
function ampliacion(t)
{
	...
	var dif = (sizenueva - sizeactual)/2;
	$(t).animate({width: 333+dif+"px",}, 800 );

	//animate image
	var my_img = $(t).find('img');
	$(my_img).animate({width: 320+dif-10/2+"px", height: 100+dif/2+"px"},800);
