...
class Ion_auth
{
        ...
        if ($this->ion_auth_model->forgotten_password($identity)) {   //changed
            ...
            if ($user) {
                $data = array(
                    ...
                    'link' => site_url('usuarios/reset_password/'.$user->forgotten_password_code),
                ...
                if (!$this->config->item('use_ci_email', 'ion_auth')) {
                    ...
                    $message = $html_email = $this->parser->parse('emails/recuperar_contrasena', $data, false);
                    $this->email->clear();
                    $this->email->from(ADMINEMAIL, $this->config->item('site_title', 'ion_auth'));
     ...
     * @author Ben Edmunds
     * */
    public function is_creativo($id = false)
    {
        $this->ion_auth_model->trigger_events('is_creativo');
     ...
     * @author Ben Edmunds
     * */
    public function is_empresa($id = false)
    {
        $this->ion_auth_model->trigger_events('is_empresa');
        return $this->in_group('empresas', $id);
    }
