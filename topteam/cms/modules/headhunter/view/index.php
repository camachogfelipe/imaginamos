...
$htmlFormheadhunter = new Forms($db);
?>
			...
			<div id="shadowhead"></div>
              ...
              <div id="content">
                <div class="inner">
					...
					<!-- full width -->
                    <div class="widget" >
                        <div class="header"><span ><span class="ico gray pictures_folder"></span>SECCION HEADHUNTER</span>
					...
					$db->doQuery("SHOW TABLES LIKE 'cms_headhunter'",SHOW_TABLE_QUERY);
					//Si recibimos TRUE como respuesta quiere decir que si existe la tabla
					if($show = $db->show)
						{
						$db->doQuery("SELECT headhunter_config_funcionality FROM cms_headhunter_config WHERE headhunter_config_id = 1",SELECT_QUERY);					
						$result = $db->results;
						//Imprimimos el formulario que necesitamos
						echo $htmlFormheadhunter->printFormheadhunter($result[0][headhunter_config_funcionality]);
						}
					else
						//Si la tabla no existe presentamos el formulario de configuración del módulo "NEWS"
						echo $htmlFormheadhunter->printFormInstall();
					?>
                    </fieldset>
