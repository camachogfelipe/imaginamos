jQuery(document).ready(function($){
	...
	function updateUploadifySettings1(uploader)
	{
	...
	function submitForm1(event)
	{
		...
		if(queue.children().length>0)
		{
			...
			$(this).bind("submit", preventSubmit1);
			...
			if(typeof(self.attr("ajax"))!="undefined" && self.attr("ajax")=="true")
			{
				...
				$.ajax({
					...
					success: function(data){
						...
						form.one("submit", {'uploader':self}, submitForm1);
					}
				});
			}
			else
	...
	$(".IMGR1").each(function(index){
		var self = $(this);
		self.attr("id", "IMGR1"+index);
		if(typeof(self.attr("startOn"))!="undefined")
		{
			if(self.attr("startOn")=="manually")
			{
				self.after("<input type='button' value='" + (typeof(self.attr("buttonCaption"))=="undefined" ? '':self.attr("buttonCaption")) + "' id='IMGR1"+index+"start' class='uploadButton' />");
				$("#IMGR1"+index+"start").click(function(){
					updateUploadifySettings1(self);
					self.uploadifyUpload();					
				});
			}
			else if(self.attr("startOn").substr(0, 8)=="onSubmit")
				$("#"+self.attr("startOn").substr(9)).one("submit", {'uploader':self}, submitForm1);
		}
		var fileExt = "";
		...
		self.uploadify({
			...
			'onComplete' : function(event, queueId, fileObj, response, data){
				...
				else
				{
					if(self.attr("startOn").substr(0, 8)=="onSubmit")
						$("#"+self.attr("startOn").substr(9)).append("<input type='hidden' name='IMGR1Files[]' value='"+responseJSON.filename.replace("'", "%27")+"' />");
			...
			'onAllComplete' : function(event, data){
				if(typeof(self.attr("afterUpload"))!="undefined" && self.attr("afterUpload")!="link" && self.attr("afterUpload")!="filename" && self.attr("afterUpload")!="image" && self.attr("afterUpload")!="none")
				{
					...
					message = message.replace("IMGR1filesUploaded", data.filesUploaded);
					message = message.replace("IMGR1errors", data.errors);
					message = message.replace("IMGR1totalSizeB", data.allBytesLoaded);
					message = message.replace("IMGR1totalSizeKB", roundNumber1(data.allBytesLoaded/1024, 2));
					message = message.replace("IMGR1totalSizeMB", roundNumber1(data.allBytesLoaded/1024/1024, 2));
					message = message.replace("IMGR1totalSizeGB", roundNumber1(data.allBytesLoaded/1024/1024/1024, 2));
					message = message.replace("IMGR1speedB", roundNumber1(Math.round(data.speed*1024), 2));
					message = message.replace("IMGR1speedKB", roundNumber1(Math.round(data.speed), 2));
					message = message.replace("IMGR1speedMB", roundNumber1(Math.round(data.speed/1024), 2));
					message = message.replace("IMGR1speedGB", roundNumber1(Math.round(data.speed/1024/1024), 2));
					$("#" + self.attr("id") + "Queue").html(message);
				}
				if(self.attr("startOn").substr(0, 8)=="onSubmit")
				{
					var form = $("#"+self.attr("startOn").substr(9));
					form.unbind("submit", preventSubmit1);
					if(ajax=="true")
					{
