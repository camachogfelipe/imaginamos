jQuery(document).ready(function($){
	...
	$(".IMGR3").each(function(index){
		var self = $(this);
		self.attr("id", "IMGR3"+index);
		if(typeof(self.attr("startOn"))!="undefined")
		{
			if(self.attr("startOn")=="manually")
			{
				self.after("<input type='button' value='" + (typeof(self.attr("buttonCaption"))=="undefined" ? '':self.attr("buttonCaption")) + "' id='IMGR3"+index+"start' class='uploadButton' />");
				$("#IMGR3"+index+"start").click(function(){
					updateUploadifySettings(self);
					self.uploadifyUpload();					
				});
			}
		...
		self.uploadify({
			...
			'onComplete' : function(event, queueId, fileObj, response, data){
				...
				else
				{
					if(self.attr("startOn").substr(0, 8)=="onSubmit")
						$("#"+self.attr("startOn").substr(9)).append("<input type='hidden' name='IMGR3Files[]' value='"+responseJSON.filename.replace("'", "%27")+"' />");
			...
			'onAllComplete' : function(event, data){
				if(typeof(self.attr("afterUpload"))!="undefined" && self.attr("afterUpload")!="link" && self.attr("afterUpload")!="filename" && self.attr("afterUpload")!="image" && self.attr("afterUpload")!="none")
				{
					...
					message = message.replace("IMGR3filesUploaded", data.filesUploaded);
					message = message.replace("IMGR3errors", data.errors);
					message = message.replace("IMGR3totalSizeB", data.allBytesLoaded);
					message = message.replace("IMGR3totalSizeKB", roundNumber(data.allBytesLoaded/1024, 2));
					message = message.replace("IMGR3totalSizeMB", roundNumber(data.allBytesLoaded/1024/1024, 2));
					message = message.replace("IMGR3totalSizeGB", roundNumber(data.allBytesLoaded/1024/1024/1024, 2));
					message = message.replace("IMGR3speedB", roundNumber(Math.round(data.speed*1024), 2));
					message = message.replace("IMGR3speedKB", roundNumber(Math.round(data.speed), 2));
					message = message.replace("IMGR3speedMB", roundNumber(Math.round(data.speed/1024), 2));
					message = message.replace("IMGR3speedGB", roundNumber(Math.round(data.speed/1024/1024), 2));
