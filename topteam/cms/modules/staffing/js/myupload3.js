jQuery(document).ready(function($){
	...
	function updateUploadifySettings2(uploader)
	{
	...
	function submitForm2(event)
	{
		...
		if(queue.children().length>0)
		{
			...
			$(this).bind("submit", preventSubmit2);
			...
			if(typeof(self.attr("ajax"))!="undefined" && self.attr("ajax")=="true")
			{
				...
				$.ajax({
					...
					success: function(data){
						...
						form.one("submit", {'uploader':self}, submitForm2);
					}
				});
			}
			else
        ...
        $(".IMGSec2").each(function(index){
		var self = $(this);
		self.attr("id", "IMGSec2"+index);
		if(typeof(self.attr("startOn"))!="undefined")
		{
			if(self.attr("startOn")=="manually")
			{
				self.after("<input type='button' value='" + (typeof(self.attr("buttonCaption"))=="undefined" ? '':self.attr("buttonCaption")) + "' id='IMGSec2"+index+"start' class='uploadButton' />");
				$("#IMGSec2"+index+"start").click(function(){
					updateUploadifySettings2(self);
					self.uploadifyUpload();					
				});
			}
			else if(self.attr("startOn").substr(0, 8)=="onSubmit")
				$("#"+self.attr("startOn").substr(9)).one("submit", {'uploader':self}, submitForm2);
		}
		var fileExt = "";
		...
		self.uploadify({
			...
			'onComplete' : function(event, queueId, fileObj, response, data){
				...
				else
				{
					if(self.attr("startOn").substr(0, 8)=="onSubmit")
						$("#"+self.attr("startOn").substr(9)).append("<input type='hidden' name='IMGSec2Files[]' value='"+responseJSON.filename.replace("'", "%27")+"' />");
			...
			'onAllComplete' : function(event, data){
				if(typeof(self.attr("afterUpload"))!="undefined" && self.attr("afterUpload")!="link" && self.attr("afterUpload")!="filename" && self.attr("afterUpload")!="image" && self.attr("afterUpload")!="none")
				{
					...
					message = message.replace("IMGSec2filesUploaded", data.filesUploaded);
					message = message.replace("IMGSec2errors", data.errors);
					message = message.replace("IMGSec2totalSizeB", data.allBytesLoaded);
					message = message.replace("IMGSec2totalSizeKB", roundNumber2(data.allBytesLoaded/1024, 2));
					message = message.replace("IMGSec2totalSizeMB", roundNumber2(data.allBytesLoaded/1024/1024, 2));
					message = message.replace("IMGSec2totalSizeGB", roundNumber2(data.allBytesLoaded/1024/1024/1024, 2));
					message = message.replace("IMGSec2speedB", roundNumber2(Math.round(data.speed*1024), 2));
					message = message.replace("IMGSec2speedKB", roundNumber2(Math.round(data.speed), 2));
					message = message.replace("IMGSec2speedMB", roundNumber2(Math.round(data.speed/1024), 2));
					message = message.replace("IMGSec2speedGB", roundNumber2(Math.round(data.speed/1024/1024), 2));
					$("#" + self.attr("id") + "Queue").html(message);
				}
				if(self.attr("startOn").substr(0, 8)=="onSubmit")
				{
					var form = $("#"+self.attr("startOn").substr(9));
					form.unbind("submit", preventSubmit2);
					if(ajax=="true")
					{
