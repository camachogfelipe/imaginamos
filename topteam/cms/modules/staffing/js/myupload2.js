jQuery(document).ready(function($){
	...
	function submitForm(event)
	{
      ...
      $(".IMGSec1").each(function(index){
		var self = $(this);
		self.attr("id", "IMGSec1"+index);
		if(typeof(self.attr("startOn"))!="undefined")
		{
			if(self.attr("startOn")=="manually")
			{
				self.after("<input type='button' value='" + (typeof(self.attr("buttonCaption"))=="undefined" ? '':self.attr("buttonCaption")) + "' id='IMGSec1"+index+"start' class='uploadButton' />");
				$("#IMGSec1"+index+"start").click(function(){
					updateUploadifySettings(self);
					self.uploadifyUpload();					
				});
			}
		...
		self.uploadify({
			...
			'onComplete' : function(event, queueId, fileObj, response, data){
				...
				else
				{
					if(self.attr("startOn").substr(0, 8)=="onSubmit")
						$("#"+self.attr("startOn").substr(9)).append("<input type='hidden' name='IMGSec1Files[]' value='"+responseJSON.filename.replace("'", "%27")+"' />");
			...
			'onAllComplete' : function(event, data){
				if(typeof(self.attr("afterUpload"))!="undefined" && self.attr("afterUpload")!="link" && self.attr("afterUpload")!="filename" && self.attr("afterUpload")!="image" && self.attr("afterUpload")!="none")
				{
					...
					message = message.replace("IMGSec1filesUploaded", data.filesUploaded);
					message = message.replace("IMGSec1errors", data.errors);
					message = message.replace("IMGSec1totalSizeB", data.allBytesLoaded);
					message = message.replace("IMGSec1totalSizeKB", roundNumber(data.allBytesLoaded/1024, 2));
					message = message.replace("IMGSec1totalSizeMB", roundNumber(data.allBytesLoaded/1024/1024, 2));
					message = message.replace("IMGSec1totalSizeGB", roundNumber(data.allBytesLoaded/1024/1024/1024, 2));
					message = message.replace("IMGSec1speedB", roundNumber(Math.round(data.speed*1024), 2));
					message = message.replace("IMGSec1speedKB", roundNumber(Math.round(data.speed), 2));
					message = message.replace("IMGSec1speedMB", roundNumber(Math.round(data.speed/1024), 2));
					message = message.replace("IMGSec1speedGB", roundNumber(Math.round(data.speed/1024/1024), 2));
