jQuery(document).ready(function($){
	...
	function submitForm(event)
	{
      ...
      $(".LOG").each(function(index){
		var self = $(this);
		self.attr("id", "LOG"+index);
		if(typeof(self.attr("startOn"))!="undefined")
		{
			if(self.attr("startOn")=="manually")
			{
				self.after("<input type='button' value='" + (typeof(self.attr("buttonCaption"))=="undefined" ? '':self.attr("buttonCaption")) + "' id='LOG"+index+"start' class='uploadButton' />");
				$("#LOG"+index+"start").click(function(){
					updateUploadifySettings(self);
					self.uploadifyUpload();					
				});
			}
		...
		self.uploadify({
			...
			'onComplete' : function(event, queueId, fileObj, response, data){
				...
				else
				{
					if(self.attr("startOn").substr(0, 8)=="onSubmit")
						$("#"+self.attr("startOn").substr(9)).append("<input type='hidden' name='LOGFiles[]' value='"+responseJSON.filename.replace("'", "%27")+"' />");
			...
			'onAllComplete' : function(event, data){
				if(typeof(self.attr("afterUpload"))!="undefined" && self.attr("afterUpload")!="link" && self.attr("afterUpload")!="filename" && self.attr("afterUpload")!="image" && self.attr("afterUpload")!="none")
				{
					...
					message = message.replace("LOGfilesUploaded", data.filesUploaded);
					message = message.replace("LOGerrors", data.errors);
					message = message.replace("LOGtotalSizeB", data.allBytesLoaded);
					message = message.replace("LOGtotalSizeKB", roundNumber(data.allBytesLoaded/1024, 2));
					message = message.replace("LOGtotalSizeMB", roundNumber(data.allBytesLoaded/1024/1024, 2));
					message = message.replace("LOGtotalSizeGB", roundNumber(data.allBytesLoaded/1024/1024/1024, 2));
					message = message.replace("LOGspeedB", roundNumber(Math.round(data.speed*1024), 2));
					message = message.replace("LOGspeedKB", roundNumber(Math.round(data.speed), 2));
					message = message.replace("LOGspeedMB", roundNumber(Math.round(data.speed/1024), 2));
					message = message.replace("LOGspeedGB", roundNumber(Math.round(data.speed/1024/1024), 2));
