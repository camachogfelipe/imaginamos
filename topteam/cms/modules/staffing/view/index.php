			...
			<div id="shadowhead"></div>
              ...
              <div id="content">
                <div class="inner">
					...
					<!-- full width -->
                    <div class="widget" >
                        <div class="header"><span ><span class="ico gray pictures_folder"></span>SECCION staffing</span>
					...
					$db->doQuery("SHOW TABLES LIKE 'cms_staffing'",SHOW_TABLE_QUERY);
					//Si recibimos TRUE como respuesta quiere decir que si existe la tabla
					if($show = $db->show)
						{
						$db->doQuery("SELECT staffing_config_funcionality FROM cms_staffing_config WHERE staffing_config_id = 1",SELECT_QUERY);					
						$result = $db->results;
						//Imprimimos el formulario que necesitamos
						echo $htmlFormstaffing->printFormstaffing($result[0][staffing_config_funcionality]);
						}
					else
						//Si la tabla no existe presentamos el formulario de configuración del módulo "NEWS"
						echo $htmlFormstaffing->printFormInstall();
					?>
                    </fieldset>
