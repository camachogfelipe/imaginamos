...
$htmlFormsoluciones = new Forms($db);
?>
			...
			<div id="shadowhead"></div>
              ...
              <div id="content">
                <div class="inner">
					...
					<!-- full width -->
                    <div class="widget" >
                        <div class="header"><span ><span class="ico gray pictures_folder"></span>SECCION SOLUCIONES</span>
					...
					$db->doQuery("SHOW TABLES LIKE 'cms_soluciones'",SHOW_TABLE_QUERY);
					//Si recibimos TRUE como respuesta quiere decir que si existe la tabla
					if($show = $db->show)
						{
						$db->doQuery("SELECT soluciones_config_funcionality FROM cms_soluciones_config WHERE soluciones_config_id = 1",SELECT_QUERY);					
						$result = $db->results;
						//Imprimimos el formulario que necesitamos
						echo $htmlFormsoluciones->printFormsoluciones($result[0][soluciones_config_funcionality]);
						}
					else
						//Si la tabla no existe presentamos el formulario de configuración del módulo "NEWS"
						echo $htmlFormsoluciones->printFormInstall();
					?>
                    </fieldset>
