jQuery(document).ready(function($){
	...
	$(".IMGInt1").each(function(index){
		var self = $(this);
		self.attr("id", "IMGInt1"+index);
		if(typeof(self.attr("startOn"))!="undefined")
		{
			if(self.attr("startOn")=="manually")
			{
				self.after("<input type='button' value='" + (typeof(self.attr("buttonCaption"))=="undefined" ? '':self.attr("buttonCaption")) + "' id='IMGInt1"+index+"start' class='uploadButton' />");
				$("#IMGInt1"+index+"start").click(function(){
					updateUploadifySettings(self);
					self.uploadifyUpload();					
				});
			}
		...
		self.uploadify({
			...
			'onComplete' : function(event, queueId, fileObj, response, data){
				...
				else
				{
					if(self.attr("startOn").substr(0, 8)=="onSubmit")
						$("#"+self.attr("startOn").substr(9)).append("<input type='hidden' name='IMGInt1Files[]' value='"+responseJSON.filename.replace("'", "%27")+"' />");
			...
			'onAllComplete' : function(event, data){
				if(typeof(self.attr("afterUpload"))!="undefined" && self.attr("afterUpload")!="link" && self.attr("afterUpload")!="filename" && self.attr("afterUpload")!="image" && self.attr("afterUpload")!="none")
				{
					...
					message = message.replace("IMGInt1filesUploaded", data.filesUploaded);
					message = message.replace("IMGInt1errors", data.errors);
					message = message.replace("IMGInt1totalSizeB", data.allBytesLoaded);
					message = message.replace("IMGInt1totalSizeKB", roundNumber(data.allBytesLoaded/1024, 2));
					message = message.replace("IMGInt1totalSizeMB", roundNumber(data.allBytesLoaded/1024/1024, 2));
					message = message.replace("IMGInt1totalSizeGB", roundNumber(data.allBytesLoaded/1024/1024/1024, 2));
					message = message.replace("IMGInt1speedB", roundNumber(Math.round(data.speed*1024), 2));
					message = message.replace("IMGInt1speedKB", roundNumber(Math.round(data.speed), 2));
					message = message.replace("IMGInt1speedMB", roundNumber(Math.round(data.speed/1024), 2));
					message = message.replace("IMGInt1speedGB", roundNumber(Math.round(data.speed/1024/1024), 2));
