jQuery(document).ready(function($){
	...
	$(".IMGR2").each(function(index){
		var self = $(this);
		self.attr("id", "IMGR2"+index);
		if(typeof(self.attr("startOn"))!="undefined")
		{
			if(self.attr("startOn")=="manually")
			{
				self.after("<input type='button' value='" + (typeof(self.attr("buttonCaption"))=="undefined" ? '':self.attr("buttonCaption")) + "' id='IMGR2"+index+"start' class='uploadButton' />");
				$("#IMGR2"+index+"start").click(function(){
					updateUploadifySettings(self);
					self.uploadifyUpload();					
				});
			}
		...
		self.uploadify({
			...
			'onComplete' : function(event, queueId, fileObj, response, data){
				...
				else
				{
					if(self.attr("startOn").substr(0, 8)=="onSubmit")
						$("#"+self.attr("startOn").substr(9)).append("<input type='hidden' name='IMGR2Files[]' value='"+responseJSON.filename.replace("'", "%27")+"' />");
			...
			'onAllComplete' : function(event, data){
				if(typeof(self.attr("afterUpload"))!="undefined" && self.attr("afterUpload")!="link" && self.attr("afterUpload")!="filename" && self.attr("afterUpload")!="image" && self.attr("afterUpload")!="none")
				{
					...
					message = message.replace("IMGR2filesUploaded", data.filesUploaded);
					message = message.replace("IMGR2errors", data.errors);
					message = message.replace("IMGR2totalSizeB", data.allBytesLoaded);
					message = message.replace("IMGR2totalSizeKB", roundNumber(data.allBytesLoaded/1024, 2));
					message = message.replace("IMGR2totalSizeMB", roundNumber(data.allBytesLoaded/1024/1024, 2));
					message = message.replace("IMGR2totalSizeGB", roundNumber(data.allBytesLoaded/1024/1024/1024, 2));
					message = message.replace("IMGR2speedB", roundNumber(Math.round(data.speed*1024), 2));
					message = message.replace("IMGR2speedKB", roundNumber(Math.round(data.speed), 2));
					message = message.replace("IMGR2speedMB", roundNumber(Math.round(data.speed/1024), 2));
					message = message.replace("IMGR2speedGB", roundNumber(Math.round(data.speed/1024/1024), 2));
