jQuery(document).ready(function($){
	...
	function submitForm2(event)
	{
		...
		var self = $(this);
		self.attr("id", "PDF"+index);
		if(typeof(self.attr("startOn"))!="undefined")
		{
			if(self.attr("startOn")=="manually")
			{
				self.after("<input type='button' value='" + (typeof(self.attr("buttonCaption"))=="undefined" ? '':self.attr("buttonCaption")) + "' id='PDF"+index+"start' class='uploadButton' />");
				$("#PDF"+index+"start").click(function(){
					updateUploadifySettings2(self);
					self.uploadifyUpload();					
				});
			}
		...
		self.uploadify({
			...
			'onComplete' : function(event, queueId, fileObj, response, data){
				...
				else
				{
					if(self.attr("startOn").substr(0, 8)=="onSubmit")
						$("#"+self.attr("startOn").substr(9)).append("<input type='hidden' name='PDFFiles[]' value='"+responseJSON.filename.replace("'", "%27")+"' />");
			...
			'onAllComplete' : function(event, data){
				if(typeof(self.attr("afterUpload"))!="undefined" && self.attr("afterUpload")!="link" && self.attr("afterUpload")!="filename" && self.attr("afterUpload")!="image" && self.attr("afterUpload")!="none")
				{
					...
					message = message.replace("PDFfilesUploaded", data.filesUploaded);
					message = message.replace("PDFerrors", data.errors);
					message = message.replace("PDFtotalSizeB", data.allBytesLoaded);
					message = message.replace("PDFtotalSizeKB", roundNumber2(data.allBytesLoaded/1024, 2));
					message = message.replace("PDFtotalSizeMB", roundNumber2(data.allBytesLoaded/1024/1024, 2));
					message = message.replace("PDFtotalSizeGB", roundNumber2(data.allBytesLoaded/1024/1024/1024, 2));
					message = message.replace("PDFspeedB", roundNumber2(Math.round(data.speed*1024), 2));
					message = message.replace("PDFspeedKB", roundNumber2(Math.round(data.speed), 2));
					message = message.replace("PDFspeedMB", roundNumber2(Math.round(data.speed/1024), 2));
					message = message.replace("PDFspeedGB", roundNumber2(Math.round(data.speed/1024/1024), 2));
