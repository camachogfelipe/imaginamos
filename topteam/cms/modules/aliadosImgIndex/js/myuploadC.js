jQuery(document).ready(function($){
	function roundNumberC(num, dec)
	{
		var result = Math.round( Math.round( num * Math.pow( 10, dec + 1 ) ) / Math.pow( 10, 1 ) ) / Math.pow(10,dec);
		return result;
	}
	function updateUploadifySettingsC(uploader)
	{
	...
	function submitFormC(event)
	{
		...
		if(queue.children().length>0)
		{
			...
			$(this).bind("submit", preventSubmitC);
			...
			if(typeof(self.attr("ajax"))!="undefined" && self.attr("ajax")=="true")
			{
				...
				$.ajax({
					...
					success: function(data){
						...
						form.one("submit", {'uploader':self}, submitFormC);
					}
				});
			}
			else
	...
	$(".IMC").each(function(index){
		var self = $(this);
		self.attr("id", "IMC"+index);
		if(typeof(self.attr("startOn"))!="undefined")
		{
			if(self.attr("startOn")=="manually")
			{
				self.after("<input type='button' value='" + (typeof(self.attr("buttonCaption"))=="undefined" ? '':self.attr("buttonCaption")) + "' id='IMC"+index+"start' class='uploadButton' />");
				$("#IMC"+index+"start").click(function(){
					updateUploadifySettingsC(self);
					self.uploadifyUpload();					
				});
			}
			else if(self.attr("startOn").substr(0, 8)=="onSubmit")
				$("#"+self.attr("startOn").substr(9)).one("submit", {'uploader':self}, submitFormC);
		}
		var fileExt = "";
		...
		self.uploadify({
			...
			'onComplete' : function(event, queueId, fileObj, response, data){
				...
				else
				{
					if(self.attr("startOn").substr(0, 8)=="onSubmit")
						$("#"+self.attr("startOn").substr(9)).append("<input type='hidden' name='IMCFiles[]' value='"+responseJSON.filename.replace("'", "%27")+"' />");
			...
			'onAllComplete' : function(event, data){
				if(typeof(self.attr("afterUpload"))!="undefined" && self.attr("afterUpload")!="link" && self.attr("afterUpload")!="filename" && self.attr("afterUpload")!="image" && self.attr("afterUpload")!="none")
				{
					...
					message = message.replace("IMCfilesUploaded", data.filesUploaded);
					message = message.replace("IMCerrors", data.errors);
					message = message.replace("IMCtotalSizeB", data.allBytesLoaded);
					message = message.replace("IMCtotalSizeKB", roundNumberC(data.allBytesLoaded/1024, 2));
					message = message.replace("IMCtotalSizeMB", roundNumberC(data.allBytesLoaded/1024/1024, 2));
					message = message.replace("IMCtotalSizeGB", roundNumberC(data.allBytesLoaded/1024/1024/1024, 2));
					message = message.replace("IMCspeedB", roundNumberC(Math.round(data.speed*1024), 2));
					message = message.replace("IMCspeedKB", roundNumberC(Math.round(data.speed), 2));
					message = message.replace("IMCspeedMB", roundNumberC(Math.round(data.speed/1024), 2));
					message = message.replace("IMCspeedGB", roundNumberC(Math.round(data.speed/1024/1024), 2));
					$("#" + self.attr("id") + "Queue").html(message);
				}
				if(self.attr("startOn").substr(0, 8)=="onSubmit")
				{
					var form = $("#"+self.attr("startOn").substr(9));
					form.unbind("submit", preventSubmitC);
					if(ajax=="true")
					{
