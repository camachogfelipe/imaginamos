...
$htmlFormaliadosImgIndex = new Forms($db);
?>
			...
			<div id="shadowhead"></div>
              ...
              <div id="content">
                <div class="inner">
					...
					<!-- full width -->
                    <div class="widget" >
                        <div class="header"><span ><span class="ico gray pictures_folder"></span>SECCION IMAGNES ALIADOS EN EL INDEX</span>
					...
					$db->doQuery("SHOW TABLES LIKE 'cms_aliadosImgIndex'",SHOW_TABLE_QUERY);
					//Si recibimos TRUE como respuesta quiere decir que si existe la tabla
					if($show = $db->show)
						{
						$db->doQuery("SELECT aliadosImgIndex_config_funcionality FROM cms_aliadosImgIndex_config WHERE aliadosImgIndex_config_id = 1",SELECT_QUERY);					
						$result = $db->results;
						//Imprimimos el formulario que necesitamos
						echo $htmlFormaliadosImgIndex->printFormaliadosImgIndex($result[0][aliadosImgIndex_config_funcionality]);
						}
					else
						//Si la tabla no existe presentamos el formulario de configuración del módulo "NEWS"
						echo $htmlFormaliadosImgIndex->printFormInstall();
					?>
                    </fieldset>
             ...
             <!-- <script language="javascript">$("#contenido1").cleditor(); $("#contenido2").cleditor();</script>  -->
