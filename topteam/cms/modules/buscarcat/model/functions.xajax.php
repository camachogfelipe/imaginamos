...
{  
	...
	if($texto1 == "")
		...
		$objResponse->script("showError('Ingrese el t&iacute;tulo',3000);");
		...
		$db->doQuery("UPDATE cms_buscar_catexp SET buscar_catexp_nombre = '$texto1' WHERE buscar_catexp_id = '$formData[id]'",UPDATE_QUERY);
		
		$objResponse->script("loading('Procesando',1);");
		$objResponse->script("setTimeout(\"unloading()\", 2000 );");
		$objResponse->script("showSuccess('Actualización correcta',3000);");
		$objResponse->script("setInterval('window.location.href = \"../view/indexE.php\"', 2000);");
	}
...
{  
	...
	if($texto1 == "")
		...
		$db->doQuery("INSERT INTO cms_buscar_catexp (buscar_catexp_id, buscar_catexp_nombre, buscar_catexp_activa) VALUES('','$texto1','1')",INSERT_QUERY);
...
{  
	...
	if($texto1 == "")
		...
		$db->doQuery("INSERT INTO cms_buscar_catpro (buscar_catpro_id, buscar_catpro_nombre, buscar_catpro_activa) VALUES('','$texto1','1')",INSERT_QUERY);
		
		$objResponse->script("loading('Procesando',1);");
		$objResponse->script("setTimeout(\"unloading()\", 2000 );");
		$objResponse->script("showSuccess('Carga correcta',3000);");
		$objResponse->script("setInterval('window.location.href = \"../view/indexP.php\"', 2000);");
...
{  
	...
	if($texto1 == "")
		...
		$db->doQuery("UPDATE cms_buscar_catpro SET buscar_catpro_nombre = '$texto1' WHERE buscar_catpro_id = '$formData[id]'",UPDATE_QUERY);
...
$xajax->registerFunction("edit3");
$xajax->registerFunction("agregar2");
$xajax->registerFunction("agregar3");
