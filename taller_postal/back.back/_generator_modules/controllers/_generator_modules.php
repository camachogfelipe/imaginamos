...
class _generator_modules extends Back_Controller {
    ...
    public function get_lista_head($value,$key) {
        switch (strtolower($value)) {
            case "view":
                $this->thead[$key] = "<th>".$key."</th>";
            break;
            default:
            break;
        }
   ...
   public function get_componente($metodo = "", $label = "", $key = "",$inst = "",$span = "span3",$class_name ="") {
       ...
       switch (strtolower($metodo)) {
            case "imagen":
                $html = "\n\$data['form_content'] .= \$this->imagen(\$obj->".$key.",\$obj->imagen_path, \"" . $label . "\", \"" . $inst . "\", false, \$obj->is_rule(\"" . $key . "\",\"required\"), \"" . $span . "\");\n".$html;
                break;
            case "input":
                $html .= "\$data['form_content'] .= \$this->input(\$obj->".$key.", \"".$key."\",\$obj->get_rule(\"".$key."\",\"max_length\") ,\"" . $label . "\", \"Maximo \".\$obj->get_rule(\"".$key."\",\"max_length\").\" caracteres\", \$obj->is_rule(\"" . $key . "\",\"required\"), \"" . $span . "\");\n";
                break;
            case "inputcolor":
                $html .= "\$data['form_content'] .= \$this->inputColor(\$obj->".$key.", \"".$key."\" ,\"" . $label . "\", \"Formato Hexadecimal\", \$obj->is_rule(\"" . $key . "\",\"required\"), \"" . $span . "\",\"hex\");\n";
                break;
            case "inputtime":
                $html .= "\$data['form_content'] .= \$this->inputTime(\$obj->".$key.", \"".$key."\" ,\"" . $label . "\", \"Formato 12 Horas\", \$obj->is_rule(\"" . $key . "\",\"required\"), \"" . $span . "\",\$formato = \"tp-default\");\n";
                break;
            case "inputhidden":
                $html .= "\$data['form_content'] .= \$this->inputHidden(\$obj->".$key.", \"".$key."\", \$obj->get_rule(\"".$key."\",\"max_length\"));";
                break;
            case "text":
                $html .= "\$data['form_content'] .= \$this->text(\$obj->".$key.", \"".$key."\", \"".$label."\", \"Maximo \".\$obj->get_rule(\"".$key."\",\"max_length\").\" caracteres\", \$obj->is_rule(\"".$key."\",\"required\"), \"".$span."\", true, false, \$obj->get_rule(\"".$key."\",\"max_length\"),3,4);\n";
                break;
            case "combox":
                 $key1 = str_replace("_id", "", $key);
                 $html .= "		\$var_".$key." = new ".$class_name."();\n\$data['form_content'] .= \$this->combox(\$obj->".$key.",\$var_".$key."->get_".$key1."_list(),\"".$key."\", \"".$label."\", \"".$inst."\", \$obj->is_rule(\"".$key."\",\"required\"), \"".$span."\");\n";
                break;
            case "input_money":
                $html .= "\$data['form_content'] .= \$this->input_money(\$obj->".$key.", \"".$key."\" ,\"$\",\"" . $label . "\", \"".$inst."\", \$obj->is_rule(\"" . $key . "\",\"required\"), \"" . $span . "\");\n";
                break;
            case "select_multiple":
                $html .= "";
                break;
            case "inputdate":
                 $html .= "\$data['form_content'] .= \$this->inputDate(\$obj->".$key.", \"".$key."\" ,\"" . $label . "\", \"\", is_rule(\"" . $key . "\",\"required\"), \"" . $span . "\",\$formato = \"dd/mm/yyyy\");\n";
                 break;
            case "inputfile":
                 $html .= "\$data['form_content'] .= \$this->inputFile(\$obj->".$key.", \"".$key."\" ,\"" . $label . "\", \"Maximo 200 mb\", \$obj->is_rule(\"" . $key . "\",\"required\"), \"" . $span . "\");\n";
                 break;
            default:
                break;
        }
