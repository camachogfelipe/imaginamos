...
class  Back_Controller extends CMS_Controller {
     ...
     * @param type $n
     * @return type
     * @uses $data['form_content'] .= $this->imagen($obj->key,$obj->imagen_path, "label", "instrucciones", false, $obj->is_rule("key","required"), "span3");
     */                        
   ...
   public function inputCheck($dato="", $name = "", $label="Texto", $instructios = "Seleccione...!" , $required = true, $span ="span8") {
        $data['title'] = $label ;
        $data['name'] = $name ;
        $data['dato'] = $dato;
        $data['instrutions'] = $instructios;
        ...
        $data['class_span'] = $span;
        $data['class_required'] = ($required)?"req":"";   
        return $this->buildajax(ADMINPATH . 'component/input_check',$data, $required = true, $span ="span8");
     }
   /**
    * @param type $dato
    * @param type $name
    ...
    * @param string $span
    * @return type
    * @uses $data['form_content'] .= $this->inputColor($obj->archivo, "archivo", "Archivo", "Maximo 200 mb" , $obj->is_rule("archivo","required"), $span ="span8"); 
    */
    ...
    * @param type $formato
    * @return type
    * @uses $data['form_content'] .= $this->inputColor($obj->color, "color", "Color", "Formato Hexadecimal",$obj->is_rule("color","required"),"span2","hex"); 
    */
    ...
    * @param type $formato
    * @return type
    * @uses $this->inputDate($obj->fecha, "fecha", "Fecha", "",$obj->is_rule("fecha","required"),"span2","dd/mm/yyyy");
    */
    ...
    * @param type $formato
    * @return type
    * @uses $data['form_content'] .= $this->inputTime($obj->time, "time", "Hora", "Formato 12 Horas",$obj->is_rule("time","required"),"span2","tp-default"); 
    */
    ...
    * @return type
    * @uses $data['form_content'] .= $this->inputHidden($obj->id, "id",false); 
    * 
    */
        ...
        foreach ($obj->_fields as $key) {
            ...
            try {
                if (!is_null($key) && $key != "id") {
                    ...
                    $d = strpos($key,'imagen');
                    $i = 0;
                    if ($d !== false or ($key == 'imagen_id')) {
                        $i = str_replace("_id", "", $key);
                        $i = str_replace("imagen", "", $i);
                        $i= (!is_numeric($i))?0:$i;
                        ...
                        switch ($key) {
                            case "link_path":
                                ...
                                $obj->{$key} = ($IMG != false) ? $IMG : (($this->_post('link_path') !== "") ? $this->_post('link_path') : NULL);
                                break;
                            default:
                                $obj->{$key} = $default;
                                break;
   ...
   public function loadobj(&$obj1,$class = ''){
        ...
        if ($archivo != "") {
	      ...
	      $destino = md5(uniqid(mt_rand())).".".$type;
            ...
            if (!@move_uploaded_file($_FILES[$field]['tmp_name'],$this->dirImg.$destino)) {
                ...
                $this->redimensionar_imagen($this->dirImg.$destino, str_replace(".".$type,"_th.".$type,$destino), 200, 200, 1); 
            }
    ...
    /**
     * @author Felipe Camacho <www.cogroupsas.com>
     * @param type $src
     * @param string $dst
     * @param type $width
     * @param type $height
    ...
    function redimensionar_imagen($src, $dst, $width, $height, $crop = 0) {
         $dir_th = $this->dirImg."thumb/";                    
        if (!file_exists($dir_th))
          @mkdir($dir_th, 0777, true);
        $dst = $dir_th. $dst;
        ...
        public function Error_No_Found($datos)
    {
        ...
        $this->output->set_header('Content-Length: ' . strlen($datos));
