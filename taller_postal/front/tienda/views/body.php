  ...
  <div class="content_cat right">
    <div class="clearfix">
        ...
        <div <?php if($i == 3){echo ' style="margin-right:0" '; $i=0;} $i++; ?>  class="item_cat left">
            <a href="<?php echo base_url(); ?>custom_prod/info/<?php echo $categoria ;?>/<?php echo $product->id; ?>" class="item_dest">
        ...
        <h4>por <a href="<?php echo base_url(); ?>disenadores/disenador/<?php echo $disenador->id ?>"><?php echo $disenador->nombre ?></a></h4>
        <div class="select_colors">
         <?php 
		 $i= 1;
		 foreach ($color as $cvalue): ?>  
          <a class="cambia_color" data-url="<?php echo base_url(); ?>custom_prod/guarda_color" id="<?php echo $i;?>" rel="<?php echo base_url().$cvalue->imagen_path; ?>" style="background-color:<?php echo $cvalue->color_valor; ?>;"></a>
        <?php 
		$i++;
		endforeach;
...
$(document).ready(function(){
	...
	var id = $(this).attr('id');
		$.post("<?php echo base_url(); ?>custom_prod/guarda_color/"+id,function(data){
            /*alert(data);*/
        });
	});
});
