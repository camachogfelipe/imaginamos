	...
	public function index() {
        ...
        public function add_to_cart() {
           ...
           $mejase = $this->_post('mensaje'); // mensaje
            ...
            $color_diseno_actual = $this->_post('color_diseno_actual'); 
			$cantidad_producto_precio = is_float($this->_post('cantidad_producto_precio'))?$this->_post('cantidad_producto_precio'):0; 
			$papel_sobre_precio = is_float($this->_post('papel_sobre_precio'))?$this->_post('papel_sobre_precio'):0; 
			$cantidad_sobre_precio = is_float($this->_post('cantidad_sobre_precio'))?$this->_post('cantidad_sobre_precio'):0;   
			$sticker_cierre = is_float($this->_post('sticker_cierre'))?$this->_post('sticker_cierre'):0;
			$papel_producto_precio = is_float($this->_post('papel_producto_precio'))?$this->_post('papel_producto_precio'):0; 
			$producto_precio = is_float($this->_post('producto_precio'))?$this->_post('producto_precio'):0; 
            ...
            $precio_pro = (($papel_producto_precio*$producto_precio)+$precio_diseno+($precio_dorso*$cantidad_cat_id)); 
            $precio_sobre_pro = ($papel_sobre_precio*$cantidad_sobre_precio)+($cantidad_sobre_id*$sticker_cierre); 
            $color = $producto->get_color('color','producto','color_id');  
            ...
            if($producto->exists()){
                ...
                $data = array(
				   ...
				   'color_diseno_actual' => $color_diseno_actual,
                   'mensaje' => $mejase,
                   'sobre_id' => $sobre_id,
