...
/*
 * @file               : oficinas.php
 * @brief              : Script para la interaccion con la tabla oficinas
...
if(isset($_GET["id_edit"])){
  $db->doQuery("SELECT * FROM oficinas WHERE id=".(int)$_GET["id_edit"], SELECT_QUERY);
  $id = $db->results[0]["id"];
...
if(isset($_POST["id"])){
  ...
  if( (int)$_POST["id"]>0 ){
    $update=$db->doQuery("UPDATE oficinas SET  ciudad='".$ciudad_c."' WHERE id=".(int)$_POST["id"], UPDATE_QUERY);
    ...
    $insert=$db->doQuery("INSERT INTO oficinas ( ciudad) VALUES ( '".$ciudad_c."');", INSERT_QUERY);
    ...
    $retorno = ClassFIle::UploadImagenFile("img", "../../../../img/oficinas/", "original_" . $idinsertado, "redimension_" . $idinsertado, 500, 500);
    if ($retorno["Status"]=="Uploader"){
    $db->doQuery("UPDATE oficinas SET imagen='".$retorno["NameFile"]."' WHERE id=".$idinsertado, UPDATE_QUERY);
    ...
    $retorno = ClassFIle::UploadFile("archivo", "../../../../img/oficinas/", "oficinas_" . $idinsertado);
    if ($retorno["Status"]=="Uploader"){
    $db->doQuery("UPDATE oficinas SET archivo='".$retorno["NameFile"]."' WHERE id=".$idinsertado, UPDATE_QUERY);
    }
...
if( isset($_GET["id_del"]) && isset($_GET["confirm"]) ){
  if($_GET["confirm"] == base64_encode(md5($_GET["id_del"]))){
    $db->doQuery("DELETE FROM oficinas WHERE id=".(int)$_GET["id_del"], DELETE_QUERY);
  }
}

// Obtenemos las datos
$db->doQuery("SELECT * FROM oficinas ORDER BY id ASC", SELECT_QUERY);
$dataAll = $db->results;
?>
...
<div class="widget">
  <div class="header">
    <span>
      ...
      Administraci&oacute;n de Contenidos Oficinas
    </span>
  </div>
  ...
  <div class="content">
    <div class="formEl_b">
      <fieldset>
        <?php if($id>0){
          echo "<h3>Editando Oficinas '".$id."'</h3>";
        }else{
          echo "<h3>Oficinas</h3>";
        } ?>
          ...
          <tbody>
            ...
            <tr class="odd gradeX">
              ...
              <td class="center " width="100px">
                <a class="uibutton icon add" href="../../descripcion_oficina/view/index.php?seccion=form&idc=<?= $data[$i]["id"]?>">Agregar Items</a><br/>
